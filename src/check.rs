...
pub fn check_program(
    ...
    // Stone 243.6b — fused pre-inference validator pass. All independent
    // walker passes (sequi confirmed order-independent accumulator-drains)
    // collapsed into one per-body traversal: one `for func` running all
...
fn register_builtins(env: &mut CheckEnv) {
    ...
    env.register(
        ...
        TypeScheme {
            ...
            ret: TypeExpr::Parametric {
                head: "wat::core::Option".into(),
                args: vec![TypeExpr::Path(":wat::rete::AxisViolation".into())],
            },
            rest_param_type: None,
        },
    );
