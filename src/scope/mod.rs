...
//! ## Contents
//!
//! - `identifier` (from `wat-reader`) — `Identifier`, `ScopeId`, `fresh_scope`:
//!   the bare sets-of-scopes primitives. The parser emits `Identifier::bare`
//!   (empty scope set); the macro expander mints fresh `ScopeId`s and calls
