...
impl CheckError {
    ...
    /// error. Span lives at the outer struct; variant data in `kind`.
    pub fn diagnostic(&self) -> crate::diagnostic::Diagnostic {
        use crate::diagnostic::Diagnostic;
        let span = &self.span;

        // ONE elide-aware location mechanism: unknown spans are omitted so no
        // "<runtime>:0:0" value ever surfaces in a diagnostic field.  Route
        // EVERY span-to-field emission through this helper — unguarded direct
        // format!("{}", span) in a .field() call is structurally impossible
        // after this refactor because there is exactly one path.
        fn loc_field(d: Diagnostic, name: &str, s: &Span) -> Diagnostic {
            if s.is_unknown() { d } else { d.field(name, s.to_string()) }
        }

        match &self.kind {
            CheckErrorKind::ArityMismatch { callee, expected, got } => {
                let diag = Diagnostic::new("ArityMismatch")
            ...
            CheckErrorKind::ReturnTypeMismatch { function, expected, got, remedies } => {
                let diag = Diagnostic::new("ReturnTypeMismatch")
            ...
            CheckErrorKind::UnknownCallee { callee } => {
                let diag = Diagnostic::new("UnknownCallee").field("callee", callee.as_str());
                loc_field(diag, "span", span)
            }
            CheckErrorKind::MalformedForm { head, reason, remedies } => {
                let diag = Diagnostic::new("MalformedForm")
            ...
            CheckErrorKind::CommCallOutOfPosition { callee } => {
                let diag = Diagnostic::new("CommCallOutOfPosition")
                    .field("callee", callee.as_str());
                loc_field(diag, "span", span)
            }
            CheckErrorKind::ScopeDeadlock {
                ...
                let diag = Diagnostic::new("ScopeDeadlock")
            ...
            CheckErrorKind::ProcessJoinBeforeOutputDrain {
                ...
                let diag = Diagnostic::new("ProcessJoinBeforeOutputDrain")
                    .field("process_identifier", process_identifier.as_str())
                    .field("output_accessor", output_accessor.as_str());
                let diag = loc_field(diag, "join_location", span);
                loc_field(diag, "output_location", output_accessor_span)
            }
            CheckErrorKind::ProcessJoinHoldsStdinSender {
                process_identifier,
                stdin_sender_span,
            } => {
                let diag = Diagnostic::new("ProcessJoinHoldsStdinSender")
                    .field("process_identifier", process_identifier.as_str());
                let diag = loc_field(diag, "join_location", span);
                loc_field(diag, "bind_location", stdin_sender_span)
            }
            CheckErrorKind::ChannelPairDeadlock {
                callee,
                sender_arg,
                ...
                let diag = Diagnostic::new("ChannelPairDeadlock")
                    .field("callee", callee.as_str())
            ...
            CheckErrorKind::BareLegacyPrimitive { primitive, fqdn } => {
                let diag = Diagnostic::new("BareLegacyPrimitive")
                    .field("primitive", primitive.as_str())
                    .field("fqdn", fqdn.as_str());
                loc_field(diag, "location", span)
            }
            CheckErrorKind::BareLegacyUnitType => {
                let diag = Diagnostic::new("BareLegacyUnitType")
                    .field("primitive", ":()")
                    .field("fqdn", ":wat::core::nil");
                loc_field(diag, "location", span)
            }
            CheckErrorKind::BareLegacyUnitName => {
                let diag = Diagnostic::new("BareLegacyUnitName")
                    .field("retired", ":wat::core::unit")
                    .field("fqdn", ":wat::core::nil");
                loc_field(diag, "location", span)
            }
            CheckErrorKind::BareLegacyLetStar => {
                let diag = Diagnostic::new("BareLegacyLetStar")
                    .field("retired", ":wat::core::let*")
                    .field("fqdn", ":wat::core::let");
                loc_field(diag, "location", span)
            }
            CheckErrorKind::BareLegacyLambda => {
                let diag = Diagnostic::new("BareLegacyLambda")
                    .field("retired", ":wat::core::lambda")
                    .field("fqdn", ":wat::core::fn");
                loc_field(diag, "location", span)
            }
            CheckErrorKind::BareLegacyLowercaseFn => {
                let diag = Diagnostic::new("BareLegacyLowercaseFn")
                    .field("retired", ":fn(...)->ret")
                    .field("fqdn", ":wat::core::Fn(...)->ret");
                loc_field(diag, "location", span)
            }
            CheckErrorKind::BareLegacyContainerHead { head, fqdn } => {
                let diag = Diagnostic::new("BareLegacyContainerHead")
                    .field("head", head.as_str())
                    .field("fqdn", fqdn.as_str());
                loc_field(diag, "location", span)
            }
            CheckErrorKind::BareLegacyStreamPath { old, new } => {
                let diag = Diagnostic::new("BareLegacyStreamPath")
                    .field("old", old.as_str())
                    .field("new", new.as_str());
                loc_field(diag, "location", span)
            }
            CheckErrorKind::BareLegacyTelemetryServicePath { old, new } => {
                let diag = Diagnostic::new("BareLegacyTelemetryServicePath")
                    .field("old", old.as_str())
                    .field("new", new.as_str());
                loc_field(diag, "location", span)
            }
            CheckErrorKind::BareLegacyLruCacheServicePath { old, new } => {
                let diag = Diagnostic::new("BareLegacyLruCacheServicePath")
                    .field("old", old.as_str())
                    .field("new", new.as_str());
                loc_field(diag, "location", span)
            }
            CheckErrorKind::BareLegacyKernelQueuePath { old, new } => {
                let diag = Diagnostic::new("BareLegacyKernelQueuePath")
                    .field("old", old.as_str())
                    .field("new", new.as_str());
                loc_field(diag, "location", span)
            }
            CheckErrorKind::SandboxScopeLeak { offending_name, outer_define_span } => {
                let diag = Diagnostic::new("SandboxScopeLeak")
                    .field("offending_name", offending_name.as_str());
                let diag = loc_field(diag, "call_span", span);
                loc_field(diag, "outer_define_span", outer_define_span)
            }
            CheckErrorKind::DefRedefForbidden { name, original_def_span } => {
                let diag = Diagnostic::new("DefRedefForbidden")
                    .field("name", name.as_str());
                let diag = loc_field(diag, "prior_loc", original_def_span);
                loc_field(diag, "current_loc", span)
            }
            CheckErrorKind::DefRedefTypeChange { name, prior_type, new_type, original_def_span } => {
                let diag = Diagnostic::new("DefRedefTypeChange")
                    .field("name", name.as_str())
            ...
            CheckErrorKind::BareLegacyMainSignature => {
                let diag = Diagnostic::new("BareLegacyMainSignature")
            ...
            CheckErrorKind::BareLegacyConsolePath { path } => {
                let diag = Diagnostic::new("BareLegacyConsolePath")
            ...
            CheckErrorKind::DefRestrictedCallerNotAllowed {
                ...
                let diag = Diagnostic::new("DefRestrictedCallerNotAllowed")
                    .field("callee", callee.as_str())
            ...
            CheckErrorKind::NoMatchingClauseAtCallSite {
                ...
                let diag = Diagnostic::new("NoMatchingClauseAtCallSite")
                    .field("name", name.as_str())
            ...
            CheckErrorKind::GuardExprNotBoolean { defclause_name, clause_index, got_type } => {
                let diag = Diagnostic::new("GuardExprNotBoolean")
            ...
            CheckErrorKind::EnsureFnInvalid { defclause_name, clause_index, reason } => {
                let diag = Diagnostic::new("EnsureFnInvalid")
                    .field("defclause_name", defclause_name.as_str())
                    .field("clause_index", *clause_index)
                    .field("reason", reason.as_str());
                loc_field(diag, "span", span)
