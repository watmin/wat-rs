...
impl crate::to_edn::ToEdn for StdlibError {
    ...
    ///
    /// Arc 296 Strike 3a: variant fields are generated by `#[derive(ToEdn)]` on
    /// `StdlibErrorKind`; `:span` is spliced in last by `splice_span` (elided
    /// when unknown). The `cause` field uses `error_edn_of` (floor form) matching
    /// the deleted hand-written `cause.error_edn()` call.
