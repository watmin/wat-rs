...
pub(super) fn is_resolvable_call_head(head: &str, sym: &SymbolTable, macros: &MacroRegistry) -> bool {
    ...
    // `UseDeclarations::covers` and emits its own located diagnostic. Answering here would
    // duplicate that question and answer it WORSE: this predicate has no `use_decls` to consult.
    // So the rung defers — it accepts the shape and lets the declaration check adjudicate the
    // name, which it does, on the very next lines.
    //
    // ⚠ NOT a surviving blanket. The old blanket accepted `:wat::*` names NOTHING validated.
    // Every `:rust::*` name reaching here IS validated, immediately, by the authority that owns
    // it.
    if head.starts_with(":rust::") {
        return true;
    }
