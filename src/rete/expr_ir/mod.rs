...
mod eval;
// ⚠ `eval_lower` intentionally NOT re-exported here, unlike grok's original split. On this tree
// it is registered as `#[wat_intrinsic(":wat::rete::lower")]` (arc 255 Stone P6-c-W5c, main's
// syntax — grok's version at this commit was still a hand-dispatched fn taking `args: &[WatAST]`).
// The intrinsic macro wires it into the dispatch table by attribute, so nothing calls it by this
// path anymore; re-exporting it left `cargo build` warning `unused import: eval_lower`.
pub(crate) use eval::{apply_op, exec, exec_call, exec_value, exec_where};
...
fn lower_list(items: &[WatAST], span: &Span, cx: &mut LowerCx) -> Result<Expr, LowerError> {
    ...
    if head == ":wat::holon::literal" {
        ...
        let holon = crate::holon::to_holon_inner(data, items[1].span())
...
fn lower_pat(ast: &WatAST, cx: &mut LowerCx) -> Result<Pat, LowerError> {
    ...
    match ast {
        ...
        // Nested variant `[Variant {:k v}]` (no body) — a pattern, not an arm. HEAD KEY-FIRST.
