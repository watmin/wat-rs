...
///
/// ONE COPY. This resolution — `decompose_variant` (main's one door; the grok-rete
/// original used `rsplit_once("::")`), registry `get`, match `TypeDef::Enum`,
...
pub(crate) fn enum_variant_ctor<'a>(
    ...
    let (enum_path, variant) = wat_reader::identifier::decompose_variant(head)?;
...
//
// rune:lint(cited-name-absent) eval_alpha_match — deleted at arc 255 Stone P6-c-W5a; the dispatch now lives as a #[wat_intrinsic] handler in src/intrinsic/rete.rs calling alpha_match_inner directly.
// rune:lint(cited-name-absent) eval_alpha_match_local — deleted at arc 255 Stone P6-c-W5a; the dispatch now lives as a #[wat_intrinsic] handler in src/intrinsic/rete.rs calling alpha_match_inner_local directly.
// rune:lint(cited-name-absent) eval_alpha_match_kind — deleted at arc 255 Stone P6-c-W5a alongside its siblings; the shared arity-guard dispatch it named no longer exists as a standalone fn.
// rune:lint(cited-name-absent) eval_alpha_match_under — deleted at arc 255 Stone P6-c-W5a; the dispatch now lives as a #[wat_intrinsic] handler in src/intrinsic/rete.rs calling alpha_match_inner_seeded directly.
// rune:lint(cited-name-absent) eval_cond_has_deferred_constraint — deleted at arc 255 Stone P6-c-W5a; the dispatch now lives as a #[wat_intrinsic] handler in src/intrinsic/rete.rs calling cond_has_deferred_constraint directly.
