    ...
    match value_to_ast_literal(v.clone()) {
        ...
        None => Err(match &v {
            ...
            Value::Enum(ev) if !ev.fields.is_empty() => format!(
                // rune:lint(one-variant-separator, display) — human-facing diagnostic prose naming
                // the tagged variant, not the registry lookup key; `matcher.rs`'s
                // `value_to_ast_literal` is where a real keyword gets composed, via `compose_variant`.
...
///   substituted: `(:wat::rete::i64::< -5 0)` from `(:wat::rete::i64::< ?c 0)` with
