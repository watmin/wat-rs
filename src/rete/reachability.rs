...
fn drive(src: &str, op: &str) -> Verdict {
    let world = match startup_from_source(src, None, Arc::new(crate::load::loader::InMemoryLoader::new())) {
        Ok(w) => w,
...
fn i64_gt() -> Cell {
    operands_for(":wat::rete::i64::>").expect("the baseline row must be in the table")
}
...
fn special_for(rete_name: &str) -> Option<(&'static str, &'static str, &'static str, &'static str, &'static str)> {
    let t = match rete_name {
        ...
        ":wat::rete::holon::cosine" => (":wat::holon::HolonAST", "#holon [1 2 3]", "#holon [7 8 9]", "(:wat::rete::f64::> (:wat::rete::holon::cosine {f} #holon [1 2 3] :undefined 0.0) 0.9)", ""),
        ":wat::rete::holon::dot" => (":wat::holon::HolonAST", "#holon [1 2 3]", "#holon [7 8 9]", "(:wat::rete::f64::> (:wat::rete::holon::dot {f} #holon [1 2 3] :undefined 0.0) 1000.0)", ""),
        _ => return None,
    };
    Some(t)
}
...
fn the_head_spellings_rete_accepts_today_are_recorded_for_the_edn_migration() {
    let cell = operands_for(":wat::rete::i64::>").expect("the baseline row must be in the table");
    ...
    println!("\n─── head spelling x call site, `:wat::rete::i64::>` ───");
    for l in &lines {
        println!("{l}");
    }
...
fn an_inline_constraint_that_computes_now_computes() {
    let cell = operands_for(":wat::rete::i64::+").expect("row must be in the table");
...
"#;
    ...
    for (name, src, constant, never_c) in [
        ...
        assert_eq!(
            raw_count(src),
            Ok(1),
            // rune:lint(one-variant-separator, not-a-name) — `{name}::=` is the rete comparator suffix (keyword::=/enum::=), not an enum/variant join
...
"#
    ...
    const SUM: &str = "(:wat::rete::core::match {SUBJ} ({vx :x  vy :y} \
                       (:wat::rete::i64::+ vx vy :undefined 0)))";
...
fn probe_eq_for(p: &ParamType) -> Option<&'static str> {
    Some(match p {
        ParamType::I64 | ParamType::Var(_) => ":wat::rete::i64::=",
        ParamType::Bool => ":wat::rete::core::bool::=",
        ParamType::Keyword => ":wat::rete::core::keyword::=",
        ParamType::String => ":wat::rete::string::=",
        ParamType::F64 => ":wat::rete::f64::=",
        _ => return None,
    })
}
...
(:wat::core::defn :probe::run [] -> :wat::core::i64
  (:wat::core::let
    [rules   (:wat::rete::collect-rules :probe)
     session (:wat::core::match (:wat::rete::compile-all rules (:wat::core::PersistentVector (:probe::q))) [:wat::rete::CompileOutcome.Compiled {{:session __session}} __session] [:wat::rete::CompileOutcome.MayNotTerminate {{:rule __rule :fact-type __ft}} (:wat::kernel::assertion-failed! :message "compile: the rule set may not terminate")])
     session (:wat::core::match (:wat::rete::insert session (:probe::In :v 1)) [:wat::rete::InsertOutcome.Inserted {{:session __staged}} __staged] [:wat::rete::InsertOutcome.MemoryCeilingExceeded {{:limit __ilimit :used __iused :staged __icount}} (:wat::kernel::assertion-failed! :message "insert: session memory ceiling exceeded while staging")])
     session (:wat::core::match (:wat::rete::insert session (:probe::In :v 2)) [:wat::rete::InsertOutcome.Inserted {{:session __staged}} __staged] [:wat::rete::InsertOutcome.MemoryCeilingExceeded {{:limit __ilimit :used __iused :staged __icount}} (:wat::kernel::assertion-failed! :message "insert: session memory ceiling exceeded while staging")])
     fired   (:wat::core::match (:wat::rete::fire-rules session) [:wat::rete::FireOutcome.Fired {{:value __fired}} __fired] [:wat::rete::FireOutcome.MemoryCeilingExceeded {{:limit __limit :used __used :rounds __rounds}} (:wat::kernel::assertion-failed! :message "fire-rules: session memory ceiling exceeded")] [:wat::rete::FireOutcome.RoundCapExceeded {{:cap __cap :still-deriving __still}} (:wat::kernel::assertion-failed! :message "fire-rules: fixpoint round cap exceeded")])]
    (:wat::core::length (:wat::rete::query fired (:probe::q)))))
"#
    ))
