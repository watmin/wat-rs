...
fn node_share_where_cost_decomposition() {
    ...
    // `(message actual expected)` form) — the same class as the pre-existing `src` string above
    // (lines 14-15), brought current by hand since no codemod reaches a wat literal inside a
    // `.rs` string. Measured: the retired form raises `#wat.kernel/AssertionFailure` at load
    // ("the positional (message actual expected) form is retired") before this fix.
    let staged = format!(
        "(:nsh::seed (:wat::core::match (:wat::rete::compile (:nsh::build-rules {N})) [:wat::rete::CompileOutcome.Compiled {{:session __session}} __session] [:wat::rete::CompileOutcome.MayNotTerminate {{:rule __rule :fact-type __ft}} (:wat::kernel::assertion-failed! :message \"compile: the rule set may not terminate\")]) {M})"
    );
    ...
    // ⛔ R21 exception (finding 33) — same retired-syntax class as `staged` above, brought
    // current by hand.
    let count_src = format!(
        "(:wat::core::match (:wat::rete::fire-rules {staged}) [:wat::rete::FireOutcome.Fired {{:value __fired}} __fired] [:wat::rete::FireOutcome.MemoryCeilingExceeded {{:limit __limit :used __used :rounds __rounds}} (:wat::kernel::assertion-failed! :message \"fire-rules: session memory ceiling exceeded\")] [:wat::rete::FireOutcome.RoundCapExceeded {{:cap __cap :still-deriving __still}} (:wat::kernel::assertion-failed! :message \"fire-rules: fixpoint round cap exceeded\")])"
    );
...
fn node_share_filter_counts(n: i64, m: i64) -> (u64, u64, u64) {
    let world = startup_from_source(NODE_SHARE_WORLD, None, Arc::new(InMemoryLoader::new()))
        .expect("node-share world should freeze");
    // ⛔ R21 exception (finding 33) — grok's own diff shipped this driver (extracted from the
    // pre-existing `node_share_filter_eval_census` inline body) with RETIRED syntax
    // (`::Variant` positional-tuple match arms, `assertion-failed!`'s retired positional
    // `(message actual expected)` form) — the same class as `node_share_census`'s `src` string
    // above and the `staged`/`count_src` strings below, brought current by hand since no
