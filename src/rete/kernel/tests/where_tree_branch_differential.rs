    ...
    if let Value::wat__core__PersistentMap(m) = pm {
        for (_node, v) in m.iter() {
            if let Value::wat__core__PersistentVector(pv) = v {
                for fact in pv.iter() {
                    // ⛔ R21 exception (finding-33-adjacent — a stale module path, not embedded
                    // wat, but the same "no codemod reaches a Rust reference inside a test file"
                    // class): grok's tree has a `crate::edn_shim` re-export; this tree keeps the
                    // function at its real home, `crate::edn::render`.
                    out.push(crate::edn::render::value_to_edn_string_lossy(fact, None));
                }
            }
        }
    }
...
fn stage_row(world: &crate::freeze::FrozenWorld, axis: &UniformAxis, row: i64) -> Value {
    ...
    // ⛔ R21 exception (finding 33) — grok's diff shipped this driver with RETIRED syntax (a
    ...
    let src = format!(
        "(:{ns}::seed (:wat::core::match (:wat::rete::compile-all (:{ns}::{rules_verb} {row}) (:wat::core::PersistentVector (:{ns}::q-Hit))) [:wat::rete::CompileOutcome.Compiled {{:session __session}} __session] [:wat::rete::CompileOutcome.MayNotTerminate {{:rule __rule :fact-type __fact-type}} (:wat::kernel::assertion-failed! :message \"compile: the rule set may not terminate\")]) (:{ns}::items))"
    );
...
fn where_tree_branch_agrees_with_the_reference_filter() {
    ...
    // `node-share [50 200]` — 50 TestNodes over 200 tokens, the axis the strike was drawn on.
    {
        ...
        // ⛔ R21 exception (finding 33) — same retired-syntax class fixed above and at #369.
        let src = "(:nsh::seed (:wat::core::match (:wat::rete::compile (:nsh::build-rules 50)) [:wat::rete::CompileOutcome.Compiled {:session __session} __session] [:wat::rete::CompileOutcome.MayNotTerminate {:rule __rule :fact-type __ft} (:wat::kernel::assertion-failed! :message \"compile: the rule set may not terminate\")]) 200)";
