    ...
    let (_fired, diffs) = super::with_leaf_occ_diff(|| {
        eval_in(&world,
          "(:wat::core::let \
             ...
             (:wat::core::match (:wat::rete::fire-rules s3) [:wat::rete::FireOutcome.Fired {:value __f} __f] [:wat::rete::FireOutcome.MemoryCeilingExceeded {:limit __a :used __b :rounds __c} (:wat::kernel::assertion-failed! :message \"c\")] [:wat::rete::FireOutcome.RoundCapExceeded {:cap __a :still-deriving __b} (:wat::kernel::assertion-failed! :message \"r\")]))")
    });
