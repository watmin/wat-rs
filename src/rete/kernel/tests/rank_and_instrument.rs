...
/// (`::Variant` positional-tuple match arms, `assertion-failed!`'s retired positional
/// `(message actual expected)` form) — the same class already fixed earlier in this same file
/// (`accum_gather_visits` and `one_rule_fold_ns`'s fire drivers, above), brought current by hand
/// since no codemod reaches a wat literal inside a `.rs` string.
...
// ⛔ R21 exception (finding 33) — grok's own diff shipped this world's seed drivers with RETIRED
// syntax (`::Variant` positional-tuple match arms, `assertion-failed!`'s retired positional
// `(message actual expected)` form) — the same recurring class already fixed elsewhere in this
...
/// (`::Variant` positional-tuple match arms, `assertion-failed!`'s retired positional
/// `(message actual expected)` form) — the same recurring class already fixed elsewhere in this
/// file (`accum_gather_visits` and `one_rule_fold_ns`'s fire drivers, above), brought current by
/// hand since no codemod reaches a wat literal inside a `.rs` string.
fn join_extend_lookups(keys: i64, fanout: i64) -> u64 {
    ...
    let src = format!(
        "(:wat::core::match (:wat::rete::fire-rules (:jx::seed (:wat::core::match (:wat::rete::compile (:wat::rete::collect-rules :jx)) [:wat::rete::CompileOutcome.Compiled {{:session __session}} __session] [:wat::rete::CompileOutcome.MayNotTerminate {{:rule __rule :fact-type __ft}} (:wat::kernel::assertion-failed! :message \"compile: the rule set may not terminate\")]) {keys} {fanout})) [:wat::rete::FireOutcome.Fired {{:value __fired}} __fired] [:wat::rete::FireOutcome.MemoryCeilingExceeded {{:limit __limit :used __used :rounds __rounds}} (:wat::kernel::assertion-failed! :message \"fire-rules: session memory ceiling exceeded\")] [:wat::rete::FireOutcome.RoundCapExceeded {{:cap __cap :still-deriving __still}} (:wat::kernel::assertion-failed! :message \"fire-rules: fixpoint round cap exceeded\")])"
    );
    let ast = crate::parse_one!(src.as_str()).expect("parse the fire driver");
...
fn fanout_prod_entry_fire(keys: i64, fanout: i64) -> (Value, u64, u64) {
    ...
    let src = format!(
        "(:wat::core::match (:wat::rete::fire-rules (:fan::seed (:wat::core::match (:wat::rete::compile (:wat::rete::collect-rules :fan)) [:wat::rete::CompileOutcome.Compiled {{:session __session}} __session] [:wat::rete::CompileOutcome.MayNotTerminate {{:rule __rule :fact-type __ft}} (:wat::kernel::assertion-failed! :message \"compile: the rule set may not terminate\")]) {keys} {fanout})) [:wat::rete::FireOutcome.Fired {{:value __fired}} __fired] [:wat::rete::FireOutcome.MemoryCeilingExceeded {{:limit __limit :used __used :rounds __rounds}} (:wat::kernel::assertion-failed! :message \"fire-rules: session memory ceiling exceeded\")] [:wat::rete::FireOutcome.RoundCapExceeded {{:cap __cap :still-deriving __still}} (:wat::kernel::assertion-failed! :message \"fire-rules: fixpoint round cap exceeded\")])"
    );
    let ast = crate::parse_one!(src.as_str()).expect("parse the fire driver");
...
fn fire_col_field(world: &str, ns: &str, seed_args: &str) -> super::ColFieldCounts {
    ...
    let src = format!(
        // rune:lint(one-variant-separator, namespace) — `{ns}::seed`/`{ns})` compose a RECORD
        // NAMESPACE prefix (callers pass "fan"/"agc"), never an enum type; no TypeDef::Enum
        // variant is named here.
        "(:wat::core::match (:wat::rete::fire-rules (:{ns}::seed (:wat::core::match (:wat::rete::compile (:wat::rete::collect-rules :{ns})) [:wat::rete::CompileOutcome.Compiled {{:session __session}} __session] [:wat::rete::CompileOutcome.MayNotTerminate {{:rule __rule :fact-type __ft}} (:wat::kernel::assertion-failed! :message \"compile: the rule set may not terminate\")]) {seed_args})) [:wat::rete::FireOutcome.Fired {{:value __fired}} __fired] [:wat::rete::FireOutcome.MemoryCeilingExceeded {{:limit __limit :used __used :rounds __rounds}} (:wat::kernel::assertion-failed! :message \"fire-rules: session memory ceiling exceeded\")] [:wat::rete::FireOutcome.RoundCapExceeded {{:cap __cap :still-deriving __still}} (:wat::kernel::assertion-failed! :message \"fire-rules: fixpoint round cap exceeded\")])"
    );
    let ast = crate::parse_one!(src.as_str()).expect("parse the fire driver");
...
/// (`::Variant` positional-tuple match arms, `assertion-failed!`'s retired positional
/// `(message actual expected)` form) — the same recurring class already fixed elsewhere in this
/// file (`fire_col_field`'s driver, above), brought current by hand since no codemod reaches a
/// wat literal inside a `.rs` string.
...
/// (`::Variant` positional-tuple match arms, `assertion-failed!`'s retired positional
/// `(message actual expected)` form) — the same recurring class already fixed elsewhere in this
/// file (`fire_col_field`/`fire_root_join`'s drivers, above), brought current by hand since no
/// codemod reaches a wat literal inside a `.rs` string.
fn fire_gather_keys_world(world: &str, ns: &str, seed_args: &str) -> Vec<super::GatherKeyRow> {
    ...
    let src = format!(
        // rune:lint(one-variant-separator, namespace) — `{ns}::seed`/`{ns})` compose a RECORD
        // NAMESPACE prefix (callers pass "agc"/etc.), never an enum type; no TypeDef::Enum
        // variant is named here.
        "(:wat::core::match (:wat::rete::fire-rules (:{ns}::seed (:wat::core::match (:wat::rete::compile (:wat::rete::collect-rules :{ns})) [:wat::rete::CompileOutcome.Compiled {{:session __session}} __session] [:wat::rete::CompileOutcome.MayNotTerminate {{:rule __rule :fact-type __ft}} (:wat::kernel::assertion-failed! :message \"compile: the rule set may not terminate\")]) {seed_args})) [:wat::rete::FireOutcome.Fired {{:value __fired}} __fired] [:wat::rete::FireOutcome.MemoryCeilingExceeded {{:limit __limit :used __used :rounds __rounds}} (:wat::kernel::assertion-failed! :message \"fire-rules: session memory ceiling exceeded\")] [:wat::rete::FireOutcome.RoundCapExceeded {{:cap __cap :still-deriving __still}} (:wat::kernel::assertion-failed! :message \"fire-rules: fixpoint round cap exceeded\")])"
    );
    let ast = crate::parse_one!(src.as_str()).expect("parse the fire driver");
...
/// ⛔ R21 exception (finding 33), same class as `fire_gather_keys_world` above.
