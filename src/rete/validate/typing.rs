...
/// EXISTS, else `keyword`.
///
/// ⚠ **THIS TYPED BY ENUM PREFIX ALONE AND NEVER CHECKED THE VARIANT EXISTED** (vigilia Class D1,
/// driven 2026-08-31). `decompose_variant` + "is that path a `TypeDef::Enum`" typed `:evt::G::Hii`
...
fn classify_keyword_constant<'a>(k: &'a str, types: &TypeEnv) -> KeywordConstant<'a> {
    match crate::rete::matcher::enum_variant_ctor(types, k) {
        ...
        // rune:lint(one-variant-separator, type-path) — `enum_variant_ctor` (the ONE
        // decompose_variant-based resolver) already declined, so a live dot-form variant would
        // have resolved above; what remains here is specifically the RETIRED `::` spelling
        // (grok-rete's own native separator, pre-dot-flip) as evidence of a typo — decompose_variant
        // is dot-only and structurally cannot see a `::`-only string. `enum_path` is the enum's OWN
        // path, looked up in the type registry; this recognizes the mistake, it does not decompose
        // a currently-valid variant name.
