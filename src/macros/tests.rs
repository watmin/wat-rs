...
fn program_body_producing_non_ast_rejected() {
    ...
    let hit = err.0.iter().find_map(|e| match &e.kind {
        crate::check::CheckErrorKind::ReturnTypeMismatch {
            function,
            expected,
            got,
            ..
        } if function == ":my::m" && expected == ":wat::WatAST" => Some(got.clone()),
        _ => None,
    });
...
/// A macro return that names a user type is a `MalformedDefmacro` at the
/// declaration. `:wat::core::Option` is a core type and is accepted here.
#[test]
fn macro_return_naming_a_user_type_is_malformed() {
    let err = expand_src(
        r#"(:wat::core::defmacro :my::mk [] -> :user::P `(:wat::core::aggregate-new :user::P 3 4))"#,
    )
    .unwrap_err();
    match err {
        MacroError {
    ...
    let err = expand_src(
        r#"(:wat::core::defmacro :my::mk [] -> (:wat::core::Vector :- [:user::P]) `[])"#,
    )
    .unwrap_err();
    match err {
        MacroError {
    ...
    expand_src(
        r#"(:wat::core::defmacro :my::some7 [] -> (:wat::core::Option :- [:wat::core::i64]) `(:wat::core::Option.Some {:value 7}))"#,
    )
    .expect("Option is a core type; the declaration is legal");
}
