...
//!    digest the user supplied in `(:wat::load-file! path (<algo>
...
fn write_canonical_wat(ast: &WatAST, out: &mut Vec<u8>, renumber: &mut ScopeRenumber) {
    match ast {
        ...
        // Arc 300 stone B — rational literal. Already reduced (den >= 2,
        // sign on numerator); encode numer/denom as length-prefixed decimal
        // strings (BigInt has no fixed-width byte form).
        WatAST::RationalLit(r, _) => {
            out.push(TAG_RATIONAL);
            let numer = r.numer().to_string();
            let denom = r.denom().to_string();
            out.extend_from_slice(&(numer.len() as u32).to_le_bytes());
...
/// hex-encoded expected digest. Used by `(:wat::load-file! path
...
/// Used for per-file signatures inside `(:wat::load-file! path (signed
/// <algo> <sig> <pubkey>))`.
///
...
/// `(:wat::signed-load! ...)` (the payload is a loaded file's
/// parsed form list) and `(:wat::eval-signed! ...)` (the payload
