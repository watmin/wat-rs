...
// co-located `.wat` fixture, driven via `call_beside_value`. PersistentVector's per-type
...
fn persistent_vector_core_behavior() {
    ...
    // 2. get by index
    assert_eq!(
        call_beside_value(file!(), ":t::p2-get-by-index").expect("eval"),
    ...
    assert_eq!(
        call_beside_value(file!(), ":t::p3-conj-immutable-original").expect("eval"),
        Value::i64(2),
        "conj must NOT mutate the original (structural sharing)"
    );
    assert_eq!(
        call_beside_value(file!(), ":t::p4-conj-extended").expect("eval"),
        Value::i64(3),
    ...
    assert_eq!(
        call_beside_value(file!(), ":t::p5-generic-get").expect("eval"),
        Value::Option(Arc::new(Some(Value::i64(30)))),
        "generic get on a PersistentVector"
    );
    assert_eq!(
        call_beside_value(file!(), ":t::p6-generic-conj").expect("eval"),
        Value::i64(2),
