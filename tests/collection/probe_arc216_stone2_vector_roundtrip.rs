...
fn probe_10_check_fails_for_non_atomizable_t() {
    ...
    assert_eq!(
        err,
        r##"#wat.check/CheckErrors {:message "2 type-check errors" :location nil :causes [] :errors [#wat.check/TypeMismatch {:message ":wat::holon::to-holon: parameter #1 expects atomizable type (primitive | HolonAST | WatAST | HashSet<T> | Vector<T> | HashMap<K,V> for atomizable T); got :wat::core::Fn(wat::core::i64)->wat::core::i64" :location #wat.core/Span {:file "tests/collection/probe_arc216_stone2_vector_roundtrip_p10_bad.wat" :line 6 :col 28 :end #wat.core.Option/Some #wat.core/Pos {:line 6 :col 29}} :causes [] :callee ":wat::holon::to-holon" :param "#1" :expected "atomizable type (primitive | HolonAST | WatAST | HashSet<T> | Vector<T> | HashMap<K,V> for atomizable T)" :got ":wat::core::Fn(wat::core::i64)->wat::core::i64" :remedies []} #wat.check/ReturnTypeMismatch {:message ":user::compute: body produces :wat::holon::HolonAST; signature declares :()" :location #wat.core/Span {:file "tests/collection/probe_arc216_stone2_vector_roundtrip_p10_bad.wat" :line 4 :col 3 :end #wat.core.Option/Some #wat.core/Pos {:line 6 :col 31}} :causes [] :function ":user::compute" :expected ":()" :got ":wat::holon::HolonAST" :remedies []}]}
---
