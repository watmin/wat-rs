...
fn probe_2_get_hit_and_miss() {
    ...
    let ast = wat::parse_one!("(:t::p2a-get-hit)").expect("parse");
    match eval_in_frozen(&ast, &world, &env).expect("eval").value_owned() {
        Value::i64(n) => assert_eq!(n, 42, "get hit returns Some(42)"),
        other => panic!("expected i64; got {:?}", other),
    }

    let ast = wat::parse_one!("(:t::p2b-get-miss)").expect("parse");
    match eval_in_frozen(&ast, &world, &env).expect("eval").value_owned() {
...
fn probe_3_assoc_insert_overwrite() {
    ...
    let ast = wat::parse_one!("(:t::p3a-assoc-insert)").expect("parse");
    match eval_in_frozen(&ast, &world, &env).expect("eval").value_owned() {
        Value::i64(n) => assert_eq!(n, 2, "assoc inserts new key → length 2"),
        other => panic!("expected i64; got {:?}", other),
    }

    let ast = wat::parse_one!("(:t::p3b-assoc-overwrite)").expect("parse");
    match eval_in_frozen(&ast, &world, &env).expect("eval").value_owned() {
        Value::i64(n) => assert_eq!(n, 999, "assoc overwrites existing key"),
        other => panic!("expected i64; got {:?}", other),
    }

    let ast = wat::parse_one!("(:t::p3c-assoc-immutable)").expect("parse");
    match eval_in_frozen(&ast, &world, &env).expect("eval").value_owned() {
...
fn probe_4_dissoc_removes() {
    ...
    let ast = wat::parse_one!("(:t::p4a-dissoc-remove)").expect("parse");
    match eval_in_frozen(&ast, &world, &env).expect("eval").value_owned() {
        Value::i64(n) => assert_eq!(n, 2, "dissoc removes one key → length 2"),
        other => panic!("expected i64; got {:?}", other),
    }

    let ast = wat::parse_one!("(:t::p4b-dissoc-noop)").expect("parse");
    match eval_in_frozen(&ast, &world, &env).expect("eval").value_owned() {
...
fn probe_5_keys_semantic_correction() {
    ...
    let ast = wat::parse_one!("(:t::p5a-keys-len)").expect("parse");
    match eval_in_frozen(&ast, &world, &env).expect("eval").value_owned() {
        Value::i64(n) => assert_eq!(n, 2, "keys returns Vec of length 2"),
        other => panic!("expected i64; got {:?}", other),
    }

    let ast = wat::parse_one!("(:t::p5b-keys-values)").expect("parse");
    match eval_in_frozen(&ast, &world, &env).expect("eval").value_owned() {
...
fn probe_6_values() {
    let world = startup_beside(file!()).expect("startup");
    let ast = wat::parse_one!("(:t::p6-values-len)").expect("parse");
...
fn probe_7_contains_key() {
    ...
    let ast = wat::parse_one!("(:t::p7a-contains-hit)").expect("parse");
    match eval_in_frozen(&ast, &world, &env).expect("eval").value_owned() {
        Value::bool(b) => assert!(b, "contains-key? hit"),
        other => panic!("expected bool; got {:?}", other),
    }

    let ast = wat::parse_one!("(:t::p7b-contains-miss)").expect("parse");
    match eval_in_frozen(&ast, &world, &env).expect("eval").value_owned() {
...
fn probe_8_length() {
    ...
    let ast = wat::parse_one!("(:t::p8a-length-four)").expect("parse");
    match eval_in_frozen(&ast, &world, &env).expect("eval").value_owned() {
        Value::i64(n) => assert_eq!(n, 4, "length of 4-entry map"),
        other => panic!("expected i64; got {:?}", other),
    }

    let ast = wat::parse_one!("(:t::p8b-length-empty)").expect("parse");
    match eval_in_frozen(&ast, &world, &env).expect("eval").value_owned() {
...
fn probe_9_empty_q() {
    ...
    let ast = wat::parse_one!("(:t::p9a-empty-true)").expect("parse");
    match eval_in_frozen(&ast, &world, &env).expect("eval").value_owned() {
        Value::bool(b) => assert!(b, "empty? true for empty map"),
        other => panic!("expected bool; got {:?}", other),
    }

    let ast = wat::parse_one!("(:t::p9b-empty-false)").expect("parse");
    match eval_in_frozen(&ast, &world, &env).expect("eval").value_owned() {
...
fn probe_10_nested_hashmap() {
    ...
    let ast = wat::parse_one!("(:t::p10a-nested-contains)").expect("parse");
    match eval_in_frozen(&ast, &world, &env).expect("eval").value_owned() {
        Value::bool(b) => assert!(b, "nested HashMap: outer contains :inner key"),
        other => panic!("expected bool; got {:?}", other),
    }

    let ast = wat::parse_one!("(:t::p10b-nested-get)").expect("parse");
    match eval_in_frozen(&ast, &world, &env).expect("eval").value_owned() {
...
fn probe_11_hashset_as_key() {
    ...
    let ast = wat::parse_one!("(:t::p11a-hashset-key-len)").expect("parse");
    match eval_in_frozen(&ast, &world, &env).expect("eval").value_owned() {
        Value::i64(n) => assert_eq!(n, 1, "HashMap<HashSet<i64>, String> length 1"),
        other => panic!("expected i64; got {:?}", other),
    }

    let ast = wat::parse_one!("(:t::p11b-hashset-key-contains)").expect("parse");
    match eval_in_frozen(&ast, &world, &env).expect("eval").value_owned() {
