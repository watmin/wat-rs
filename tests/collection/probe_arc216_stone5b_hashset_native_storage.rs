...
//!
//! Verifies that `Value::wat__core__HashSet` now stores `Arc<HashSet<Value>>`
...
fn probe_1_construction_primitives() {
    ...
    match call_beside_value(file!(), ":t::p1a-i64-set-len").expect("eval") {
        Value::i64(n) => assert_eq!(n, 3, "i64 set length"),
        other => panic!("expected i64; got {:?}", other),
    }

    match call_beside_value(file!(), ":t::p1b-str-set-len").expect("eval") {
        Value::i64(n) => assert_eq!(n, 3, "String set length"),
        other => panic!("expected i64; got {:?}", other),
    }

    match call_beside_value(file!(), ":t::p1c-bool-set-len").expect("eval") {
        Value::i64(n) => assert_eq!(n, 2, "bool set length"),
        other => panic!("expected i64; got {:?}", other),
    }

    match call_beside_value(file!(), ":t::p1d-kw-set-len").expect("eval") {
...
fn probe_2_contains_q_hit_and_miss() {
    ...
    match call_beside_value(file!(), ":t::p2a-contains-i64-hit").expect("eval") {
        Value::bool(b) => assert!(b, "i64 hit"),
        other => panic!("expected bool; got {:?}", other),
    }

    match call_beside_value(file!(), ":t::p2b-contains-i64-miss").expect("eval") {
        Value::bool(b) => assert!(!b, "i64 miss"),
        other => panic!("expected bool; got {:?}", other),
    }

    match call_beside_value(file!(), ":t::p2c-contains-str-hit").expect("eval") {
        Value::bool(b) => assert!(b, "String hit"),
        other => panic!("expected bool; got {:?}", other),
    }

    match call_beside_value(file!(), ":t::p2d-contains-str-miss").expect("eval") {
        Value::bool(b) => assert!(!b, "String miss"),
        other => panic!("expected bool; got {:?}", other),
    }

    match call_beside_value(file!(), ":t::p2e-contains-kw-hit").expect("eval") {
...
#[test]
fn probe_3_length() {
    match call_beside_value(file!(), ":t::p3-length").expect("eval") {
...
fn probe_4_empty_q() {
    ...
    match call_beside_value(file!(), ":t::p4a-nonempty").expect("eval") {
        Value::bool(b) => assert!(!b, "non-empty is false"),
        other => panic!("expected bool; got {:?}", other),
    }

    match call_beside_value(file!(), ":t::p4b-dedup-nonempty").expect("eval") {
...
fn probe_5_conj_and_dedupe() {
    ...
    match call_beside_value(file!(), ":t::p5a-conj-add").expect("eval") {
        Value::bool(b) => assert!(b, "conj adds new element"),
        other => panic!("expected bool; got {:?}", other),
    }

    match call_beside_value(file!(), ":t::p5b-conj-dup").expect("eval") {
        Value::i64(n) => assert_eq!(n, 2, "conj duplicate is idempotent"),
        other => panic!("expected i64; got {:?}", other),
    }

    match call_beside_value(file!(), ":t::p5c-conj-immutable").expect("eval") {
...
fn probe_6_conj_bool_elements() {
    ...
    match call_beside_value(file!(), ":t::p6a-conj-bool-false").expect("eval") {
        Value::bool(b) => assert!(b, "conj false into set-with-true and find it"),
        other => panic!("expected bool; got {:?}", other),
    }

    match call_beside_value(file!(), ":t::p6b-conj-bool-dedup").expect("eval") {
...
fn probe_7_nested_hashset() {
    ...
    match call_beside_value(file!(), ":t::p7a-nested-len").expect("eval") {
        Value::i64(n) => assert_eq!(n, 2, "outer HashSet has 2 inner sets"),
        other => panic!("expected i64; got {:?}", other),
    }

    match call_beside_value(file!(), ":t::p7b-nested-contains").expect("eval") {
        Value::bool(b) => assert!(b, "inner HashSet found by value equality"),
        other => panic!("expected bool; got {:?}", other),
    }

    match call_beside_value(file!(), ":t::p7c-nested-dedup").expect("eval") {
...
fn probe_8_atom_round_trip() {
    ...
    match call_beside_value(file!(), ":t::p8a-rt-len").expect("eval") {
        Value::i64(n) => assert_eq!(n, 3, "round-trip preserves length"),
        other => panic!("expected i64; got {:?}", other),
    }

    match call_beside_value(file!(), ":t::p8b-rt-contains").expect("eval") {
...
fn probe_9_hashset_as_hashmap_value() {
    match call_beside_value(file!(), ":t::p9-hashset-as-hm-val").expect("eval") {
...
fn probe_10_hashset_as_hashmap_key() {
    match call_beside_value(file!(), ":t::p10-hashset-as-hm-key").expect("eval") {
