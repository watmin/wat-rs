...
// co-located `.wat` fixture, driven via `call_beside_value`. PersistentMap's per-type
...
fn persistent_map_core_behavior() {
    // 1. ctor + length
    assert_eq!(
        call_beside_value(file!(), ":t::p1-ctor-length").expect("eval"),
        Value::i64(2),
    ...
    assert_eq!(
        call_beside_value(file!(), ":t::p2-contains-hit").expect("eval"),
        Value::bool(true),
        "contains-key? hit"
    );
    assert_eq!(
        call_beside_value(file!(), ":t::p3-contains-miss").expect("eval"),
        Value::bool(false),
        "contains-key? miss"
    );
    ...
    assert_eq!(
        call_beside_value(file!(), ":t::p4-assoc-immutable-original").expect("eval"),
        Value::i64(1),
        "assoc must NOT mutate the original (structural sharing)"
    );
    assert_eq!(
        call_beside_value(file!(), ":t::p5-assoc-extended").expect("eval"),
        Value::i64(2),
    ...
    // 4. dissoc
    assert_eq!(
        call_beside_value(file!(), ":t::p6-dissoc-removes").expect("eval"),
        Value::bool(false),
    ...
    assert_eq!(
        call_beside_value(file!(), ":t::p7-generic-contains").expect("eval"),
        Value::bool(true),
        "generic contains? on a PersistentMap"
    );
    assert_eq!(
        call_beside_value(file!(), ":t::p8-generic-assoc").expect("eval"),
        Value::i64(2),
