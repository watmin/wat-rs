...
fn from_inner_type_error_is_refused() {
    ...
    // rune:lint(bare-is-err) — THE GAP arm; expected kind is ConstraintTypeMismatch; grok's own assertion is bare, replayed as written.
    assert!(
        result.is_err(),
...
fn inline_twin_is_refused() {
    ...
    // rune:lint(bare-is-err) — CONTROL proving rejection AT ALL (a ConstraintTypeMismatch); grok's own assertion is bare, replayed as written.
    assert!(
        result.is_err(),
...
fn busy_count(with_threshold: bool, acc: &str, gate: &str, min: i64, readings: &[(&str, i64)]) -> Result<i64, String> {
    // Match-arm text, factored out of the `format!` calls below (single braces, no `{{`/`}}`
    // doubling needed) — `no_inlined_edn`'s own placeholder stripper is not `format!`-escape-aware
    // and mis-splits an adjacent `{{…}}` pair into a malformed one-element map literal; splicing
    // these in via a plain `{name}` capture keeps the emitted wat identical while giving the lint's
    // reader-based check clean text to parse.
    let insert_arm = "[:wat::rete::InsertOutcome.Inserted {:session __staged} __staged] [:wat::rete::InsertOutcome.MemoryCeilingExceeded {:limit __ilimit :used __iused :staged __icount} (:wat::kernel::assertion-failed! :message \"insert: session memory ceiling exceeded while staging\")]";
    let compile_arm = "[:wat::rete::CompileOutcome.Compiled {:session __session} __session] [:wat::rete::CompileOutcome.MayNotTerminate {:rule __rule :fact-type __ft} (:wat::kernel::assertion-failed! :message \"compile: the rule set may not terminate\")]";
    let fired_arm = "[:wat::rete::FireOutcome.Fired {:value __fired} __fired] [:wat::rete::FireOutcome.MemoryCeilingExceeded {:limit __limit :used __used :rounds __rounds} (:wat::kernel::assertion-failed! :message \"fire-rules: session memory ceiling exceeded\")] [:wat::rete::FireOutcome.RoundCapExceeded {:cap __cap :still-deriving __still} (:wat::kernel::assertion-failed! :message \"fire-rules: fixpoint round cap exceeded\")]";
    let reading_inserts: String = readings
        .iter()
        .map(|(loc, v)| format!("             session (:wat::core::match (:wat::rete::insert session (:w3::Reading :location \"{loc}\" :value {v})) {insert_arm})\n"))
        .collect();
    let threshold_insert = if with_threshold {
        format!("             session (:wat::core::match (:wat::rete::insert session (:w3::Threshold :min {min})) {insert_arm})\n")
    } else {
        String::new()
    };
    let run = format!(
...
{threshold_insert}\
{reading_inserts}\
             fired   (:wat::core::match (:wat::rete::fire-rules session) {fired_arm})]\n\
