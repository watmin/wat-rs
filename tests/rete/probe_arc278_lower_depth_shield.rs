...
/// Nested `(:wat::rete::i64::+ INNER 1 :undefined -1)` towers, `depth` lists deep.
...
fn nested_plus(depth: usize, inner: &str) -> String {
    ...
    for _ in 0..depth {
        s = format!("(:wat::rete::i64::+ {s} 1 :undefined -1)");
    }
    s
}
...
// The true type: `startup_from_source` alone raises `StartupError`; `eval_go` additionally
// chains a `parse_one!` (`ParseError`, `From<ParseError> for StartupError`) and
// `eval_in_frozen` (`RuntimeError`, `From<RuntimeError> for StartupError`) — a genuine
// multi-kind union, so `StartupError` (not a flattened `String`) is the right return type
// per `no_error_flattening_helper`'s rubric. Test bodies format it with `{e}`
// (`impl Display for StartupError`) at the assertion site when they need substring checks.
...
fn quote_door_refuses_past_the_bound() {
    ...
    match eval_go(&quote_world(depth)) {
        ...
        Err(e) => {
            ...
            assert!(
                msg.contains("EXPANSION_DEPTH_LIMIT") && msg.contains("lowering nesting depth"),
                "the refusal must name the lowering depth bound, got {msg}"
            );
        }
    }
}
...
fn source_past_the_expansion_wall_is_the_expander() {
    ...
    match startup(&source_world(depth)) {
        ...
        Err(e) => {
            ...
            assert!(
                msg.contains("ExpansionDepthExceeded") || msg.contains("macro expansion exceeded depth"),
                "past-expansion source must be the expander's refusal, not lowering's, got {msg}"
            );
            // rune:lint(loose-assert) — targeted ABSENCE: lowering must not have run
            assert!(
                !msg.contains("lowering nesting depth"),
                "the expander must refuse before lowering, got {msg}"
            );
        }
    }
}
