...
fn count_binds_exact_value() {
    ...
    assert!(matches!(busy_count(COUNT, "(:wat::core::= ?n 3)", three), Ok(1)), "count = 3 → fires");
    assert!(matches!(busy_count(COUNT, "(:wat::core::= ?n 5)", three), Ok(0)), "count ≠ 5 → no fire");
}
...
fn accumulate_joins_on_shared_var() {
    ...
    assert!(matches!(busy_count(COUNT, "(:wat::core::= ?n 3)", mixed), Ok(1)), "Bergen not counted → still 3");
}
...
fn sum_folds_the_value() {
    let three = &[("Oslo", 10), ("Oslo", 20), ("Oslo", 30)];
    assert!(matches!(busy_count(SUM, "(:wat::core::= ?n 60)", three), Ok(1)), "sum = 60 → fires");
}
...
fn minimum_finding_set_composition() {
    ...
    assert!(matches!(busy_count(COUNT, "(:wat::core::>= ?n 3)", three), Ok(1)), "3 ≥ 3 → fires");
    assert!(matches!(busy_count(COUNT, "(:wat::core::>= ?n 4)", three), Ok(0)), "3 ≥ 4 → no fire");
}

/// 5 — EMPTY: a Station with zero Readings → count = 0 (count emits on empty; gate `= 0` fires).
#[test]
fn count_emits_on_empty() {
    assert!(matches!(busy_count(COUNT, "(:wat::core::= ?n 0)", &[]), Ok(1)), "no readings → count 0 → fires");
}
