    ...
    let inner_text = match outer {
        OwnedValue::Vector(mut xs) if !xs.is_empty() => match xs.remove(0) {
            OwnedValue::Tagged(_, body) => match *body {
                // grok-rete envelope: Vector of error-EDN strings.
                ...
                // main envelope: map-bodied LociDiedError.RuntimeError; :message is the inner EDN.
                OwnedValue::Map(m) => m
                    .iter()
                    .find(|(k, _)| *k == OwnedValue::Keyword(Keyword::new("message")))
                    .and_then(|(_, v)| match v {
                        OwnedValue::String(s) => Some(s.to_string()),
                        _ => None,
                    })
                    .unwrap_or_else(|| panic!("expected :message string; got {m:?}")),
                other => panic!("expected a vector of error strings or a map body; got {other:?}"),
            },
