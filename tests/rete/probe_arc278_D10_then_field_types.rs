...
//!
//! ## ⛔ MERGE NOTE (replay #344) — three arms below fire a DIFFERENT error kind than grok wrote
//!
//! This tree already carried an independent, earlier-landed `:then`-operand type check
//! (`RhsOperandTypeMismatch`, `src/rete/validate/mod.rs`'s `check_rhs_operands` /
//! `then_operand_declared_type`, from `6df9ba1ab` — an ANCESTOR of this replay's own base, arc-277
//! work unrelated to grok-rete) by the time D10 landed here. It compares DECLARED PATHS, not rete
//! segments, which makes it the finer-grained resolver on the one axis where the two disagree: two
//! DISTINCT enums both segment to `"enum"` under `resolve_operand_type`/`rete_type_segment_of`
//! (this file's own doc above says so), so a `RhsFieldTypeMismatch`-only wall would silently pass
//! an Alpha-into-Beta swap that `RhsOperandTypeMismatch` already refuses and already has a test
//! for (`tests/rete/probe_then_operand_fits_the_field.rs::enum_into_a_different_enum_is_refused`).
//! Conversely, `then_operand_declared_type` has no arm for a COMPUTED operand whose `RETE_OPS` row
//! declares a return type (source 4 below) — measured empirically (pre-#344, on this tree):
//! `(:wat::rete::i64::+ ?k 1 :undefined 0)` written into a `String` field compiled clean, `rc=0`.
//!
//! So `check_then_field_type`/`RhsFieldTypeMismatch` (grok's D10 cure, landed here unmodified in
//! `src/rete/validate/typing.rs`) is wired as a FALLBACK: it runs only where
//! `then_operand_declared_type` found nothing to compare, so it never double-reports an operand
//! the declared-path resolver already classified. Concretely: **arms 1–3 below (bound var,
//! literal, positional) are refused by the PRE-EXISTING `RhsOperandTypeMismatch`**, because
//! `then_operand_declared_type` resolves all three directly; **only arm 4 (computed) reaches
//! `RhsFieldTypeMismatch`**, because a computed operand is exactly the gap
//! `then_operand_declared_type` does not cover. The `.edn` goldens below were regenerated
//! (`UPDATE_EDN=1`) against this tree's real, merged output — never hand-edited — and the
//! doc-comments on arms 1–3 are corrected to name the kind that actually fires.
...
///
/// ⛔ MERGE NOTE (replay #344): refused by the PRE-EXISTING `RhsOperandTypeMismatch`, not
/// `RhsFieldTypeMismatch` — see this file's header. `then_operand_declared_type` resolves a bound
/// `?var` directly (source 2 is exactly its `Symbol` arm), so grok's fallback never runs here.
#[test]
fn a_then_value_bound_to_a_wrongly_typed_var_is_refused() {
    ...
    wat::assert_edn_matches_file!(
        ...
        "`RhsOperandTypeMismatch` must be the ONLY finding; it must name BOTH declared types \
...
///
/// ⛔ MERGE NOTE (replay #344): refused by the PRE-EXISTING `RhsOperandTypeMismatch` — see this
/// file's header. `then_operand_declared_type` resolves a `StringLit` directly (source 3).
#[test]
fn a_then_value_that_is_a_wrongly_typed_literal_is_refused() {
    ...
    wat::assert_edn_matches_file!(
        ...
        "`RhsOperandTypeMismatch` must be the ONLY finding, quoting the literal AS WRITTEN"
    );
}
...
///
/// ⛔ MERGE NOTE (replay #344): refused by the PRE-EXISTING `RhsOperandTypeMismatch` — see this
/// file's header. Main's positional branch calls the same `then_operand_declared_type`-based
/// check as the kwargs branch, so both swapped fields are caught by it, same as arms 1–2.
#[test]
fn a_positional_then_with_wrongly_typed_args_is_refused() {
    ...
    wat::assert_edn_matches_file!(
        ...
        "TWO `RhsOperandTypeMismatch` findings, one per swapped field, each at its own arg's span"
    );
}
...
///
/// ⛔ MERGE NOTE (replay #344): this IS `RhsFieldTypeMismatch`, unlike arms 1–3 — see this file's
/// header. `then_operand_declared_type` has no arm for a computed call form, so it returns `None`
/// and grok's fallback (`check_then_field_type`) is the one that actually fires here. Verified
/// empirically pre-#344 on this tree: this exact operand into this exact field compiled clean
/// (`rc=0`) before the fallback was wired.
#[test]
