...
#[test]
fn count_folds() {
    assert!(matches!(call_beside_value(file!(), ":user::count-folds").expect("eval"), Value::i64(3)), "count = 3 (bare)");
}

/// sum ?bytes → BARE 600 (empty sum = 0; never Option).
#[test]
fn sum_folds() {
    assert!(matches!(call_beside_value(file!(), ":user::sum-folds").expect("eval"), Value::i64(600)), "sum = 600 (bare)");
}

/// min ?bytes → Some(100).
#[test]
fn min_folds() {
    assert!(is_some_i64(&call_beside_value(file!(), ":user::min-folds").expect("eval"), 100), "min = 100");
}

/// max ?bytes → Some(300).
#[test]
fn max_folds() {
    assert!(is_some_i64(&call_beside_value(file!(), ":user::max-folds").expect("eval"), 300), "max = 300");
}

/// mean ?bytes → Some(200) — THE composition: sum(600)/count(3).
#[test]
fn mean_is_sum_over_count() {
    assert!(is_some_i64(&call_beside_value(file!(), ":user::mean-is-sum-over-count").expect("eval"), 200), "mean = 600/3 = 200");
}

/// distinct ?port → BARE vec of length 2 (80, 443 — the duplicate 80 collapses).
#[test]
fn distinct_folds() {
    let v = call_beside_value(file!(), ":user::distinct-folds").expect("eval");
...
#[test]
fn all_folds() {
    let v = call_beside_value(file!(), ":user::all-folds").expect("eval");
...
#[test]
fn group_by_folds() {
    let v = call_beside_value(file!(), ":user::group-by-folds").expect("eval");
...
fn count_empty_is_zero() {
    assert!(matches!(call_beside_value(file!(), ":user::count-empty-is-zero").expect("eval"), Value::i64(0)), "count [] = 0 (bare)");
}

/// EMPTY: min over an empty set → None (no token — there is no minimum of nothing).
#[test]
fn min_empty_is_none() {
    let v = call_beside_value(file!(), ":user::min-empty-is-none").expect("eval");
