...
fn fence_interior_type_error_is_refused() {
    ...
    // rune:lint(bare-is-err) — BANKED arm; the future cure's error KIND is unknown until it lands.
    assert!(
        result.is_err(),
...
fn inline_twin_is_refused() {
    ...
    // rune:lint(bare-is-err) — CONTROL proving rejection AT ALL (a boxed ConstraintTypeMismatch).
    assert!(
        result.is_err(),
