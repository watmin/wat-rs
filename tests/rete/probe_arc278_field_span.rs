...
/// `(:wat::rete::i64::= :nofield 5)`. POST: `:nofield` alone — line 12, col 52, end col 60,
/// which is exactly its 8 characters at its hand-counted column in the fixture.
///
/// ⚠ These columns are THIS TREE's own, not grok's: grok's own fixture spelled the head
/// `:wat::rete::core::i64::=` (the pre-rehome form, six characters longer), so its own hand-count
/// read col 58/66. Ours already carries the numerics-rehome (`:wat::rete::i64::=`), so the SAME
/// keyword lands six columns earlier. Re-derived from the actual golden, not copied from grok's
/// comment.
#[test]
