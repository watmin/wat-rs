...
#[test]
fn comparison_true() {
    assert!(matches!(call_beside(file!(), ":user::comparison-true"), Ok(Value::bool(true))), "5>3 → true");
}

/// 2 — a false comparison over bindings → false.
#[test]
fn comparison_false() {
    assert!(matches!(call_beside(file!(), ":user::comparison-false"), Ok(Value::bool(false))), "3>5 → false");
}

/// 3 — a pure intrinsic predicate (string::starts-with?) over a string binding.
#[test]
fn string_predicate_over_binding() {
    let r = call_beside(file!(), ":user::string-predicate-over-binding");
...
fn computed_operand_true() {
    let r = call_beside(file!(), ":user::computed-operand-true");
...
fn computed_operand_false() {
    let r = call_beside(file!(), ":user::computed-operand-false");
...
fn user_fn_predicate() {
    let r = call_beside(file!(), ":user::user-fn-predicate");
...
fn non_bool_result_is_error() {
    let r = call_beside(file!(), ":user::non-bool-result-is-error");
