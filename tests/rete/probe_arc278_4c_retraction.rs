...
fn fire_keeps_input_facts_distinct_from_derived() {
    ...
    assert_eq!(call_beside(file!(), ":user::part-a-temperature-in-facts").expect("eval"), Value::i64(1),
        "input Temperature stays in Session.facts");
    // derived facts NOT leaked into Session.facts (they live in production-memory):
    assert_eq!(call_beside(file!(), ":user::part-a-coldandwindy-in-facts").expect("eval"), Value::i64(0),
        "derived ColdAndWindy must NOT be in Session.facts (input distinct from derived)");
    // but it IS derived (in production-memory):
    assert_eq!(call_beside(file!(), ":user::part-a-coldandwindy-derived").expect("eval"), Value::i64(1),
...
fn retract_removes_derived_consequence() {
    assert_eq!(call_beside(file!(), ":user::part-b-coldandwindy-derived-after-retract").expect("eval"), Value::i64(0),
...
fn retract_cascades_transitively() {
    ...
    assert_eq!(call_beside(file!(), ":user::part-c-weatheralert-derived-after-retract").expect("eval"), Value::i64(0),
...
fn retract_leaves_independent_derivations() {
    ...
    assert_eq!(call_beside(file!(), ":user::part-d-coldandwindy-derived-after-retract-oslo").expect("eval"), Value::i64(1),
