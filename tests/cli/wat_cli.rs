...
fn check_output_edn_emits_record_per_diagnostic() {
    ...
    // :file field is prepended first; function and callee follow. ⛔ ORDER FLIPPED AT C20
    // (#375, this tree) — see the EDN twin below for why (source order, not hash order); this
    // tree's own `:wat::i64::+` spelling is kept (not grok's `:wat::core::i64::+` — the numerics
    // rehome is already this tree's spelling, unrelated to C20).
    ...
    assert!(lines[1].contains(":callee \":wat::i64::+\"")); // rune:lint(loose-assert) — EDN line includes variable :file field (temp path with pid + nanosecond timestamp); targeted field check is the contract
}
...
fn check_output_json_emits_record_per_diagnostic() {
    ...
    assert!(lines[1].contains("\":callee\":\":wat::i64::+\"")); // rune:lint(loose-assert) — JSON line includes variable \":file\":\"...\" field (temp path with pid + nanosecond timestamp); targeted field check is the contract
}
