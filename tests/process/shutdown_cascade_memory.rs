...
//! # Design
//!
//! The SIGTERM raised during the probe cannot kill the test runner: the
//! substrate handler installed in step 2 catches it, and nextest already gives
//! every test its own process (.config/nextest.toml). The probe:
//!
...
//! 6. The assertions above hold — probe PASSES.
//!
//! If shutdown cascade does NOT work:
//! - The recv-thread never wakes → join blocks → the test hangs → nextest's
//!   slow-timeout SIGTERMs then SIGKILLs it. The test is deliberately
//!   structured to hang (not silently pass) on regression.
//!
