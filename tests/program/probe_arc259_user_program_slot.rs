//! Arc 259 — the `user-data` slot: the env's user-extension half (the floor).
//!
//! The env's `wat.*` fields are platform-owned. User data lives in a nested slot
//! `user-data`, typed `:wat::core::Record` (the root — every record is a subtype, so
...
//! and the seam env carries no `user-data`.
//!
...
/// The record carries `user-data` holding a record (RED via arity at HEAD: a
...
fn env_record_carries_user_program_slot() {
    ...
    assert_eq!(
        got,
        Value::bool(true),
        "Env carries user-data (6th field) holding a record"
    );
}

/// The SEAM defaults `user-data` to `:wat::program::EmptyEnv` — a real record,
...
fn seam_defaults_user_program_to_empty_env() {
    ...
    assert!(
        ...
        "the seam must default user-data to a :wat::program::EmptyEnv record"
    );
}
