...
//! - `:wat::core::char/of`  → encodes to EDN `:wat.core/char/of`
//!   (two `/` in the body) — the EDN parser rejects this as "more than one /".
//! - `:wat::core::HashMap/length`, `:wat::core::HashSet/length` — same issue.
//!
...
//! a form containing `:wat::core::char/of`, rather than silently corrupting the keyword.
//!
//! The sample program below is the one from the brief MINUS the two `/`-in-name
...
/// (`:wat::core::char/of` is covered by `stop_trigger_slash_in_name_keyword` below.)
#[test]
...
///
/// `:wat::core::char/of` cannot round-trip because the `/` in the name segment
/// (`char/of` after the last `::`) causes the EDN writer to emit `:wat.core/char/of`
/// — a keyword body with TWO `/` separators, which the EDN parser rejects
/// ("more than one / in wat.core/char/of").
///
/// Root cause: `keyword_from_wat_path` splits on the LAST `::` to get
/// (ns="wat.core", name="char/of"), then calls `Keyword::try_ns("wat.core", "char/of")`.
/// `try_ns` only validates the FIRST CHARACTER, so it ACCEPTS `char/of` as a name.
/// But the EDN writer emits `:ns/name` → `:wat.core/char/of`, which the EDN parser's
...
/// for `:wat::core::char/of` fails to parse back), rather than silently returning
/// the wrong keyword.
#[test]
fn stop_trigger_slash_in_name_keyword() {
    let kw = ":wat::core::char/of";
    let ast = WatAST::keyword(kw);
    let edn_val = watast_to_edn(&ast);
    let edn_str = wat_edn::write(&edn_val);
