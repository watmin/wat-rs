...
use std::sync::Arc;
use wat::freeze::{startup_bare, BootstrapArgs};
use wat::services::{install_ambient_stdio, take_ambient_stdio, uninstall_thread_io, AmbientStdio};
use wat::io::{PipeReader, PipeWriter, WatReader, WatWriter};

// ─── helpers ──────────────────────────────────────────────────────────────

/// Create a bare FrozenWorld (stdlib only; no user source — the bootstrap
/// machinery needs the stdlib services, not a user program).
fn freeze_ok() -> wat::freeze::FrozenWorld {
    startup_bare().expect("startup_bare should succeed")
}
