...
//!   `cargo nextest run --release -E 'binary(kernel)' -F probe_arc259_brackets_worker`
//!
...
fn map_worker_doubles_in_order_ignoring_worker_id() {
    let got = run_compute_vec("tests/kernel/probe_arc259_brackets_worker_map_doubles.wat");
...
fn map_worker_delivers_worker_id_as_runner_index() {
    let expected_n = std::thread::available_parallelism()
        ...
        .unwrap_or(1)
        .min(50);
    let got = run_compute_vec("tests/kernel/probe_arc259_brackets_worker_map_id.wat");
...
fn each_worker_drains_and_returns_nil() {
    run_compute_nil("tests/kernel/probe_arc259_brackets_worker_each_drains.wat");
}
