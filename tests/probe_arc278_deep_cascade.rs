...
fn gen_expr(depth: usize, width: usize, fire_verb: &str) -> String {
    ...
    let mut prev = 0usize;
    for i in 0..width {
        binds.push_str(&format!("  s{idx} (:wat::rete::insert s{prev} (:casc::Stage0 {i}))\n"));
        prev = idx; idx += 1;
        binds.push_str(&format!("  s{idx} (:wat::rete::insert s{prev} (:casc::Tag0 {i}))\n"));
        prev = idx; idx += 1;
    }
    format!(
...
fn deep_cascade_native_matches_wat_depth10() {
    ...
    let wat = run(depth, width, ":wat::rete::fire-rules");
