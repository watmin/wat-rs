...
fn eval_vec_strings(fn_name: &str) -> Result<Vec<String>, String> {
    match call_beside(file!(), fn_name).map_err(|e| format!("eval: {e:?}"))? {
...
fn contract_01_subs_prefix() {
    assert_eq!(eval_string(":user::c01"), Ok("hello".into()));
}

#[test]
fn contract_02_subs_suffix() {
    assert_eq!(eval_string(":user::c02"), Ok("world".into()));
}

#[test]
fn contract_03_subs_empty_range() {
    assert_eq!(eval_string(":user::c03"), Ok("".into()));
}
