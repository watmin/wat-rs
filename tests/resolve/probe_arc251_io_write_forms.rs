...
// drive it via `call_beside_value`. The claim under test is the file bytes written, inspected
...
fn contract_01_write_file() {
    ...
    let r = call_beside_value(file!(), ":user::compute-c01");
...
fn contract_02_with_open_file() {
    ...
    let _ = std::fs::remove_file(p);
    let r = call_beside_value(file!(), ":user::compute-c02");
...
fn contract_03_with_open_file_returns_body_result() {
    ...
    let _ = std::fs::remove_file(p);
    let r = call_beside_value(file!(), ":user::compute-c03").map_err(|e| format!("{e:?}"));
