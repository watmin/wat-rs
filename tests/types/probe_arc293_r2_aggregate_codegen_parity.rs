//! Arc 293.R2 — the aggregate codegen annihilation: ONE toolkit, holder is the only variance.
//!
//! THE PARITY BREAK: ctor + accessor synthesis is still TWO Rust functions split by holder —
//! `register_struct_methods` (runtime.rs:924, `holder == Struct`) and `register_record_methods`
//! (runtime.rs:1315, `holder != Struct`) — and they drifted. The struct path carries `type_params` and
...
//! `register_aggregate_methods` mints accessors for all three holders, generic-aware, bare key.
...
fn aggregate_codegen_parity_generic_record_accessors() {
    ...
    match got {
        Value::i64(n) => assert_eq!(
            n, 60,
            "10 (core-record) + 20 (holon-record) + 30 (struct) across the three holders; got {n}"
        ),
        other => panic!("expected i64 60 from the three-holder accessor parity; got {other:?}"),
    }
}
