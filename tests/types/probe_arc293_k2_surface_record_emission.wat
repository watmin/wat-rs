...
;; holder is the only variance: `$struct` (Struct), `$core-record` (Record), `$holon-record`
;; (HolonRecord). (`$` is a legal keyword char — confirmed empirically.)
;;
;; Arc 293 K2 originally emitted a single `$record` (holder = surface's `:holder`). K3 changed
;; the emit to the fixed triple; this fixture updated to use `$core-record` (holder = Record,
;; matching the surface's `:holder :wat::core::Record` — the portable EDN data tier).
;;
