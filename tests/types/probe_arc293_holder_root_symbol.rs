...
//! (`:wat::Record`), not the magic shorthand `:record`. A 0-member `:holder` surface is "any
...
//! so `:holder :wat::Record` is a `MalformedDecl` → the world won't start. GREEN once `:holder`
...
fn surface_holder_root_symbol_accepts_record() {
    let world = startup_beside(file!()).expect("startup: a `:holder :wat::Record` surface must parse");
...
/// A struct does NOT satisfy a `:holder :wat::Record` surface — the holder bound is a hard reject.
#[test]
fn surface_holder_root_symbol_rejects_struct() {
    let r = startup_from_file("tests/types/probe_arc293_holder_root_symbol_bad.wat");
    assert!(
        r.is_err(),
        "a struct must not satisfy a `:holder :wat::Record` surface (non-portable, Struct < Record); got Ok"
    );
}
