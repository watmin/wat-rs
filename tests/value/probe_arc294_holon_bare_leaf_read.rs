...
fn row7_bare_bundle_raises_on_encode_never_falls_back() {
    ...
    // that.
    let err = wat::edn::render::value_to_edn_with(&v, None).expect_err(
        "a bare Bundle is neither data nor a recognized directive — encode must REFUSE,          never fall back to a Bundle/nil/best-effort rendering",
    );
    let msg = format!("{err}");
    assert!( // rune:lint(loose-assert) — the message embeds a #wat.core/Span whose :line/:col
             // legitimately drifts whenever edn/render.rs gains or loses lines above the refusal
