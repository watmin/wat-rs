...
//! 9. Deep nesting — `Value::Vec([Value::wat__core__HashMap(...)])` hashes consistently
...
fn probe_7_hashset_value_set_semantics() {
    // Build two Value::wat__core__HashSet with the same elements inserted in different order.
    ...
    let set_a = Value::wat__core__HashSet(Arc::new(set_a_inner));
    ...
    let set_b = Value::wat__core__HashSet(Arc::new(set_b_inner));
...
fn probe_8_hashmap_value_map_semantics() {
    // Build two Value::wat__core__HashMap with the same pairs in different order.
    ...
    let hmap_a = Value::wat__core__HashMap(Arc::new(map_a));
    ...
    let hmap_b = Value::wat__core__HashMap(Arc::new(map_b));
...
fn probe_9_deep_nesting() {
    // Value::Vec([Value::wat__core__HashMap({keyword(":a") → i64(1)})])
    // Stone 216.5c — storage is now HashMap<Value, Value>; K is the native key.
    #[allow(clippy::mutable_key_type)]
    ...
    let hmap_val = Value::wat__core__HashMap(Arc::new(inner_map));
