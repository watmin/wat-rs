...
//!
//! GREEN after A.0: every variant is `#tag [field-vec]`. Superseded by arc 296 H-2:
...
// ── User enum UNIT variant → `#ns/Enum.Variant {}` ────────────────────────────

#[test]
fn user_unit_variant_writes_empty_vector_body() {
    let env = color_env();
    let s = write_value_with(&color_red(), &env);
    wat::assert_edn_matches_file!(s, "probe_arc278_a0_uniform_variant__user_unit.edn", "a user enum unit variant must write `#ns/Enum.Variant {}`");
}
...
fn option_some_writes_single_element_vector_body() {
    ...
    wat::assert_edn_matches_file!(write_value(&v), "probe_arc278_a0_uniform_variant__option_some.edn", "Some(v) must write `#wat.core/Option.Some {:value v}`");
}
...
fn result_ok_writes_single_element_vector_body() {
    ...
    wat::assert_edn_matches_file!(write_value(&v), "probe_arc278_a0_uniform_variant__result_ok.edn", "Ok(v) must write `#wat.core/Result.Ok {:value v}`");
}

#[test]
fn result_err_writes_single_element_vector_body() {
    let v = Value::Result(Arc::new(Err(Value::String(Arc::new("e".into())))));
    wat::assert_edn_matches_file!(write_value(&v), "probe_arc278_a0_uniform_variant__result_err.edn", "Err(e) must write `#wat.core/Result.Err {:error e}`");
}
