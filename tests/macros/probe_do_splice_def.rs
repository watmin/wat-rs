...
fn probe_do_def_two_vars_visible() {
    let world = startup_from_file("tests/macros/probe_do_splice_def_two_defs.wat").expect("freeze");
...
fn probe_do_defn_via_expansion() {
    let world = startup_from_file("tests/macros/probe_do_splice_def_defn_expand.wat").expect("freeze");
    assert!(world.symbols().get(":my::helper").is_some(), ":my::helper not registered");
    assert!(world.symbols().get(":my::main").is_some(), ":my::main not registered");
}
...
fn probe_do_def_via_macro_emission() {
    let world = startup_from_file("tests/macros/probe_do_splice_def_via_macro.wat").expect("freeze");
    assert!(world.symbols().get(":my::helper").is_some(), ":my::helper not registered");
    assert!(world.symbols().get(":my::main").is_some(), ":my::main not registered");
}
