...
/// `C2`'s findings in source order.
///
/// ⛔ REPLAY #375 — THIS TREE HAS 23 FINDINGS, NOT GROK'S 9, AND THAT IS MEASURED, NOT ASSUMED.
/// Main independently carries a stricter check absent from grok's tree at the point this gate was
/// written: `(:wat::service::Outcome.Reply s expr)` / `(:probe::SN::OpResponse.Ok expr)` —
/// POSITIONAL variant construction, which this fixture's boilerplate uses in all 7 services — now
/// raises `MalformedForm` ("positional variant construction is retired"; `src/check.rs:14039`,
/// `src/record/construct.rs:277`). Verified this is unrelated to C20 and pre-dates it on this
/// tree: checked out at `efbfd6b71` (#374, one step before this one, before `check.rs`'s sort
/// even existed) and ran `wat --check` directly — 14 `MalformedForm` + 9 `TypeMismatch` = 23,
/// identical to the count measured after landing the sort. The sort only reorders; grok's own
/// 9-error count for this fixture was never true on this tree. Fixture content is untouched by
/// #375's diff (grok's own file list for this commit does not include this `.wat.bad`), so this
/// is main's own long-standing stricter check meeting a fixture grok wrote against a looser one —
/// not a regression from any replay step, and not something a codemod can reach (the mismatch is
/// between fixture content and a Rust-side check, not a corpus-wide rename).
///
/// The 14 `MalformedForm`s are seven `Outcome.Reply`/`OpResponse.Ok` pairs, one pair per service
/// (`:probe::S1`..`:probe::S7`, lines 17-72), then the seven `:wat::core::match` scrutinee
/// mismatches inside those services' `defn`s (grok's original 7), then the two `kwargs-check`
/// parameter mismatches in `:user::main` (grok's original 2) — SAME-SPAN PAIR, identical file,
/// line, col, end, and variant, so a sort keyed on `(line, col)` alone would order them by INPUT
...
/// `W2A`'s findings in source order.
///
/// ⛔ REPLAY #375 — SAME CLASS AS `C2_SOURCE_ORDER` ABOVE, MEASURED THE SAME WAY. This tree finds
/// 8, not grok's 4: this fixture's own boilerplate carries 4 more `Outcome.Reply`/`Response.Ok`
/// positional-construction sites (lines 18-32), refused by the same pre-existing, C20-independent
/// check. Verified pre-dating C20 the same way (checked out `efbfd6b71`, `wat --check` directly:
/// 4 `MalformedForm` + 1 `ReturnTypeMismatch` + 3 `TypeMismatch` = 8). Grok's original 4 (the
