...
//! (mem-store' + journal' + producer' + consumer') run on PROCESS — the guarantee that the
//! consumer can't hold `:prod::*` is a process property (separate registries per fork).
//!
//! Expect exactly 60 survivors (the Alert-high quarter of 240).
//!
...
#[test]
fn sift_arena_foreign_reader_counts_exact_survivors_across_a_process_fork() {
    ...
    assert!(
        ...
        "expected the consumer, which CANNOT hold :prod::*, to count exactly 60 Alert-high \
         survivors (the quarter of 240) via the foreign reader, paging across many journal \
         pages; got {got:?}"
    );
}
