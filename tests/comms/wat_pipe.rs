...
fn pipe_returns_writer_reader_tuple() {
    ...
    let v = call_beside_value(file!(), ":my::pipe-returns-writer-reader-tuple").expect("eval");
...
fn pipe_writeln_then_read_line_round_trips() {
    let v = call_beside_value(file!(), ":my::pipe-writeln-round-trips").expect("eval");
    assert_eq!(unwrap_some_string(v), "hello");
}

#[test]
fn pipe_multiple_writelns_read_line_by_line() {
    let v = call_beside_value(file!(), ":my::pipe-multiple-writelns").expect("eval");
...
fn pipe_write_string_then_read_exact_bytes() {
    ...
    let v = call_beside_value(file!(), ":my::pipe-write-string-exact-bytes").expect("eval");
...
fn pipe_preserves_utf8_lines() {
    let v = call_beside_value(file!(), ":my::pipe-preserves-utf8").expect("eval");
