//! Arc 170 — DIAGNOSTIC clone of probe_pdeathsig_kills_orphan_child.
//!
//! Same shape as the original probe BUT the supervisor's pre-`_exit` sleep
//! is configurable via env var `WAT_PROBE_SUPERVISOR_DELAY_MS` (default 0).
//!
//! Hypothesis under test: supervisor sometimes `_exit(0)`s BEFORE the
//! grandchild reaches `prctl(PR_SET_PDEATHSIG)`, registering no parent-death
//! signal → kernel reparents to init without sending SIGTERM → grandchild
//! lives forever as orphan.
//!
//! If hypothesis true: WAT_PROBE_SUPERVISOR_DELAY_MS=0 leaks orphans
//! intermittently; WAT_PROBE_SUPERVISOR_DELAY_MS=10 should eliminate them
//! (10ms >> 50-200µs fork-to-prctl window).
//!
//! Run with:
