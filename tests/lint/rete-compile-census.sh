...
run_one() {
  ...
  { echo; echo '(:wat::core::defn :user::main [] -> :wat::core::nil (:wat::core::do'
    for ns in $nss; do
      ...
      echo "    [:wat::rete::CompileOutcome.Compiled {:session __s} (:wat::kernel::println \"CENSUS-OK\")]"
      echo "    [:wat::rete::CompileOutcome.MayNotTerminate {:rule __r :fact-type __f} (:wat::kernel::println \"CENSUS-MNT\")])"
    done
    echo '))'; } >> "$WORK/$base"
