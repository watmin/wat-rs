...
fn each_resolver_half_answers_a_name_no_other_half_can() {
    /// `SiftRulesResponse` (`src/rete/purity.rs`'s own convention citation against
    /// `wat/query.wat`) no longer discriminates on this tree: `src/check.rs:23614` puts the
    /// identifier in a Rust CODE position (a string literal argument to a field-lookup helper,
    /// not a comment), so `u.rust` now contains it too. Its paired name from the SAME
    /// `wat/query.wat` convention — `req-kw`'s `"::SiftRulesRequest"` (`wat/query.wat:152`) — has
    /// no such Rust-side use and stays wat-only.
    const WAT_ONLY: &str = "SiftRulesRequest";
