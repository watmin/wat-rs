...
///
/// ⚠ REPLAY #278, LANDING-TIME FINDING (this tree only, unrelated to grok's own diff): two names
/// this attestation would otherwise admit are a DIFFERENT class of false signal from the registry
/// file's prose — `src/intrinsic/special/rete_alias.rs`'s `#[wat_special_form(":wat::rete::core::
/// map")]` / `"...::filter"` are ORPHANED special-form registrations, dead relative to
/// `src/rete/vocabulary.rs`'s RETE_OPS table (whose own comment there, dated 2026-08-28, records
/// that `map`/`filter` were REPLACED by `mapv`/`filterv` because the lazy heads are unreachable
/// from a compiled `:where` fence). Nobody removed the Rust-side registration when the RETE_OPS
/// row was deleted — a THIRD failure mode `prose_in_rust_does_not_attest_a_name`'s own two
/// hypotheses (stripper broken / genuinely re-minted) did not anticipate, driven out by checking
/// `src/rete/vocabulary.rs` directly as that test's own message instructs: no RETE_OPS row exists
/// for either name, so the codemod's phantom-name diagnosis for the `wat-scripts/` corpus stands
/// unchanged. Excluded HERE, not from the main gate's verdict: registry-namespace names (both of
/// these are `:wat::rete::core::…`) are judged solely by [`registry_rows`]/[`KNOWN_FORMS`] in
/// [`every_rete_name_in_wat_scripts_code_resolves`] and never consult this function at all, so the
/// exclusion changes zero verdicts there — it only stops this ATTESTATION set (whose sole
/// consumers are the non-registry family and this file's own negative control) from reporting a
/// stale special form as if it were a live, intentional re-minting. Retiring the orphaned struct
/// itself is a separate, un-replayed defect (filed, not fixed, here — out of this step's diff).
const KNOWN_ORPHANED_SPECIAL_FORMS: &[&str] =
    &[":wat::rete::core::map", ":wat::rete::core::filter"];
...
fn attested() -> BTreeSet<String> {
    ...
    for orphan in KNOWN_ORPHANED_SPECIAL_FORMS {
        out.remove(*orphan);
    }
    out
}
