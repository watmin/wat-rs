...
//!   key. Value MUST be a `LitStr`; a non-string is a parse error (the grammar
//!   closes the hole).
//! - **`#[to_edn(via = path::to::fn)]`** — Instead of calling `field.to_edn()`,
//!   emit `(key, path::to::fn(field))`. The fn signature must be
//!   `fn(&FieldType) -> ::wat_edn::OwnedValue`. The value MUST be a bare path
...
//!   constant string fields `(:k1 "v1") (:k2 "v2") …` in declared order,
//!   before the field-derived pairs. Works on unit variants (no fields) AND
//!   struct variants. Keys are idents (snake→kebab), values are `LitStr` only.
//! - **`#[to_edn(via(key = "k", fn = path::to::fn, args(a, b, c)))]`** —
//!   Append a computed field after the field pairs. Evaluates
//!   `path::to::fn(a, b, c)` where `a, b, c` are bound field idents of THIS
...
//! safe token class (`LitStr` or bare path). An inline expression
//! (`.method()`, field access, arithmetic) is a PARSE ERROR — the smuggle hole
//! is closed by the parser, not by documentation.
//!
//! Unknown `#[to_edn(…)]` keys emit `compile_error!` naming the allowed set.
//!
//! ## Namespace
//!
...
//! `crate::to_edn::ToEdn`, rustc emits:
//!
//! ```text
                        ...
                        quote! {
                                ...
                                #via_fn(#(#via_args),*)
                            {
                                __fields.push((
                                    ...
                                    crate::to_edn::ToEdn::to_edn(&__via_val),
                                ));
                            }
                        }
                    });
