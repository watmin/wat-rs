...
(:wat::rete::defrule :ul::unwrap
  :when [(:wat::grep::Node  (?id <- :id) (?p <- :parent) (?i <- :index) (?k <- :kind) (:wat::rete::core::string::= ?k "keyword") (:wat::rete::core::i64::= ?i 0))
         (:wat::grep::Named (?id <- :id) (?n <- :name) (:wat::rete::core::string::= ?n ":wat::core::Option/expect"))]
...
(:wat::rete::defrule :ul::arg
  :when [(:ul::Unwrap (?outer <- :parent))
         (:wat::grep::Node (?arg <- :id) (?outer <- :parent) (?ai <- :index) (?ak <- :kind) (:wat::rete::core::i64::= ?ai 1) (:wat::rete::core::string::= ?ak "list"))]
...
(:wat::rete::defrule :ul::match
  :when [(:ul::ArgIsList (?outer <- :outer) (?arg <- :arg))
         (:wat::grep::Node  (?h <- :id) (?arg <- :parent) (?hi <- :index) (:wat::rete::core::i64::= ?hi 0))
         (:wat::grep::Named (?h <- :id) (?hn <- :name) (:wat::rete::core::string::= ?hn ":wat::core::HashMap/get"))
