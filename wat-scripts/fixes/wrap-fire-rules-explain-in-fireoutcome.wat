;; wat-scripts/fixes/wrap-fire-rules-explain-in-fireoutcome.wat — arc 278 the FIRE-OUTCOME wall, S2b
;; (`fire-rules-explain` — the THIRD head, kept as its own recorded migration because its payload
;; differs: it answers `(FireOutcome :- [Explained])`, not `(FireOutcome :- [Session])`.)
;;
;; Self-hosted, comment-faithful fix-wat codemod — NO hand-editing of .wat files, use the tool.
;; A VERBATIM copy of wrap-fire-once-in-fireoutcome.wat with two head keywords and the arm strings
...
;;
;; THE CHANGE: `:wat::rete::fire-rules` (and its `$oracle` twin, same TYPE by the dual-impl
;; contract) now returns `(:wat::rete::FireOutcome :- [Session])` instead of a bare Session — a
...
;;
;;   (fire-rules ARG)
;;     ->  (:wat::core::match (fire-rules ARG)
;;           ((FireOutcome::Fired __explained) __explained)
...
;;
;; MATCHER (structural): a list whose head keyword is `:wat::rete::fire-rules` or
;; `:wat::rete::fire-rules$oracle`. ⚠ NOT `fire-rules-explain` — that returns
;; `(FireOutcome :- [Explained])`, a DIFFERENT payload, and its call sites bind the Explained.
;; An exact head match keeps them apart; a prefix match would silently mangle them.
...
;;   printf '["pathA" …]\n' | ./target/release/wat ./wat-scripts/fixes/wrap-fire-rules-explain-in-fireoutcome.wat
...
;; fire-rules-call? — a list whose head keyword is `:wat::rete::fire-rules` or its `$oracle`
...
(:wat::core::defn :user::fire-rules-call? [node <- :wat::WatAST] -> :wat::core::bool
  (:wat::core::let [h (:user::head-kw-name node)]
    (:wat::core::if (:wat::core::= h ":wat::rete::fire-rules-explain")
      true
      (:wat::core::= h ":wat::rete::fire-rules-explain$oracle"))))
    ...
    (:wat::core::let [ch (:wat::core::ast->children node)]
      (:wat::core::if (:wat::core::< (:wat::core::length ch) 3)
        ...
        (:wat::core::let
          ...
          (:wat::core::if (:user::fire-rules-call? scrut)
  ...
  (:wat::core::if (:wat::core::empty? paths)
    ...
    (:wat::core::let [path (:wat::core::first paths)]
      (:wat::core::do
        ...
        (:wat::kernel::println (:wat::core::string::concat "[wrap-fireoutcome-explain] " path))
