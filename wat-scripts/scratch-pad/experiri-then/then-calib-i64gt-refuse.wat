;; experiri (vigilia cast, 2026-09-08) — `:then` value-operand position, CALIBRATION cell 2/4.
;; Refuse-drive for :wat::rete::core::i64::> — single operand negated (both facts get the
;; DECLARATION-CONSTRAINED discriminating value 3, which fails > 10). Expect: 0.
...
(:wat::rete::defrule :probe::rule
  ...
  [(:probe::In (?k <- :k) (?v <- :v))]
  :then
  [(:probe::Out :k ?k :ok (:wat::rete::i64::> ?v 10))])
  ...
  (:wat::kernel::println
    (:wat::core::let
      [rules   (:wat::rete::collect-rules :probe)
       ...
       session (:wat::core::match (:wat::rete::insert session (:probe::In :k "hit"  :v 3)) [:wat::rete::InsertOutcome.Inserted {:session __staged} __staged] [:wat::rete::InsertOutcome.MemoryCeilingExceeded {:limit __ilimit :used __iused :staged __icount} (:wat::kernel::assertion-failed! :message "insert: session memory ceiling exceeded while staging")])
