...
;; LAW A CONTROL: does `:then` also refuse a CORE-spelled head (`:wat::core::>`), the way
;; `:when`'s inline-constraint and where-fence positions do (reachability.rs's `law_a_core_head`)?
;; This is NOT one of the calibration's pinned cells (its answer was not known in advance) — it is
;; itself a finding about whether Law A's boundary is a `:when`-only rule or a whole-rule rule.
...
(:wat::rete::defrule :probe::rule
  ...
  [(:probe::In (?k <- :k) (?v <- :v))]
  :then
  [(:probe::Out :k ?k :ok (:wat::core::> ?v 10))])
  ...
  (:wat::kernel::println
    (:wat::core::let
      [rules   (:wat::rete::collect-rules :probe)
       ...
       session (:wat::core::match (:wat::rete::insert session (:probe::In :k "hit"  :v 42)) [:wat::rete::InsertOutcome.Inserted {:session __staged} __staged] [:wat::rete::InsertOutcome.MemoryCeilingExceeded {:limit __ilimit :used __iused :staged __icount} (:wat::kernel::assertion-failed! :message "insert: session memory ceiling exceeded while staging")])
       session (:wat::core::match (:wat::rete::insert session (:probe::In :k "miss" :v 3)) [:wat::rete::InsertOutcome.Inserted {:session __staged} __staged] [:wat::rete::InsertOutcome.MemoryCeilingExceeded {:limit __ilimit :used __iused :staged __icount} (:wat::kernel::assertion-failed! :message "insert: session memory ceiling exceeded while staging")])
