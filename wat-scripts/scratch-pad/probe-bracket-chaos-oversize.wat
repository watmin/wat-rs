;; MEASUREMENT: a work-fn that returns an O above DEFAULT-MAX-MESSAGE-BYTES.
;; Process tier — thread peers do not encode a frame.
;; 1 runner, 1 item: no survivor to re-dispatch to.
;; Report whichever collect-loop arm actually raises.
...
(:wat::core::defn :user::work
  [_x <- :wat::core::i64]
  -> :wat::core::String
  ;; 2^20 = 1 MiB of payload, well above 524288.
  (:user::double-n "x" 20))
