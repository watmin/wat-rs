...
;;            is the `<service>::Op` superset, surface + internal `-`-ops).
...
;;
;; GREEN (both tiers, exit 0) = the composition holds on the REAL `poll'`/`Peer'` path: the mixed
;;   vec type-checks AND `poll'` delivers BOTH the client's `:Ping` and the timer's `:Tick` as a
;;   `ServiceEvent::Message` (the server replies `:Pong` only AFTER it has seen BOTH, so a returned
;;   `:Pong` PROVES both were delivered from the one selectables vec). RED = the composition is
      ...
      (:wat::core::match op 
        ...
        ((:probe-homog::Op::Ping)
          (:wat::core::if saw-tick
            (:wat::core::let
              [_ (:wat::core::match (:wat::kernel::send (:wat::core::nth selectables idx) (:probe-homog::Reply::Pong)) (:wat::kernel::SendOutcome::Sent nil) (:wat::kernel::SendOutcome::Closed nil) (:wat::kernel::SendOutcome::Stopped nil) ((:wat::kernel::SendOutcome::Lost _c) nil))]
              nil)
  ...
  (:wat::core::let
    [pair (:wat::kernel::listener (:wat::spawn::thread) :probe-homog::Op :probe-homog::Reply)
     ...
     r    (:wat::core::match (:wat::kernel::recv c)
            ...
            (:wat::kernel::RecvOutcome::Closed (:wat::kernel::assertion-failed! "recv': c closed" :wat::core::None :wat::core::None)) (:wat::kernel::RecvOutcome::TimedOut (:wat::kernel::assertion-failed! "recv: timed out — the peer is alive and silent" :wat::core::None :wat::core::None)) ((:wat::kernel::RecvOutcome::Malformed _cause) (:wat::kernel::assertion-failed! "recv: malformed frame — the peer could not decode our message; this arm is an UNMIGRATED PLACEHOLDER (a-momentary-failure-is-not-fatal, stone 2 replaces it with report-final)" :wat::core::None :wat::core::None)))]
    r))
