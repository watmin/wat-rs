;; MEASUREMENT: a work-fn that waits on `after` then returns.
;; If collect-loop sees Message, the map succeeds and the result is x+1.
;; That is the arm. A stall that never sends is a different probe.
