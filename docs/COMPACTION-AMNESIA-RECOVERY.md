...
The directory `/home/john/work/holon/` contains MULTIPLE PROJECTS. The
holon root happens to be a git repo BUT IT IS FROZEN. **Never touch the
holon root git repo.** Treat `/home/john/work/holon/` as a directory
that contains sub-projects.

The active sub-projects (each its own git repo):

```
/home/john/work/holon/
...
1. **Never `git add` / `git commit` / `git push` from `/home/john/work/holon/`** — that's the frozen root repo.
...
`COMPACTION-AMNESIA-RECOVERY.md` at `/home/john/work/holon/` (the
...
(`/home/john/work/holon/`) is FROZEN — never commit there. Use
`git -C <subproject> ...` if you need to operate cross-repo without
changing cwd.

**Real incident, 2026-05-02:** The orchestrator created
`COMPACTION-AMNESIA-RECOVERY.md` at `/home/john/work/holon/` and
...
- Anchor the cwd EXPLICITLY in the agent prompt. Name the absolute
  path the agent must operate in (e.g., `/home/john/work/holon/<project>/`).
...
1. Before acting on any DESIGN section's directive (especially
   ...
   ```bash
   ls ~/.claude/projects/-home-john-work-holon/memory/project_*.md
   ```
...
"Cargo is at /home/john/...", "If you hesitate, run `which cargo`")
...
- `/home/john/work/holon/CLAUDE.md` — workspace setup (auto-loaded)
- `/home/john/work/holon/wat-rs/CLAUDE.md` — wat-rs guidance (if present)
...
- `~/.claude/projects/-home-john-work-holon/memory/MEMORY.md`
