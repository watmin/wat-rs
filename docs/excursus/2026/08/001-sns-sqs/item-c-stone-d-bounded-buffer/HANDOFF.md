...
1. `DESIGN.md` — what is already bounded (measured), why backpressure is not
   the missing piece, and the three-part contract decision.
2. `BRIEF.md` — the rooms as exact `file:line`, four STOP triggers.
3. `docs/excursus/2026/08/001-sns-sqs/item-b-batched-writer/SCORE.md` — the stone before this, and the one with no delta; its note on
