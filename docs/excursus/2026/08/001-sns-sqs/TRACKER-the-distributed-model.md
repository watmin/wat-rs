...
Full record: **`docs/arc/2026/05/170-program-entry-points/FINDING-the-writes-kept-the-1970s.md`** and **R70** in `REALIZATIONS.md`.
...
That is `docs/excursus/2026/08/001-sns-sqs/the-unknowable-state/DESIGN.md`'s prediction verbatim: *"Nobody emits an outcome for that
...
| S16 | **The SCORE diverges from today's runtime.** `docs/excursus/2026/08/001-sns-sqs/the-sane-circuit/SCORE.md:43` recorded `process ns=0 -> TIMED-OUT (500 ms guard)` on 2026-09-01. On 2026-09-03 the recv returns `Closed` promptly. Both cannot describe one behaviour. **The 09-01 line is left standing, not overwritten**, until someone establishes whether the runtime changed or the guard shape saw a different thing. | open |
