...
1. **`DESIGN.md`** — the one contract decision (block, do not drop and do
   not buffer), and the known wart (the retry is a poll, with its replacement trigger).
2. **`docs/excursus/2026/08/001-sns-sqs/FINDING-batching-needs-a-linear-store.md`** — the 2×2 this is responding to. **Do not re-derive
...
3. **If it deadlocks — STOP, capture, name the sizes, do not re-run.** Every stage blocking on the
   next is exactly the shape that can deadlock, and `docs/excursus/2026/08/001-sns-sqs/FINDING-the-drain-variance.md` is what a
