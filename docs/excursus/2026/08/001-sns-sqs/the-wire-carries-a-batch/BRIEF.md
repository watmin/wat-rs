...
1. **`DESIGN.md`** — the decomposed 3–5× estimate, the `cap`/`K`
   sweep, and the one contract decision: **no linger timer**. Read it first; the exclusion is the
   design, not a preference.
2. **`docs/excursus/2026/08/001-sns-sqs/FINDING-the-buffer-was-the-bottleneck.md`** — why a linger timer is forbidden. We spent a day
...
2. **If you find yourself adding a timer to fill a batch — STOP.** The DESIGN forbids it and
   `docs/excursus/2026/08/001-sns-sqs/FINDING-the-buffer-was-the-bottleneck.md` is why.
