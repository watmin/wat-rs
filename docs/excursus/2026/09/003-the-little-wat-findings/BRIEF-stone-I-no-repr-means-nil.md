...
⛔ **`Value::Vector` is NOT in this stone** (see the DESIGN's amendment) — leave its arm untouched.


In `src/edn/render.rs`, route the TWO arms through `opaque_nil_or_refuse`:
`wat__kernel__HandlePool` (`~:4788`) and `wat__stream__Stream` (`~:4882`,
...
Extend the stone H probe: over a process wire, each of the three cases (HandlePool, forced Stream,
**empty Stream**) now REFUSES at the sender with stone G's message; a stream materialized to a
