...
| ✓ LRU CacheService | B | `ReplyTx<V>` exists; no `ReplyRx<V>` (Level 2 mumble — unallocated rx has no domain name) | shipped K.lru 2026-05-01: added `ReplyRx<V>` + `ReplyChannel<V>` typealiases; renamed `ReqPair<K,V>` → `ReqChannel<K,V>` (in-crate suffix mumble per gaze 2026-05-01); LRU is now the Pattern B canonical reference. **✓ Arc 119 (closed 2026-05-06)** reshaped protocol body — Body<K,V> retired; Request<K,V> became enum-based with batch fields; ReplyTx<V> body widened from `Sender<Option<V>>` to `Sender<Vec<Option<V>>>`; new `PutAckTx/Rx/Channel` + `Entry<K,V>` typealiases minted. K.lru-shipped names stayed. **Arc 130 (closed 2026-05-06)** then superseded the arc 119 typealias family with pair-by-index discipline — `Reply<V>` enum unifying GetResult + PutAck on a single shared reply channel per slot; `Handle<K,V>` + `DriverPair<K,V>`; PutAck* family retired. K.lru-shipped names still stand. |
| ✓ HolonLRU | B+A (variant-scoped) | divergent from LRU: enum-Request with NO Put reply path (fire-and-forget — violates ZERO-MUTEX § Mini-TCP) | **✓ Arc 119 (closed 2026-05-06)** reshaped protocol to symmetric batch (Get→Vec<Option<HolonAST>>, Put→unit-ack) covering both services; minted `PutAckTx/Rx/Channel` + `Entry`; preserved variant-scoped naming (`Get*Reply*` Pattern B + `Put*Ack*` Pattern A). **Arc 130 (closed 2026-05-06)** then superseded the arc 119 typealias families with pair-by-index discipline — `Reply` enum unifying GetResult + PutAck on a single shared reply channel per slot; `Handle` + `DriverPair`; the entire `GetReply*` + `PutAck*` families retired. **K.holon-lru's planned `GetReplyPair → GetReplyChannel` rename is moot** post-arc-130 (those typealiases no longer exist); K.holon-lru's remaining work is the grouping-noun flatten only (`HologramCacheService::*` → `:wat::holon::lru::*`). |
...
| `:wat::core::atom-value` | `:wat::core::Atom/value` | Same rename class as Section D' (Type/verb shape for type-attached methods). Surfaced 2026-05-16 during arc 201 slice 2 — the slice originally proposed minting `:wat::holon::Atom/value`; STOP trigger 3 fired because `:wat::core::atom-value` (arc 057, runtime.rs:11772-11812) already serves the use case polymorphically. The verb is correctly placed (atomic-leaf unwrap is core-domain, not holon-specific) but the shape is legacy verb-spelling; should be `Atom/value` per Type/verb canonical. Note: the verb dispatches across `Atom` + `Symbol` + `String` + `I64` + `F64` + `Bool` leaf shapes — the `Atom/` prefix is the conceptual home (Atom is the wrapping form), but the verb fires for any atomic HolonAST. | pending |
