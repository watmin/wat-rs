...
| **C** | `spawn_process_child_branch` shrinks to shim. After fork: dup2 fds → `bootstrap_wat_vm_process` → `apply_function(closure_entry)` → cleanup. Fixes the original "fatal flaw" — spawn-process children now have full ambient stdio. | spawn_process.rs significantly smaller + correct. | M | E, F | A |
