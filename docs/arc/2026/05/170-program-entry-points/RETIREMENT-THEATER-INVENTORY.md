...
10. **Gap I-A** — `is_declaration_form` predicate mint + lift unification. Gap H closed 3 of 8 declaration forms (define/struct/enum); 5 remain uncovered (def/defmacro/define-dispatch/newtype/typealias). Architecture: mint `is_declaration_form` in src/freeze.rs adjacent to `is_mutation_form` covering the 8 declaration forms only (loads + config setters explicitly out-of-scope per four-questions verdict 2026-05-13 — honest scope-bounding, not deferral). Route `closure_extract.rs::is_prelude_form` through `is_declaration_form` OR retire `is_prelude_form` entirely. Name settled by /gaze ward 2026-05-13: candidates `is_top_level_form` (LIES — loads/config also top-level), `is_startup_form` / `is_binding_form` / `is_definer_form` / `is_def_form` (MUMBLE). `is_declaration_form` speaks: names what the forms ARE, clean prior claim from type/module systems, nests cleanly under `is_mutation_form` as readable subset. Strictly additive — closes drift Gap H left.
11. **Gap I-B** — `validate_def_position_with_wrapper` extension via `is_declaration_form`. Currently the check-time position validator catches ONLY `:wat::core::def` violations; the other 7 declaration forms rely on runtime (`DefineInExpressionPosition`) or freeze-time (`EvalForbidsMutationForm`) rejection. Gap I-B routes the validator's match arms through `is_declaration_form` so all 8 forms catch at check-time. Stricter discipline — surfaces previously runtime-only diagnostics earlier. Risk: may cascade through tests that rely on specific error variants; honest delta surfaces post-sweep. Stepping-stone after Gap I-A: predicate already proven; I-B is "one function gets the additional arms via shared predicate."
