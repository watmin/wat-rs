...
## Currently (2026-05-27 late — **INTRINSIC BOUNDARY** pivot holds; **arc 237.7b COMPLETE — all four collection ops shipped as ∀T Rust intrinsics**. NEXT decision-point = 237.7c OR pause. AUTHORITY: memory `project_intrinsic_boundary` + `feedback_green_gate_lib_and_build` + `feedback_sonnet_bash_firewall` + `DESIGN-STONE-237.7b.md` — read those + `git log`. HEAD `fad1c1c6`+. The "Headline state" fenced block BELOW is pre-pivot/STALE; this section is truth.)
...
**237.7b — Tier-A / Tier-B doctrine COMPLETE** (settled via FM-2-bis probe `tests/probe_arc237_7b_intrinsic_typing.rs`, 7/7; all four ops shipped):
- **TIER A** (concrete return, no element-typing) — **plain ∀ TypeScheme** (length-mirror): ✓ `length` 237.7a `8100d9d2` · ✓ `empty?` 237.7b-i `e401c183`.
- **TIER B** (element-typing enforced via **custom inference arm**, mirror `infer_positional_accessor`): ✓ `contains?` 237.7b-ii `fef2c8d9` (3 coll types — Vector<T>/HashSet<T> unify arg1 with T, **HashMap<K,V> with K NOT V**; bool return) · ✓ `conj` 237.7b-iii `2d3259ae` (Vector<T>+HashSet<T>, NO HashMap, type-preserving return) · ✓ `get` 237.7b-iv `fad1c1c6` (Vector<T>+i64→Option<T>, HashMap<K,V>+K→Option<V>; **the Vector arm unifies arg1 with `i64`, NOT X — the INDEX trap** honored as `let idx_ty = TypeExpr::Path(":wat::core::i64".into())`; ONE behavioral lib-test update — `hashmap_get_requires_hashmap_arg` now expects `TypeMismatch` not `MalformedForm`, correct mechanism-swap).
- **The Tier-B recipe is now reproducible.** Three variants proven: concrete-bool return (contains?), type-preserving return (conj), Option-wrapped return (get). Any future intrinsic in this shape mirrors `infer_positional_accessor` + one of the three return-shape templates.
...
**NEXT MOVE — DECISION-POINT**: clean checkpoint at HEAD `fad1c1c6`+ (lib 834/0, test-build 0 errors, probe 7/7, all four collection ops are intrinsics). User's call between:
  1. **237.7c — `assoc` multi-impl** (HashMap + Record). Different shape from 7b: `assoc` is currently a single-impl alias, NOT `define-dispatch` — this is the records-doctrine slice (assoc on a record is field-update; assoc on a HashMap is k/v put). Needs sub-DESIGN + FM-2-bis probe before BRIEF. *Owed at closure: USER-GUIDE sentence teaching base-vs-holonic records (per arc 234 records-doctrine).*
  2. **Pause + checkpoint**. The collection-ops phase is a natural seam. Could shift to 237.8 arithmetic directly (concrete-per-type defclauses → DELETE widest-contagion → HARD CUT arc-146 `DispatchRegistry`), or hand off to the user for direction.
  3. (Background queue, not blocking): arc 109 #564/#565, the deferred KNOWN-BROKEN sites under 119/130/170/213, NOTE-reconsider-atomize-materialize.
