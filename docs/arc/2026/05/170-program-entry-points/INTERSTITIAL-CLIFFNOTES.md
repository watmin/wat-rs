...
### ⚡ RESUME HERE — remedy/ ward content STAGED (3 dirty files); hash-strip DONE (`de9a5803`); cast via MCP next
**DESIGN CHANGE (`de9a5803`, user-directed):** the vigilatum marker NO LONGER embeds the `@ <commit>` hash. The hash was the hash of its own ward commit → chicken-and-egg → two-commit ward+stamp dance → read-back → fabricated-anchor 4×. CLASS ELIMINATED: drop the hash; git IS the provenance (`git log -G'vigilatum:' -- <home>/mod.rs` recovers the anchor; `git diff anchor..HEAD -- src/<home>/` empty = clean). `docs/VIGILATUM.md` rewritten; all 4 warded homes' stamps de-hashed. **A home now wards in ONE atomic commit (code + hashless stamp together) — no read-back, nothing to fabricate.**
**SPELLS MOVED TO AN MCP (user, 2026-05-31):** `datamancy.dev/*/SKILL.md` paths are STALE. To cast, `ToolSearch` for the spell MCP tools FIRST.
**remedy/ is NOT yet warded.** Its full R1-R4 strike is ALL on disk + safe: **3 files dirty in worktree** — `src/check.rs` + `src/remedy/{mod,retirement}.rs` (R4's Eq-Ord fix + phrasing + tests); **2 files ALREADY in `8a30de54`** — `src/remedy/{distance,rank}.rs` (R2/R3 fixes; smeared via staged-after-soft-reset; safe, grep-verified present — do NOT hunt for them as dirty). Branch = `arc-170-gap-j-v5-deadlock-state`, 1 commit ahead of its origin (the hash-strip, unpushed) — NORMAL push (earlier "1056 ahead of origin/main" was a FALSE alarm: main is a stale sibling branch, not our tracking branch). Gates green at R4: lib 895/0, remedy 61/0, build clean, clippy 877.
...
**NEXT STEP (one-commit ward — the hash dance is RETIRED):** (1) `ToolSearch` the spells MCP, then cast the live 8-spell vigilia on src/remedy/ (core 8: intueri, struere, solvere, purgare, sequi, temperare, conformare, cernere) — the R4 cast has NOT run yet; (2) END TURN, read all 8 verdicts next turn; (3) if all L1+L2=0 (render.rs-extraction + ctor-jargon + temperare cold-path are REJECTED-settled, re-raise=convergence): dirty-set check (exactly `src/check.rs` + `src/remedy/{mod,retirement}.rs`; distance/rank already in `8a30de54`; NO runtime.rs) → write the HASHLESS stamp `//! vigilatum: <date> — vigilia 8-spell L1+L2=0` on `src/remedy/mod.rs` line 1 → **ONE atomic commit** (R4 code + stamp together; no second commit, no read-back, nothing to fabricate) → refresh this block (WARDED→5) + memory → push (normal; ask first per session habit). Brief chain: `BRIEF-remedy-ward-{R2,R3,R4}.md`.
