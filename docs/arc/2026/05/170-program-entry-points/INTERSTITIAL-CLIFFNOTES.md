...
**WARDED homes (3):** `src/check/env.rs` @ `22c89e04`; `src/rust_deps/` @ `7b89053e` (mod.rs + custodia.rs — lifted custodia, annihilated 13 findings/7 spells incl. a dead-arc-citation deferral); `src/argspec/` @ `9b832e21` (stamp `b918d9f4` — ArgSpecError took its own Pattern A cure; EARNED via live 3-round vigilia after a premature stamp was retracted — see Currently).

### ⚡ NEXT HOME — argspec WARDED (done `392b57ce`); homes-walk continues
Argspec ward CLOSED post-compaction: Pattern A retrofit (`ArgSpecError` → `struct {span, head, kind}` + `ArgSpecErrorKind`) committed `6512131d`, vigilatum stamped `392b57ce`, pushed. Gates re-verified at live tree post-compaction (lib 890/0, argspec probe 15/0, workspace build clean). Brief: `docs/arc/2026/05/243-conformare-error-shape/BRIEF-argspec-ward.md`.
**Next home to walk:** function, remedy, or comms (orchestrator picks per momentum). Each: full 8-spell vigilia live-cast → annihilate L1+L2 → atomic commit → vigilatum stamp → push. Carry the logged debt into function/'s ward: `ParseStep::ArityMismatch{actual}` (function/parse.rs:48) is non-Pattern-A — function/'s ward cures it.
