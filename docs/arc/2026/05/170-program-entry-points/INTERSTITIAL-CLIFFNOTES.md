...
> - **248 STRIKING NOW** (`f59e7904` STRIKE-READY): the generative-macro `for`-comprehension. **A BACKGROUND SONNET is implementing 248.1.** **ON WAKE:** look for its task-completion notification; score per `docs/arc/2026/06/248-macro-comprehension/EXPECTATIONS.md` (probe 3/0/0, lib 895/0/1, build clean) + a HARD READ of the `walk_template` hygiene change (a macro form fails subtly there); commit + close. If still running, AWAIT the notification — do NOT re-spawn.
...
> - **Committed/pushed through `f59e7904`** EXCEPT the in-flight 248.1 sonnet work (uncommitted; score + commit on wake).
> - **Recovery-hardening shipped this session** (recolligere "trip" + curare "preserve-the-alarm", live on the datamancy MCP) — run recolligere, don't narrate it.
>
