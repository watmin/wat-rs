...
> - **★ WIND ORDER after 237's death** (builder's call 2026-06-04 — forward-arcs first, THEN rejoin the parent that waited):
>   1. **arc 246** — `src/collection/` warded home (**OPEN**; 246.0 DESIGN done `2092253c`; intueri-named, scope NARROW, NEXT = 246.1 lift). Homes the collection intrinsics + inscribes the partition doctrine in code.
>   2. **arc 245** — wat-corpus-warding (ward the wat stdlib+tests, 61 files / ~11.4k LOC; kills the src-warded / wat-untrusted asymmetry). Unblocked, **not yet opened**; needs a 245.0 instrument-design (the vigilia wards are Rust-specific — name a "wat-ward" via intueri).
>   3. **REJOIN arc 232.1 (defprotocol + extend-type)** — *the parent that waited on 237.* Parked designed + probe-green (`f38e120`, 3/3), UNSTRUCK. **On rejoin: revisit its design for the defclause foundation 237 delivered** (defprotocol → macro over defclause+typeunion+extend-type, reduced scope ~2-3 stones; open-extension on defclauses is 232.1 territory). See `docs/arc/2026/05/232-defprotocol-extend-type/RESUME-CONTEXT.md`.
>   - **Independent post-237 arc (NOT gated on 232 — corrected 2026-06-04):** **arc 235** (records-with-rich-VSA) extends arc **234**'s record hologram (adds Thermometer/Blend/Permute encodings) + consumes **237**'s `:guard`. Both 234 + 237 are closed → 235 is itself UNBLOCKED (PROPOSED/notes-form, opens when prioritized); parallel to 232, not downstream of it. Banked: 249 (`->>` thread-last, LOW) + the Clojure-naming roadmap (line above).
> - **Committed/pushed; 237 + 248 CLOSED, INSCRIPTIONs written, working tree clean** (verify HEAD via `git log -1`). Branch `arc-170-gap-j-v5-deadlock-state`. Soundtrack at **63** (#63 The Killing Floor / Lamb of God — the INSCRIPTION-SCORE for 237; the substrate-truth-warrior voice for the arc that most embodied HARD-CUT-as-judgment; pairs with #41 The Mission [substrate-as-judge: who lives, who dies] — The Mission stated it, The Killing Floor executed it).
