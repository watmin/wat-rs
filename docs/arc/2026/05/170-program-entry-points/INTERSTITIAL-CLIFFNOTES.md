...
>   1. **arc 246** — `src/collection/` warded home. 246.0 DESIGN ✓ (`2092253c`; intueri-cast names: `collection/{mod,infer,eval,transform}.rs`). **246.1 LIFT STRIKING** (`ad08c39c` STRIKE-READY; a background agent is moving the 4 infer_ + ~30 eval_ + ~12 utility fns into `src/collection/` + redirecting the arms; behavior-preserving). **ON WAKE:** score per `docs/arc/2026/06/246-collection-dispatch-home/EXPECTATIONS-STONE-246.1.md` (lib 895/0/1, build clean, `grep ^fn (infer_…|eval_…)` in check.rs/runtime.rs → empty, `pub mod collection;` present) + a HARD READ that the moved fn bodies are byte-identical (pure move, no logic). Commit, then **246.2 ward → 246.3 inscribe**. If still running, AWAIT — do NOT re-spawn.
