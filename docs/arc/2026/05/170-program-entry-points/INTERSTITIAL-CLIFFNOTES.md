...
## Currently (2026-05-27 late — **INTRINSIC BOUNDARY** pivot holds; arc 237.7b custom-arm recipe THRICE-PROVEN (empty? + contains? + conj shipped); **7b-iv (get) Shadowdancer in flight**. AUTHORITY: memory `project_intrinsic_boundary` + `feedback_green_gate_lib_and_build` + `feedback_sonnet_bash_firewall` + `DESIGN-STONE-237.7b.md` — read those + `git log`. HEAD `74b6cd7b`+. The "Headline state" fenced block BELOW is pre-pivot/STALE; this section is truth.)
...
**237.7b — Tier-A / Tier-B doctrine** (settled via FM-2-bis probe `tests/probe_arc237_7b_intrinsic_typing.rs`, 7/7):
- **TIER A** (concrete return, no element-typing) — **plain ∀ TypeScheme** (length-mirror): ✓ `length` 237.7a `8100d9d2` · ✓ `empty?` 237.7b-i `e401c183`.
- **TIER B** (element-typing enforced via **custom inference arm**, mirror `infer_positional_accessor`): ✓ `contains?` 237.7b-ii `fef2c8d9` (3 coll types — Vector<T>/HashSet<T> unify arg1 with T, **HashMap<K,V> with K NOT V**; bool return) · ✓ `conj` 237.7b-iii `2d3259ae` (Vector<T>+HashSet<T>, NO HashMap, type-preserving return) · 🚧 `get` 237.7b-iv IN FLIGHT (Vector<T>+i64→Option<T>, HashMap<K,V>+K→Option<V>; **the Vector arm unifies arg1 with `i64_ty()` NOT X — the INDEX trap, called out in BRIEF**).
- After 7b-iv: **237.7c** = `assoc` multi-impl (HashMap + Record — records-doctrine slice; assoc is currently a single-impl alias, NOT define-dispatch). Then **237.8** arithmetic (concrete defclauses, kill widest-contagion). Then **`DispatchRegistry` deletion** (after arithmetic evacuates).
...
**NEXT MOVE / COMPACTION-READY**: Shadowdancer on 237.7b-iv (`get` ∀T intrinsic + custom arm; agent prompt + BRIEF use raw cargo verify commands, no scripts). The Shadowdancer is the ONLY in-flight state — everything else is committed + pushed. On its completion (task-notification auto-fires; 50-min wakeup hang-detector also armed):
  1. FM-9 verify INDEPENDENTLY with raw cargo commands (NOT the script): probe `probe_arc237_7b_intrinsic_typing` 7/0 (especially `get_vector_precise_element_typing` — the Option<element> precision guard); `cargo build --release --tests --workspace` 0 errors; `cargo test --release --lib -p wat` 834/0; `grep -c "define-dispatch :wat::core::get" wat/core.wat` 0; `grep -c "fn infer_get" src/check.rs` 1; `awk '/fn infer_get/,/^}/' src/check.rs | grep -c "wat::core::HashSet"` 0 (no HashSet arm); `awk '/fn eval_get/,/^}/' src/runtime.rs | grep -c "wat__core__List"` 0; scope = `src/check.rs` + `src/runtime.rs` + `wat/core.wat` + the SCORE only.
  2. Mirror the 7b-iii commit shape (`git show 2d3259ae`) for message.
  3. Then **237.7c** (assoc multi-impl HashMap+Record) OR pause (user's call; clean checkpoint when all 4 collection ops are intrinsics).
