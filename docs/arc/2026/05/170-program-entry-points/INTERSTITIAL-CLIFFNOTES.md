...
**WARDED homes (3):** `src/check/env.rs` @ `22c89e04`; `src/rust_deps/` @ `7b89053e` (mod.rs + custodia.rs — lifted custodia, annihilated 13 findings/7 spells incl. a dead-arc-citation deferral); `src/argspec/` @ `6512131d` (mod.rs stamped — ArgSpecError took its own Pattern A cure; the founding precedent that inspired Pattern A finally embodies it).
