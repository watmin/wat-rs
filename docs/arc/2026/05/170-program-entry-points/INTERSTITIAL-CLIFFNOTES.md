...
## Currently (2026-05-29 very late — **Arc 241 Stones 241.12 + 241.13 BOTH SHIPPED** (Enemy 1 + Enemy 2 of 4 DOWN); HEAD `86d123b7`; battle plan continues at Stone 241.14 (Enemy 4 def-restricted absorption per user reordering); FULL AUDIT completed surfacing 4 orphans (1 already addressing; 3 new: Orphan #2 NoMatchingClauseAtCallSite CheckError twin / Orphan #3 arc 232 defprotocol-extend-type stalled / Orphan #4 arc 226 type-predicates-vsa-similarity stalled). Song #45 Repentless (Slayer) inscribed — DOCTRINE-RESPONSE-TO-DEFECT-DISCOVERY soundtrack; FIRST POST-DISCOVERY-PRE-REMEDIATION drop; DOCTRINE-COMPLETION PAIR with #44 Momma Sed. FOUR-ENEMY BATTLE PLAN: 241.12 = `:wat::runtime::define-alias` HARD CUT ✅ `7244cf43`; 241.13 = `:wat::core::define-dispatch` HARD CUT + DispatchRegistry deletion ✅ `86d123b7` (445-line src/dispatch.rs DELETED + full plumbing cascade + 6 test files migrated/deleted; lib 890/0 preserved; clippy 905 DOWN 3 from 908; ~25 min actual vs 90-180 predicted = substantially under-band); 241.14 = `:wat::core::def-restricted` → metadata-map absorption (Enemy 4 NEXT — honors broken Stone 241.6 D10+line182 commitment that was orphaned when Stone 241.10's scope shifted to remedy apparatus); 241.15 = `:wat::core::define` eval-time residue (Enemy 3); 241.16 = INSCRIPTION closes arc 241. **DOCTRINE FAILURE PATTERN named via audit**: `feedback_defer_by_naming` — mid-arc scope shift orphans displaced commitments when no re-commitment to new stone number lands; substrate then carries promise that nothing on queue honors. Same shape surfaces in Stone 237.4 NoMatchingClauseAtCallSite (twin work fell off; only runtime variant refined). **Stone 241.12 TWO TRAP-DOORS absorbed honestly** (closure_extract retired-form emission + runtime.rs Gap D name-field overwrite). NEW MEMORY this session: `feedback_score_present_check_before_closure`; doctrine-memory pending for mid-arc-scope-shift-requires-explicit-redirect. Arc 242 CLOSED done-done at `28ab83ef`. AUTHORITY: read in order — `docs/COMPACTION-AMNESIA-RECOVERY.md` → `docs/DUNGEON-CRAWL.md` → `docs/arc/2026/05/241-function-signature-unification/SCORE-STONE-241.13.md` (Enemy 2 result) → `SCORE-STONE-241.12.md` (Enemy 1 + trap-doors + bandaid-rip-with-receipts) → `DESIGN-STONE-241.6.md` (the orphaned commitment that surfaces in Stone 241.14 + the pattern audit found elsewhere). HEAD `86d123b7` on `arc-170-gap-j-v5-deadlock-state` — pushed.)
