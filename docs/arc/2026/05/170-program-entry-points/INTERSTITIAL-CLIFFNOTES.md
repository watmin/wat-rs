...
**remedy/ is NOT yet warded.** Its full R1-R4 strike is on disk UNCOMMITTED (survives compaction): 5 files vs origin — `src/remedy/{mod,distance,rank,retirement}.rs` + `src/check.rs` (+178 lines). HEAD=`8a30de54`, origin=`866f196f`, ahead 3 (all are R2/R3/R4 BRIEF docs only — the ward CODE is uncommitted by design). Gates green at R4: lib 895/0, remedy 61/0, build clean, clippy 877.
