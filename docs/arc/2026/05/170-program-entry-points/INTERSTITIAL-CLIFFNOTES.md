...
## Currently (2026-05-27 late — **INTRINSIC BOUNDARY** pivot holds; **arc 237.7 COMPLETE — every collection-op surface (length / empty? / contains? / conj / get / assoc) is a ∀T Rust intrinsic atop per-Type leaves; HashMap-only `define-alias :wat::core::assoc` HARD CUT; records-doctrine slice closed**. NEXT = 237.8 arithmetic OR pause. AUTHORITY: memory `project_intrinsic_boundary` + `feedback_green_gate_lib_and_build` + `feedback_sonnet_bash_firewall` + `DESIGN-STONE-237.7b.md` + `DESIGN-STONE-237.7c.md` — read those + `git log`. HEAD `a9961421`. The "Headline state" fenced block BELOW is pre-pivot/STALE; this section is truth.)
...
**237.7 — Tier-A / Tier-B doctrine COMPLETE** (settled via FM-2-bis probes `tests/probe_arc237_7b_intrinsic_typing.rs` (7/7) + `tests/probe_arc237_7c_assoc_polymorphic.rs` (6/6); all collection-op surfaces shipped):
- **TIER A** (concrete return, no element-typing) — **plain ∀ TypeScheme** (length-mirror): ✓ `length` 237.7a `8100d9d2` · ✓ `empty?` 237.7b-i `e401c183`.
- **TIER B** (element-typing enforced via **custom inference arm**, mirror `infer_positional_accessor`): ✓ `contains?` 237.7b-ii `fef2c8d9` (3 coll types — Vector<T>/HashSet<T> unify arg1 with T, **HashMap<K,V> with K NOT V**; bool return) · ✓ `conj` 237.7b-iii `2d3259ae` (Vector<T>+HashSet<T>, NO HashMap, type-preserving) · ✓ `get` 237.7b-iv `fad1c1c6` (Vector<T>+i64→Option<T>, HashMap<K,V>+K→Option<V>; INDEX trap honored) · ✓ `assoc` 237.7c `a9961421` (HashMap<K,V> via Parametric arm + Record umbrella via Path arm; HARD CUT of the `define-alias`; free ∀T at arg2 in the Record arm — first 237.7 stone where arg slot is genuinely unconstrained at check-time, field-type stability is `eval_record_assoc`'s job at runtime; **honest forward-correction: the Record Path arm matches BOTH `:wat::Record` AND `:wat::holon::Record`** per the canonical convention at `check.rs:6398` — sonnet caught the gap via the probe's holonic row, mirroring the established substrate pattern; flavor preservation via `eval_record_assoc`'s base early-return + holonic fallthrough with parity rebuild).
- **The Tier-B recipe is fully reproducible.** FOUR return-shape templates proven: concrete-bool (contains?), type-preserving (conj), Option-wrapped (get), umbrella-Path with free-∀T-arg (assoc). FOUR arg0-shape templates: 1-coll-Parametric, 2-coll-Parametric, 3-coll-Parametric, Parametric+Path-umbrella. Future intrinsics in this family mirror one combination.

**arcs 239 + 240 — CLOSED** (rot detour). 239 span-rot + #566 build gate `35dba8c5`; 240 runtime-rot `8d08eda2` (substrate gaps + telemetry .wat 4-element recipe sweep). Every non-drift runtime failure DEFERRED + KNOWN-BROKEN-marked on owning OPEN arc: lru/holon-lru→119+130, wat-cli-fork+ambient-stdio+sqlite-log-daemon→170, lifeline→213. #564 (f64 floor/ceil) + #565 (namespace reorg + intrinsic/substrate vocab) queued in arc 109. arc-109 NOTE filed: reconsider `atomize`/`materialize` vs `to-holon`/`from-holon` family (challenge intueri later). Phrase saved for **soon-but-not-now** use: `project_rooms_are_real` ("because the rooms are real" — do NOT deploy unprompted).

**GREEN-GATE (momentary)**: `cargo test --release --lib -p wat` + `cargo build --release --tests --workspace`. The wrapper `./scripts/green-gate.sh` exists BUT **NEVER invoke wrapper scripts in BRIEFs or agent prompts** — the Anthropic firewall denies them for sub-agents AND for orchestrator under restricted permission modes. Use the two raw cargo commands as SEPARATE lines (`feedback_sonnet_bash_firewall` updated 2026-05-27 with the scripts ban; the script itself is fine as a *standalone* runnable, but agent prompts give the raw cargo). Full `cargo test --workspace` RUN held off until arc 170 closes the process leaks (`feedback_green_gate_lib_and_build`).

**NEXT MOVE — DECISION-POINT (arc 237's BIG seam)**: clean checkpoint at HEAD `a9961421` (lib 834/0, test-build 0 errors, both 237.7b + 237.7c probes 13/13 across the two probe files). Collection-op intrinsic phase fully evacuated. User's call between:
  1. **237.8 — arithmetic family migration** (the heavy stone-block). Per cliffnotes "Headline state" + task list (#254–#259): 4 sub-families — arithmetic (`+`/`-`/`*`/`/`, ~16 per-type impls per arc 148 slice 2 estimate) + comparison (`=`/`<`/`>`/`<=`/`>=`) + holon-pair (4 polymorphic handlers) + time-arith (Duration math). THE DECISION locked: no implicit numeric coercion — `(:wat::core::+ 1 2.0)` → ERROR (homogenize explicitly). Recipe per family: AUDIT → migrate per-Type as concrete defclauses → DELETE widest-contagion (`infer_arithmetic` / `eval_arithmetic_variadic` / `is_numeric`) → HARD CUT arc-146 `DispatchRegistry` (after arithmetic evacuates the last consumer). Could slice as 4 stones (one per family) or 2 (numeric pair + holon/time pair).
  2. **237.9 INSCRIPTION** (after 237.8 — folds arc 146 + arc 148 closures + USER-GUIDE records-doctrine sentence per `feedback_inscription_immutable`).
  3. **Pause + checkpoint** at the seam (collection-ops + records-doctrine done is a natural milestone).
  4. **Micro-verify owed**: S-C.2d `:wat::Record/same-data?` — the `3513e930 docs(arc237): S-C.2d sub-DESIGN + BRIEF + EXPECTATIONS` commit exists; need to grep for the SHIPPED commit to confirm. If unshipped, it's a small in-flight stone to close before 237.9. (Low-risk verify; doesn't block 237.8.)
  5. (Background queue, not blocking): arc 109 #564/#565, deferred KNOWN-BROKEN sites under 119/130/170/213.
