...
> - **CHAIN TO THE DEMISE:** 248.1 (`for` tool) ✓ → **237.8d** (collections + equality BOTH stay intrinsic, scope-honest; inscribe the partition rule; `dispatch_keyword_head` retained) **[absorbs the re-aimed 248.2]** → 237.9 (INSCRIPTION) → **237 DIES**.
...
> - **Committed/pushed through `4a67f91f`** (248.1 closed; DISPATCH.md shipped; 237.8d STRIKE-READY). **237.8d STRIKING NOW** — a background agent is cutting the 4 per-Type equality aliases + inscribing the partition (`docs/DISPATCH.md`). **ON WAKE:** score per `docs/arc/2026/05/237-polymorphism-consolidation/EXPECTATIONS-STONE-237.8d.md` (probe 10/0/0, 8c grid green, lib 895/0/1, build clean, `grep i64::=\|f64::=` in src/+wat/ → zero) + a HARD READ of the diff (equality IMPL must be UNTOUCHED — comments-only at `infer_equality`, deletions at the alias arms); commit; then write **237.9 INSCRIPTION → 237 DIES.** If still running, AWAIT — do NOT re-spawn. Branch `arc-170-gap-j-v5-deadlock-state` (verify `git log -1`).
