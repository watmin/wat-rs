...
## Currently (2026-05-29 very late — **Arc 242 `lexeme-role-doctrine` DONE-DONE + Arc 241 paused at Stone 241.12 STRIKE-READY**. Spawn-block discipline: arc 242 spawned as child of arc 241 (per `feedback_spawn_block_winding`); wound depth-first; arc 241 resumes. Two doctrines now substrate-enforced law: Doctrine 1 (bare lexeme=value; :wat::core::* keyword=type); Doctrine 2 (scalar lowercase; container PascalCase). Arc 242 stones: 242.1 SHIPPED `b4eb920f` (`:wat::core::Char` HARD CUT + doctrine memory inscribed + bare nil verified-operational; 5th RETIREMENT_TABLE entry); 242.2 SHIPPED `9c8e8546` (Doctrine 1 SELF-ENFORCING — type-check rejection arm; 166 files cascade via orchestrator-direct bulk sed after FM 16 sonnet firewall trip); 242.3 INSCRIPTION SHIPPED `b7fc45f6`; **SCORE-STONE-242.2.md** CLOSED orchestrator-direct `a75f03bf` (paperwork-gap surfaced during user-directed done-done verification — BRIEF directed sonnet to author at strike time; sonnet shipped substrate but never wrote the SCORE; gap closed post-INSCRIPTION under "protocol compliance — any deviation is a violation" directive; honest delta inscribed in SCORE). Third bandaid-rip-with-receipts consumer locked (Stone 241.10 apparatus operational across 3 consumers: define HARD CUT + Char HARD CUT + value-position-rejection). EIGHTH substrate-as-teacher layer surfaced — chronicle authoring discipline (INTERSTITIAL is orchestrator-exclusive; sonnet's "draft for review" framing is the violation in writing; memory `feedback_sonnet_never_drafts_interstitial` inscribed). Two NEW MEMORIES this corner: `feedback_hard_cut_admits_no_bypasses` (no privileged paths; orchestrator must check sonnet's REASONING against doctrine) + `feedback_sonnet_never_drafts_interstitial` (chronicle integrity). **DISCIPLINE REINFORCEMENT** (from SCORE-242.2 honest-delta): post-strike checklist must verify SCORE-doc-present BEFORE marking stone closed; cliffnotes "stone SHIPPED" annotation does NOT prove SCORE shipped alongside; closure verification ≠ commit verification. Lib 890/0 preserved through both arc 242 stones; clippy 902; probe 242.1 4/4 + probe 242.2 6/6 + all prior probes preserved. **NEXT MOVE: Stone 241.12 (defalias mint) resumes at STRIKE-READY `e803e0f9`** — paused at the moment user noticed substrate alias use that couldn't migrate to defn/def; needs `defalias` form; user direction "at the end of this work :wat::runtime::define-alias is dead - :wat::core::defalias is the only way to do name aliasing"; STRIKE-READY artifacts at e803e0f9 (DESIGN + probe + BRIEF + EXPECTATIONS); spawn sonnet when user authorizes. After 241.12, Stone 241.13 INSCRIPTION closes arc 241. Arc 237.8b waits longest. **Stone 241.11.fix round 1 work was LOST** during 241.12 WIP discard before arc 242 opened — 14 test migrations + 1 doc update need re-doing within Stone 241.12 scope. AUTHORITY: read in order — `docs/COMPACTION-AMNESIA-RECOVERY.md` → `docs/DUNGEON-CRAWL.md` → arc 242 `INSCRIPTION.md` → arc 241 `DESIGN-STONE-241.12.md` + `BRIEF-STONE-241.12.md` (STRIKE-READY but augment BRIEF to fold in lost round-1 work) → `SCORE-STONE-242.2.md` → `SCORE-STONE-242.1.md`. HEAD `a75f03bf` on `arc-170-gap-j-v5-deadlock-state` — pushed.)
