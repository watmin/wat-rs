...
>   3. **arc 249** — thread-last `->>` + the `->` glyph reckoning (**STUB on disk:** `docs/arc/2026/06/249-thread-last/STUB.md`). NOT a trivial mint: `->` is the **type-arrow** — *no threading macro exists* (the STUB forward-corrects 247's false "`->` thread-first exists" premise). A design verdict, which legitimately includes **deciding NOT to add threading** (glyph collision + LOW value; builder rarely threads).
