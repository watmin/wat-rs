...
## Currently (2026-05-29 very late — **Arc 241 RESUMED at Stone 241.12 STRIKE-READY-v2 (`c64c9481`); sonnet IN FLIGHT executing the strike**. THREE-ENEMY BATTLE PLAN identified: 241.12 = `:wat::runtime::define-alias` HARD CUT (Enemy 1 — IN FLIGHT); 241.13 = `:wat::core::define-dispatch` HARD CUT (Enemy 2 — substrate scaffolding deletion since wat-callers already migrated to ∀T intrinsics per arc 237.7); 241.14 = `:wat::core::define` eval-time residue completion (Enemy 3 — closes Stone 241.11's partial HARD CUT); 241.15 = INSCRIPTION closes arc 241. User directive 2026-05-29 very late: *"define-alias, define-dispatch, define all die"* + *"this is the fight now - we know we have multiple enemies - we manage the fight - we are the best - we prove it relentlessly"*. **Trap-door audit completed BEFORE strike** — 13 active surface callers (4 wat/core + 2 wat/list + 6 tests + 1 wat/runtime macro impl); Stone 241.11.fix round 1 lost-work fold-in scope: ~24 test sites + ~10 docs; defclause already exists (Stone 237.2 `bdd9eb6c`) so Enemy 2 needs no caller migration; Enemy 3 independent. STRIKE-READY-v2 corrections from v1 contamination (5 violations): probe carried "26 callers stays" privileged-path framing (per `feedback_hard_cut_admits_no_bypasses`); probe assertions too weak (`is_ok()` tolerated unknown-form-as-no-op); DESIGN+BRIEF S3/S4 audited substrate define-as-alias (moot post-241.11); no three-enemy context; no SCORE-write directive (new memory `feedback_score_present_check_before_closure`); caller count wrong. V2 probe verified 5/5 disconfirm at HEAD. Sonnet brief includes explicit doctrine constraints (HARD CUT TOTAL; ONE form not two layers — native defalias; INTERSTITIAL orchestrator-exclusive; SCORE-write at end). Arc 242 CLOSED done-done at `28ab83ef` (SCORE-242.2 paperwork-gap closure + memory inscription completed earlier in session). Lib 890/0; clippy 902 at gate. AUTHORITY: read in order — `docs/COMPACTION-AMNESIA-RECOVERY.md` → `docs/DUNGEON-CRAWL.md` → `docs/arc/2026/05/241-function-signature-unification/DESIGN-STONE-241.12.md` (STRIKE-READY-v2; § Battle plan documents all three enemies) → `BRIEF-STONE-241.12.md` → `EXPECTATIONS-STONE-241.12.md` → prior SCOREs. HEAD `c64c9481` on `arc-170-gap-j-v5-deadlock-state` — pushed; sonnet executing.)
