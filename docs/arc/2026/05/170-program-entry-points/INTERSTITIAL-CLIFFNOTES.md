...
**WARDED homes (3):** `src/check/env.rs` @ `22c89e04`; `src/rust_deps/` @ `7b89053e` (mod.rs + custodia.rs — lifted custodia, annihilated 13 findings/7 spells incl. a dead-arc-citation deferral); `src/argspec/` @ `b64b04b4` (ArgSpecError took its own Pattern A cure; EARNED via live 3-round vigilia after a premature stamp + fabricated anchor were both corrected — see Currently).

### ⚡ NEXT HOME — argspec EARNED (`b64b04b4`); homes-walk continues
Argspec ward EARNED via live 3-round 8-spell vigilia (R1 double-span L1 → R2 own-wake → R3 CONVERGED L1+L2=0). Briefs: `BRIEF-argspec-ward{,-R2,-R3}.md`. Two corrected missteps inscribed in `feedback_warded_means_annihilated`: premature stamp (recorded≠live cast) + fabricated anchor (hash-before-commit).
**STAMP PROTOCOL (hard order, learned this ward):** (1) cast live vigilia on the dirty tree → (2) WAIT for all 8 verdicts → (3) confirm L1+L2=0 → (4) commit the strike → (5) READ BACK the commit hash → (6) write THAT hash into the stamp → (7) commit stamp → (8) push. NEVER overlap commit with cast; NEVER write a hash you haven't read back from git.
**Next home to walk:** function, remedy, or comms (orchestrator picks per momentum). Carry the logged debt into function/'s ward: `ParseStep::ArityMismatch{actual}` (function/parse.rs:48) is non-Pattern-A — function/'s ward cures it.
