...
## Currently (2026-05-29 very late — **Arc 241 Stone 241.12 SHIPPED `7244cf43` (Enemy 1 of 3 DOWN); arc 241 continues at Stone 241.13 (Enemy 2 — define-dispatch HARD CUT)**. THREE-ENEMY BATTLE PLAN STATUS: 241.12 = `:wat::runtime::define-alias` HARD CUT (Enemy 1 ✅ DONE — native defalias minted; macro impl at wat/runtime.wat:18 DELETED; 13 surface callers migrated; HARD-CUT arm + 6th RETIREMENT_TABLE entry + remedy quality); 241.13 = `:wat::core::define-dispatch` HARD CUT (Enemy 2 — substrate scaffolding deletion since wat-callers already migrated to ∀T intrinsics per arc 237.7); 241.14 = `:wat::core::define` eval-time residue completion (Enemy 3 — closes Stone 241.11's partial HARD CUT + completes 2 trap-door cleanups already done in 241.12 — see below); 241.15 = INSCRIPTION closes arc 241. **Stone 241.12 TWO TRAP-DOORS absorbed honestly (per `feedback_trap_door_build_the_dependency`)**: (1) closure_extract.rs `function_to_define_form_with_body` was emitting retired `:wat::core::define` for prologue re-freeze — Stone 241.11 latent bug surfaced by S6 cascade; rewritten to emit 6-item defn form; (2) runtime.rs Gap D `register_runtime_defs_form` step 9 overwriting `sym.functions` with `eval_fn`'s `Function { name: None }`, erasing step 6's `name: Some(...)`; closure extraction was emitting `__anon` names; preserved name field on overwrite. **Clippy 908** (+6 over 902 gate; user clarification: arc 109 sweeps to zero; not regression — +1 truly new from `register_defalias` Err-variant matching pre-existing 582-fn pattern; +5 line-shift re-attribution). Lib 890/0 preserved; probe 5/5; all prior probes preserved (242.2 6/6, 242.1 4/4, 241.11 5/5, 241.10 8/8). S6 consistency pass migrated ~19 test sites + 1 doc (docs/CIRCUIT.md) from `:wat::core::define` → `:wat::core::defn`. NEW MEMORY this session: `feedback_score_present_check_before_closure` (post-strike SCORE-doc-present check before closure marker; surfaced from arc 242 paperwork-gap). Arc 242 CLOSED done-done at `28ab83ef`. AUTHORITY: read in order — `docs/COMPACTION-AMNESIA-RECOVERY.md` → `docs/DUNGEON-CRAWL.md` → `docs/arc/2026/05/241-function-signature-unification/DESIGN-STONE-241.12.md` (STRIKE-READY-v2; § Battle plan documents all three enemies) → `SCORE-STONE-241.12.md` (Enemy 1 result + 2 trap-door lessons) → prior SCOREs. HEAD `7244cf43` on `arc-170-gap-j-v5-deadlock-state` — pushed.)
