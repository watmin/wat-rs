...
> - **237.8c equality = Shape B** (`7851d25c`) is CORRECT but UN-consolidated (a bespoke Rust `eval_eq`/`infer_equality`, not a defclause). 237 = polymorphism-CONSOLIDATION → equality must JOIN the defclause mechanism. Uniform-equality hand-written = ~22 identical clauses (ceremony) → so we build a MACRO to generate them.
...
> - **248.2 NEXT** (the strike that consolidates equality): use `for` to make `=`/`not=` a **macro-generated `defclause`** — the equality consolidation 237.8c's Shape B deferred (Shape B is correct *as a value* but a bespoke Rust `eval_eq`/`infer_equality`, NOT on the one mechanism). **ON STRIKE:** `DESIGN.md` §Slicing already sketches it → settle the DESIGN (how the macro takes the type-list + emits one uniform clause per type; equality's impl is *uniform*, so NO per-Type leaf decomposition) → FM-2-bis probe (a macro generating the `=`/`not=` defclause from a small type-list type-checks + dispatches: i64 + f64 both work) → BRIEF (positive-only) + EXPECTATIONS → spawn sonnet → independent SCORE → commit.
> - **CHAIN TO THE DEMISE:** 248.1 (`for` tool) ✓ → **248.2** (equality `=`/`not=` → macro-generated defclause = the consolidation) → 237.8d (collections STAY intrinsic, scope-honest; `dispatch_keyword_head` retained) → 237.9 (INSCRIPTION) → **237 DIES**.
> - **The discriminant** (*clause = monomorphic; intrinsic = type-level computation*) is marked IN-SOURCE: check.rs `infer_list` + runtime.rs `dispatch_keyword_head` ([[project_dispatch_clause_vs_intrinsic]]).
...
> - **Stubbed:** 245 (wat-corpus-warding), 246 (`src/collection/` home), 249 (`->>` thread-last — LOW priority, user rarely threads).
> - **Committed/pushed through `c8280343`** (248.1 closed); working tree CLEAN. Branch `arc-170-gap-j-v5-deadlock-state` (verify `git log -1`).
