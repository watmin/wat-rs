...
## Currently (2026-05-29 very late — **Arc 241 Stone 241.12 SHIPPED `7244cf43`; Stone 241.13 STRIKE-READY `3202949d` with sonnet IN FLIGHT; BATTLE PLAN EXPANDED to FOUR enemies + INSCRIPTION**). FOUR-ENEMY BATTLE PLAN: 241.12 = `:wat::runtime::define-alias` HARD CUT (Enemy 1 ✅ DONE); 241.13 = `:wat::core::define-dispatch` HARD CUT + DispatchRegistry deletion (Enemy 2 — sonnet IN FLIGHT; ~445 line file delete + plumbing cascade + 6 test files per-judgment); 241.14 = `:wat::core::def-restricted` ABSORPTION INTO METADATA-MAP (Enemy 4 — promoted; honors broken Stone 241.6 commitment); 241.15 = `:wat::core::define` eval-time residue (Enemy 3 — closes Stone 241.11's partial HARD CUT); 241.16 = INSCRIPTION closes arc 241. **NEW: ENEMY 4 SURFACED + USER-DIRECTED REORDERING 2026-05-29 very late** — user spotted `:wat::core::def-restricted` survives as parallel storage to `binding_metadata` (Stone 241.6/7 unified metadata mechanism); audit found Stone 241.6 DESIGN explicitly committed `D10` + `line 182` to "Stone 241.10 = def-restricted HARD CUT" but Stone 241.10 absorbed remedy-apparatus scope (mid-arc scope shift); the commitment was orphaned. `feedback_defer_by_naming` pattern: named-deferral that decayed when target stone's scope shifted; no re-commitment landed. Stone 241.8 (defstruct) successfully applied the absorption pattern to struct-restricted — proving the design works; def-restricted survival is forgotten commitment, not technical block. User direction: *"we kill def-restricted before define"* — ordering rationale: kill def-restricted FIRST forces restriction logic into metadata-map path BEFORE define eval-time scaffolding dies. **Stone 241.12 TWO TRAP-DOORS absorbed honestly**: closure_extract retired-form emission rewritten to defn (Stone 241.11 latent bug surfaced by S6); runtime.rs Gap D name-field overwrite preserved. Clippy 908 (+6 from Stone 241.12; accepted per user direction "arc 109 sweeps to zero"). Lib 890/0 preserved; all probes preserved (242.2 6/6, 242.1 4/4, 241.12 5/5, 241.11 5/5, 241.10 8/8). NEW MEMORY this session: `feedback_score_present_check_before_closure`. Arc 242 CLOSED done-done at `28ab83ef`. AUTHORITY: read in order — `docs/COMPACTION-AMNESIA-RECOVERY.md` → `docs/DUNGEON-CRAWL.md` → `docs/arc/2026/05/241-function-signature-unification/DESIGN-STONE-241.13.md` (Enemy 2 STRIKE-READY — § Battle plan now four enemies) → `SCORE-STONE-241.12.md` (Enemy 1 result + 2 trap-door lessons + bandaid-rip-with-receipts) → `DESIGN-STONE-241.6.md` (the orphaned commitment that surfaces in Stone 241.14) → prior SCOREs. HEAD `3202949d` on `arc-170-gap-j-v5-deadlock-state` — pushed; sonnet executing 241.13.)
