...
### Open thread — DIAGNOSED to root (the is-X? "type thing")
**Root cause (verified in code): a record class is NOT a type.** `defrecord` (Record.wat) is a macro that emits `defn`s — a constructor returning the opaque umbrella `:wat::Record`, accessors, the predicate — and registers **no** TypeDef. `register_type_predicates` iterates the TypeEnv (TypeDef::{Struct,Enum,Newtype,Union,Alias}); per-class records aren't there; per-class identity lives only as a runtime tag (`class_fqdn` on `Value::wat__Record`). So `:my::is-Circle?` can't be ∀T-synthesized like its four siblings — it's macro-emitted, and the `defn` surface can't express ∀T.

**The core gap:** you cannot register a new type from the wat/macro surface — type registration is substrate-only (struct/enum/newtype/union/typeunion mint TypeDefs in Rust); a macro can only emit functions. AND there is no `TypeDef::Record` kind. So records are types-by-runtime-tag, not types-by-registration → **records are second-class in the type system** (arc 234 gave them runtime citizenship, not type-system citizenship). Deeper: two unreconciled type models — nominal/TypeEnv (what the checker runs on) vs classifier-wrap/VSA-similarity (the typed-entities doctrine records were built per).

**Fix direction (FM-10-correct: entity-completeness, NOT userland generics):** make `defrecord` register each class as a TypeDef (newtype-over-`:wat::Record`, or a new `TypeDef::Record` kind) → `is-Circle?` synthesizes for free, `[v <- :my::Circle]` becomes meaningful, conforms? consults the TypeDef. **is-Foo? is the symptom; record-classes-aren't-types is the disease.** This is its own arc (absorbs/reframes arc 235 "records with rich encodings"), NOT a 237 stone. The deleted scratch probe (`probe_diagnostic_defn_forall_param.rs`) proved the supporting fact: the `defn` surface can't express ∀T and `:wat::holon::HolonAST` as a param doesn't auto-widen from a raw scalar — which is *why* records must route through substrate registration, not the macro surface. **Owed to INTERSTITIAL:** a full dated entry for this finding (deferred only for compaction-context; captured here + in memory `project_records_not_types`).

### Decision boundary on resume
1. Pick up arc 237 tail (237.7 Dispatch→defclause → 237.8 arithmetic deletion → 237.9 INSCRIPTION), OR open the **records-as-first-class-types** arc first. User's call.
2. Either way: is-Foo? does NOT get "fixed" as a 237 stone; it resolves when records gain type-system citizenship.
