...
>   1. **arc 246** — `src/collection/` warded home (**OPEN**; 246.0 DESIGN `2092253c`; intueri-named, scope NARROW; NEXT = 246.1 lift). Homes the collection intrinsics + inscribes the partition doctrine in code.
...
>   3. **arc 249** — threading macros: **BOTH `->` (thread-first) + `->>` (thread-last)** (**STUB on disk:** `docs/arc/2026/06/249-threading-macros/STUB.md`; promoted 2026-06-04 from thread-last-only). NOT a trivial mint: *no threading macro exists* and `->` is the **type-arrow** (the STUB forward-corrects 247's false "`->` thread-first exists" premise). ASYMMETRIC: `->>` glyph is free (mintable), `->` collides with types. A design verdict (likely an `intueri` cast), which legitimately includes **deciding NOT to add threading** (collision + LOW value; builder rarely threads).
