...
> - **★ FRONTIER (237's death UNBLOCKS these):** arc **245** (wat-corpus-warding — ward the wat stdlib+tests, 61 files / ~11.4k LOC, to a defined bar; kills the src-warded / wat-untrusted asymmetry) + arc **246** (`src/collection/` warded home — lift the ~35 collection-intrinsic impls out of check.rs/runtime.rs; execute-ready bounded homes-walk). Both STUBBED → now **OPEN**. Also banked: 249 (`->>` thread-last, LOW priority) + the Clojure-naming roadmap (line above). **NEXT ARC = user's call.**
