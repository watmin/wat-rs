...
**WARDED homes:** `src/check/env.rs` @ `22c89e04`; `src/rust_deps/` @ `7b89053e` (mod.rs + custodia.rs — lifted custodia, annihilated 13 findings/7 spells incl. a dead-arc-citation deferral).

### ⚡ RESUME HERE — argspec ward: STRIKE APPLIED + UNCOMMITTED in working tree
6 modified files survive compaction on disk (NOT committed): `src/argspec/{error,mod,parse}.rs`, `src/function/parse.rs`, `src/macros.rs`, `tests/probe_arc241_stone1_argspec_canonical.rs`. Pattern A retrofit DONE: `ArgSpecError` → `struct {span, head, kind}` + `ArgSpecErrorKind` (5 unit + MalformedTypeKeyword{inner} + TrailingItems{count}); `into_parts()`→`kind.reason()`; 4 From impls direct-field; `From<ArgSpecError> for MacroError` added (the ~30-line hand-match + the `classify()` ghost-comment in macros.rs GONE → `.map_err(MacroError::from)?`); `is_bare_symbol` promoted `pub(crate)`, 2 arrow-inlines killed (function/parse.rs:81, macros.rs:430); 241.5 PENDING→DONE; idx→cursor, post_rest→trailing_start.
**Kill-confirm re-cast: intueri CONVERGED, solvere CONVERGED, conformare CONFORMANT (ArgSpecError NOW embodies Pattern A — the founding precedent took its own cure). exigere was 4th (finding was the trivial 241.5 PENDING→DONE, already applied).** Gates verified independently: lib 890/0, function 8/0, argspec probe 15/0, clippy 894, workspace clean. `From<ArgSpecError> for CheckError` confirmed untouched-LIVE (infer.rs:71); zero `Span::unknown()` in argspec.
**NEXT STEPS (in order):** (1) re-verify gates (or confirm exigere CONVERGED — finding trivial); (2) `git add` the 6 files + atomic commit "argspec WARDED"; (3) stamp `//! vigilatum: 2026-05-30 @ <that-commit> — vigilia 8-spell L1+L2=0` on `src/argspec/mod.rs` (argspec is mod.rs-rooted → stamp on mod.rs); (4) commit the stamp; (5) push. THEN argspec = 3rd WARDED home. Brief: `docs/arc/2026/05/243-conformare-error-shape/BRIEF-argspec-ward.md`.
