...
- Slice 3 Gap F-1 + F-3 + F-2 + G (probes) + H + I-A ✅ shipped; closes the Phase E V4 substrate diagnostic cascade + the `is_prelude_form` / `is_declaration_form` unification
- **CURRENT BLOCKER:** Gap I-B (make `def` not special — retire the def-specific check-time validator arm + tighten def's runtime arm to emit a `DeclarationInExpressionPosition` error like `define`; closes Gap I-A's honest-delta requirement that def at fn-body do-prefix end-to-end-works through the lift) + deftest-hermetic Path E macro shape rewrite (small wat/test.wat edit). Architectural framing for Gap I-B: arc 157 minted def with a check-time validator AND a permissive runtime arm (assumed validator was the only entry guard). Gap I-A's lift mechanism made the validator over-restrictive for def-at-fn-body-do-prefix. The right answer is retirement of both the check-time def-specific arm AND tightening of the runtime arm to match the other 7 declarations (no silent no-op; loud position-class error). Closes a latent arc-157 defect (permissive runtime arm) while restoring symmetry. User direction 2026-05-13: *"making it not special feels best."*
