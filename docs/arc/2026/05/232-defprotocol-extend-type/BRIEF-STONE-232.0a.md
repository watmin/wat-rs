...
Mint two wat-callable reflection verbs that the typed-entities doctrine demands but the substrate hasn't yet exposed:

1. **`:wat::holon::extract-classifier <h>` → `:Option<String>`** — lift the existing Rust `fn extract_classifier(holon: &HolonAST) -> Option<String>` (at `src/runtime.rs:13986`) to a wat verb. Returns `Some(class-name)` for canonical-wrap shape `(Bind (Atom <s>) <inner>)`; `None` otherwise. defprotocol's dispatcher consumes this.

2. **`:wat::holon::Bind/inner <h>` → `:Option<HolonAST>`** — NEW Rust fn + wat verb. Returns `Some(inner)` for literal `(Bind _ inner)`; `None` otherwise. Mirrors the existing `Bundle/children` pattern (variant-narrow decomposer). defrecord accessor synthesis consumes this composed with `Bundle/children`.

After this stone: defrecord instances can be INSPECTED (the symmetric counterpart to construction). defprotocol macro work (Stone 232.1) unblocks.
...
2. **`fn bind_inner(holon: &HolonAST) -> Option<HolonAST>`** — new Rust helper alongside `extract_classifier`. Matches `HolonAST::Bind(_, inner)` → `Some((*inner).clone())`; other → None.

3. **`eval_bind_inner`** — new Rust fn that takes args + list_span, expects 1 arg evaluating to `Value::holon__HolonAST(holon_ast)`, calls `bind_inner(&holon_ast)`, wraps as `Value::Option(Arc::new(...))` with `Value::holon__HolonAST` payloads. Dispatch arm: `":wat::holon::Bind/inner" => eval_bind_inner(args, list_span, env, sym)`.

4. **Type-checker integration** — both verbs need entries in `src/check.rs` so the type checker accepts them. Look for how `Bundle/children` is registered (`src/check.rs` has parametric-call inference for `Bundle/children` returning `Vector<HolonAST>`); mirror that shape for the new verbs.
...
2. **`tests/probe_diagnostic_typed_entities_reflection.rs`** — FM 2-bis probe (5 contracts; already on disk from `96bb6f4`). Currently FAILS (verbs don't exist). **The probe IS the success criterion** — flip 0/5 → 5/5.
...
- **STOP-1:** unexpected compile errors NOT tracing to the 2 new verbs
- **STOP-2:** baseline lib tests regress below 827
- **STOP-3:** **90 min elapsed** (predicted 30-60; smaller stone)
- **STOP-4:** holon-rs touched
- **STOP-5:** new clippy warning above 54
- **STOP-6:** scope creep — defprotocol macro work, Atom/inner decomposer, defrecord accessors
