...
| 233.2 | **Provenance on Values** — see [sub-DESIGN](DESIGN-STONE-233.2.md). Shape C (`Value::Tracked` wrapper variant + transparency contracts). SLICED into 233.2.a (mint + transparency) → 233.2.b (`keyword/from-string` producer tag) → 233.2.c (4-producer sweep) → 233.2.d (uniform `list_span` substrate-symmetry; see [DESIGN-STONE-233.2.d.md](DESIGN-STONE-233.2.d.md)) → 233.2.e (AST-derived for let-bindings + literals on enriched substrate). Four-questions revealed Option-A "AST-derived-only" failed Honest (doesn't close runtime-built case); corrected to full Value-level provenance via Tracked variant. Post-compaction four-questions on 233.2.c's substrate-symmetry surfacing inserted 233.2.d substrate-symmetry foundation before AST-derived population. | medium-per-substone (5 sub-stones; each atomic) | ACTIVE — a/b/c SHIPPED; 233.2.d sub-DESIGN landed |
