...
| 233.2 | **Provenance on Values** — see [sub-DESIGN](DESIGN-STONE-233.2.md). SLICED into 233.2.a (mint + transparency) → 233.2.b (`keyword/from-string` producer) → 233.2.c (4-producer sweep) → 233.2.d (uniform `list_span`; [sub-DESIGN](DESIGN-STONE-233.2.d.md)) → 233.2.f (apply Tracked-unwrap fix) → 233.2.g (Tracked-unwrap class re-evaluation [sub-DESIGN](DESIGN-STONE-233.2.g.md); picked Shape A) → 233.2.h-k (structural pivot: mint TrackedValue → flip eval signature → migrate producers → retire Value::Tracked) → **233.2.l** (#[wat_value] proc-macro structurally forbidding future wrapping variants per FAILURE-ENGINEERING.md ✅✅✅ standard) → 233.2.e (AST-derived for let-bindings + literals on TrackedValue substrate). Three resequencings post-compaction: 233.2.d substrate-symmetry; 233.2.g Shape A pivot; 233.2.l meta-class structural prevention. | DESIGN-then-execute (10 sub-stones; each atomic) | ACTIVE — a/b/c/d/f/h/i SHIPPED; 233.2.g sub-DESIGN landed; 233.2.j-l + 233.2.e pending |
