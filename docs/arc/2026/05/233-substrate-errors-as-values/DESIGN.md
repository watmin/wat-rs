...
| 233.2 | **Provenance on Values** — see [sub-DESIGN](DESIGN-STONE-233.2.md). Shape C (`Value::Tracked` wrapper variant + transparency contracts). SLICED into 233.2.a (mint + transparency) → 233.2.b (`keyword/from-string` producer tag) → 233.2.c (additional producers) → 233.2.d (AST-derived for let-bindings + literals). Four-questions revealed Option-A "AST-derived-only" failed Honest (doesn't close runtime-built case); corrected to full Value-level provenance via Tracked variant. | medium-per-substone (4 sub-stones; each atomic) | ACTIVE — 233.2.a BRIEF pending |
