...
| 233.2 | **Provenance on Values** — see [sub-DESIGN](DESIGN-STONE-233.2.md). SLICED into 233.2.a (mint + transparency) → 233.2.b (`keyword/from-string` producer) → 233.2.c (4-producer sweep) → 233.2.d (uniform `list_span`; [sub-DESIGN](DESIGN-STONE-233.2.d.md)) → 233.2.f (apply Tracked-unwrap fix) → 233.2.g (Tracked-unwrap class re-evaluation [sub-DESIGN](DESIGN-STONE-233.2.g.md); picked Shape A) → 233.2.h-k (structural pivot: mint TrackedValue → flip eval signature → migrate producers → retire Value::Tracked) → 233.2.e (AST-derived for let-bindings + literals on TrackedValue substrate). Two resequencings post-compaction: first inserted 233.2.d substrate-symmetry foundation; second pivoted from Shape C variant to Shape A struct-wrap after audit revealed structural trap-door class. | DESIGN-then-execute (9 sub-stones; each atomic) | ACTIVE — a/b/c/d/f SHIPPED; 233.2.g sub-DESIGN landed; 233.2.h-k pending |
