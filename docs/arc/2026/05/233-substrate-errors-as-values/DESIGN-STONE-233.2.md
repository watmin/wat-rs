...
pub enum Provenance {
    ...
    /// `RuntimeBuilt { producer: ":wat::core::keyword/from-name", call_span: <the call's span> }`.
...
| 233.2.b | **Tag at the keyword/from-string producer** — `eval_keyword_from_string` wraps return value in `Value::Tracked` with `Provenance::RuntimeBuilt { producer: ":wat::core::keyword/from-name", call_span }`. Probe asserts error message includes producer info when a runtime-built keyword reaches NotCallable. The minimum-viable producer tag. | small | ✓ SHIPPED at `9cc278c` |
...
| **233.2.k** | **Retire `Value::Tracked` variant** + `Value::inner()` helper. Final compile-clean confirms structural enforcement for the CURRENT instance. | medium | PENDING |
| **233.2.l** | **Mint `#[wat_value]` proc-macro** in wat-macros crate forbidding wrapping variants on Value enum (any variant with `Value`/`Box<Value>` as direct field; collection-content `Arc<Collection<Value>>` allowed). Apply to Value. Compile-fail probe via trybuild. **Failure-engineering ✅✅✅ standard** per `scratch/FAILURE-ENGINEERING.md` four-questions verdict 2026-05-23: probe-only fails Honest (catches AFTER construction); proc-macro passes all four YES (compile error AT construction). Meta-class structurally unrepresentable; same shape as ZERO-MUTEX's "never construct the situation that would need a Mutex." | medium (proc-macro) | PENDING — blocked on 233.2.k |
| **233.2.e** | **AST-derived provenance** — Literal + SymbolBound populated on TrackedValue substrate. Re-scoped from prior 233.2.d slot; ships AFTER structural refactor lands. | medium | PENDING — blocked on 233.2.l |

233.2 closes when 233.2.a-l + 233.2.e ship + the umbrella INSCRIPTION (which lands at 233.4 or whenever 233.3 completes).
...
**Third resequencing (2026-05-23 late late, post-233.2.i audit + FAILURE-ENGINEERING.md doctrine invocation):** the 7 arc216 stone1 hashset roundtrip probes failing post-Stone-233.2.i diagnosed as live instances of the SAME trap-door class — `hashset_length_inner` (src/runtime.rs:8460) pattern-matches `Value::wat__std__HashSet(s)` directly without `.inner()`. Empirically proven: adding `.inner()` flips probe FAIL → PASS in one line. User invoked `scratch/FAILURE-ENGINEERING.md` doctrine; four-questions verdict on prevention paths: probe-only fails Honest (catches AFTER construction = ✅✅); proc-macro passes all four YES (compile error AT construction = ✅✅✅). **Verdict: Stone 233.2.l added** — `#[wat_value]` proc-macro structurally forbids future wrapping variants on Value. Same shape as ZERO-MUTEX. Stone 233.2.l ships AFTER 233.2.k retirement so the proc-macro doesn't trigger on the current state; Stone 233.2.e shifts to block on 233.2.l (since 233.2.l completes the structural sealing).
...
**Producer scope for 233.2.b:** start with `:wat::core::keyword/from-name` alone (highest payoff; the case from arc 232.0)? Or tag all known producers in one sub-stone (233.2.b/c collapse into one)?
