...
H-1a    holon declares its REAL arity        ← NEXT: 35 verbs; the root cause under O-iv-c
H-1b    the same for atom.rs                  60 verbs, on a proven shape
Q       the value door carries a SPAN        ⚠ REAL, but SIZE is downstream of H — do not size it first
...
| **H-1a** | holon declares its REAL arity — `subspace` 10 · `engram` 10 · `reckoner` 8 · `hologram` 7 | 35 verbs; deletes code, adds none | `BRIEF-STONE-H-1a-holon-declares-its-real-arity.md` | — **READY** |
| **H-1b** | the same for `atom.rs` | 60 verbs | *not drawn* | H-1a proving the shape |
| **Q** | the value door carries the CALL SPAN — a span is not binding state | `ValueHandler` type · `dispatch_substrate_impl` + its 1 caller · the macro sniff · 19 twins take an ignored param | `BRIEF-STONE-Q-the-value-door-carries-the-call-span.md` | ⚠ argument stands; **SIZE is downstream of H** — the compiler sizes it after H lands |
