...
### 4.1 The `RETE_OPS` reachability ledger · **BUILT 2026-08-28, 55 of 74 rows measured**
...
#### The matrix — all 55 `Alias` + `Fallback` rows, both positions

| outcome | rows |
|---|---:|
| FIRES inline · FIRES fence | 16 |
| REFUSED inline · FIRES fence | 9 |
| **MATCHES-NOTHING inline** · FIRES fence | **27** |
| NOT-GENERABLE (holon: two `HolonAST` operands, no literal spelling) | 3 |

**Every one of the 52 measurable rows fires in a `where` fence. Inline, only 16 work.**
...
#### What remains — 19 rows unmeasured

| block | rows | why it is a separate strike |
|---|---:|---|
| `Form` | 9 | `params` is empty and `ret` carries no scheme — nothing here can synthesize a call; bespoke per row |
| `Redispatch` | 10 | same; needs collections |
...
**As of 2026-08-28: 4.1 is BUILT and running with 48 rows left to measure; it has already produced
a new item (2) and closed a defect nothing else could see. Below that, one decomposition ruling and
three builder rulings — none of which is work, all of which is a judgment call.**

1. **4.1 the reachability ledger** — BUILT 2026-08-28 and already paying: **55 of 74 rows
   measured**, one live defect found and fixed (a row that passed every static gate and then raised
   at RUNTIME), and TWO surface-wide findings — 9 rows refused inline, and **27 accepted inline
   that silently match nothing**. **19 rows remain** (`Form` 9, `Redispatch` 10), and they carry no
   `params` at all so nothing can synthesize them; that is a bespoke strike.
2. **The inline-constraint gap — NEW, and it wants to be its own item.** 9 of 25 measured rows are
   refused as an inline constraint while firing in a `where` fence. Arc 109's NOTE frames this as
   `keyword::=`'s type-mapping bug; the ledger shows it is every unary op and every `Type/method`
   spelling as well. Whether that position SHOULD admit them is a design question nobody has been
   asked; what is settled is that the current story ("a keyword type-name defect") is too small.
3. **`partire` x7** — needs an owner or an affirmative CUT. It has been tracked in no list at all;
   another silent pass is the one outcome that is not allowed.
4. **TRACKED DECISIONS ① and ②, and the `CLAUDE.md` delivery gap** — three builder rulings. All
...
5. **`circumspicere` 1 (grid SPEED half in CI)** — re-decide on the live constraint (runner noise),
