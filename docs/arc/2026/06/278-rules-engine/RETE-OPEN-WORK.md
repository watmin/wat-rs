...
### 4.1 A reachability ledger over `RETE_OPS`
Rows are gated for purity, totality, arity and type — but **never for "can a user actually get
here"**. `keyword::=` and `keyword::not=` passed every one of those gates since they were minted
and are unreachable three independent ways
(`docs/arc/2026/04/109-kill-std/NOTE-keyword-is-two-disjoint-type-names-and-rete-keyword-equality-is-dead-surface.md`).

A ledger — *for each `RETE_OPS` row, one exercising rule, or a written reason it cannot be
exercised* — would have caught that on the day the row landed, and is the durable cure rather than
fixing the one instance. **It also answers a question nobody has asked: is `keyword::=` the only
dead row?** A table that size having exactly one would be surprising.

The scalar fuzzer (`wat-tests/rete/differential-fuzz-scalars.wat`) is the natural host: it found
this gap by trying to add the sixth type and failing.
