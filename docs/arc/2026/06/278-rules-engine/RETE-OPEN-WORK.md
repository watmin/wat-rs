...
### 4.1 The `RETE_OPS` reachability ledger · **BUILT 2026-08-28, 26 of 74 rows measured**
...
#### What remains — 48 rows unmeasured

| block | rows | why it is a separate strike |
|---|---:|---|
| non-`bool` `Alias` | 9 | returns i64/f64/String, so it must be WRAPPED in a comparison to sit where a constraint goes |
| `Fallback` | 20 | same wrapping, plus the mandatory 4-arg `:undefined <value>` marker shape |
| `Form` | 9 | `params` is empty and `ret` carries no scheme — bespoke per row |
...
1. **4.1 the reachability ledger** — BUILT 2026-08-28 and already paying: 26 of 74 rows measured,
   one live defect found and fixed (a row that passed every static gate and then raised at
   RUNTIME), and one surface-wide gap surfaced. **48 rows remain**, in four blocks sized in 4.1.
