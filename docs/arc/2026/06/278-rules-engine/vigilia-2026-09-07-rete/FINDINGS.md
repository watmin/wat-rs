...
| **C1 ★★** | conferre | 1 | `stratify.wat:138-160` vs `stratify.rs:88-94,103-122`; claim at `stratify.rs:26-27` | ⭐ **L1 — AND IT FALSIFIES A CLAIM I WROTE TODAY.** The oracle's `rule-negates` recognises negation ONLY when `:not` is the top-level head; Rust's `negate_types` recurses through `And`/`Or` and finds a nested `:not`. On `(or A (and B (not C)))` the oracle contributes nothing and Rust pushes `C` → different stratum numbers. `stratify.rs:26-27`, dated **2026-09-07** by `66a24d288`, asserts every other listed fn *"DOES mirror"*. | **L1** | ✅ **RESOLVED 2026-09-09** (`a8d95ea82`) — **the oracle now recurses; the engine was untouched.** ⭐⭐ **THE PROBE RAN FIRST AND EARNED THE STRIKE.** The two corpus instances are `defquery`, and stratification orders producers — so whether the divergence was reachable for a RULE was unestablished, and a `defrule` probe of the exact shape `(or A (and B (not C)))` with `C` derived settled it: **native raised `l24::Out` to stratum 1, the oracle's map came back EMPTY.** Live, not latent. ⛔ **AND THE OBVIOUS CURE WAS BUILT, DRIVEN, AND REJECTED**: delegating `rule-negates`'s fold straight to the existing `negated-types-under` treated **any bare positive top-level fact read as a negation**, breaking the pre-existing ANCHOR fixture (`l23::ok`'s plain `A` read reported as *"negates A"*). Shipped instead: a new `rule-negates-in` mirroring `negate_types(form, out, under_not)` — `and`/`or` recurse unconditionally looking for a nested `not`, and only once a `not` is found does it delegate to the untouched `negated-types-under`. **Mutation-proven with isolation**: reverting the `and`/`or` branch to a hardcoded `false` reddened the NESTED test at exactly its assertion while ANCHOR stayed green. ⭐ And the durable half landed too — **the stratum differential is no longer only a two-vector scratch**: `native_stratify_numbers_nested_or_and_not_against_the_oracle` now gates the shape. Floor 5488→5489. | oracle `:150` tests `hd = ":wat::rete::not"` and returns `acc` otherwise; Rust `:108-112` recurses `And\|Or` passing `under_not`. Closed when the semantics is decided and one side fixed, or the claim is scoped |
...
| **X1 ★** | excusare | 1 | `alpha.rs:338` | Bare `#[allow(clippy::too_many_arguments)]`, **ILLEGITIMATE-AT-BIRTH**. The only comment above it addresses `#[cold]`/`#[inline(never)]`, not argument count — and being a COLD path, the hot-loop excuse its siblings use would not even apply. Least defensible of the six. | L1 | ✅ **RESOLVED 2026-09-08** — the one genuinely bare allow is **gone, not explained** (`87285b5db`). `activate_deferred_mixed_classes` went from **11 parameters to 3**, taking `&mut AlphaActivateCx<'_>` — ⭐ **a struct that ALREADY EXISTED** (`fire/delta.rs:23`): nine of its eleven params were already being bundled into it inside its own body and at three other call sites in the same file, so this function was the outlier hand-passing what its siblings passed as a unit. `#[cold]`/`#[inline(never)]`, so no hot-path signature moved. Clippy rc=0 at the now-allow-free site is the proof the restructure satisfies the lint rather than hiding from it. | line above is a `#[cold]` note, not a reason; contrast `hash_join.rs:39-41` which carries an on-point one |
| **X2** | excusare | 1 | `fire/mod.rs:469`, `:2128`, `filter_after_join.rs:17`, `round_census.rs:25` | Four more bare `#[allow(clippy::too_many_arguments)]` with no argument-count reason. Each has a doc comment above describing what the fn DOES — a comment's presence is not a reason's presence. `round_census.rs:25` is `#[cfg(test)]`-only, where the lint is arguably MORE valid. | L1 | ✅ **RESOLVED 2026-09-08** — ⛔ **and the population was 11, not the 4 this row named** (`87285b5db`); the rows cited where the ward noticed them. Final state of all eleven: **5 already carried an adequate argument-count reason** (`compiled_cond.rs:246`, `hash_join.rs:41`, `hash_join.rs:427` — the model, `join_after_filter.rs:31`, `validate/typing.rs:742`), **5 got one written**, **1 was restructured away** (see `X1`). ⚠ **The executor corrected my own draft table twice**: it omitted `hash_join.rs:503` entirely, and my combined row for `fire/mod.rs:469` + `typing.rs:742` resolved in **opposite** directions once read in full — one already had a reason, one did not. The row's load-bearing claim held exactly: **a comment's presence is not a reason's presence.** | read all four lines above: *"Drain the frontier…"*, *"Exists/Not Leaf: probe…"*, etc. None mentions arity |
| **X3 ★★** | excusare | 1 | `wat/rete/oracle/fire.wat:54` | ⭐⭐ **THREE WARDS, ONE RUNE, TWO VERDICTS.** `excusare` strikes `rune:intueri(naming)` as ILLEGITIMATE-AT-BIRTH: *"rename would fork every oracle fire caller"* is a cost-of-change plea, not a domain-truth, and names no target. But `intueri` graded the same rune **CLEAR**, and `struere` (T6) said **no action required**. Vigilia forbids me re-classifying a ward's verdict — so both stand, and the disagreement IS the finding. | L2 (excusare) vs CLEAR (intueri) | ✅ **RESOLVED 2026-09-08** — the false clause struck (`690c35522`). **The plea was not the wrong KIND of warrant; it was untrue**: `walk-filter-ids` has TWO call sites, both inside the file that defines it. The historical half stands. | the rune's text at `:54-56` is a historical explanation + a refactor-cost claim; whether that is a warrant is the open question |
...
| **2X2 ★★** | excusare | `clause.rs:72-76` | ⭐⭐ **TWO WARDS, ONE RUNE, TWO VERDICTS — the second such split of this cast.** `purgare` rowed this (2P2) as mis-categorised: `trait-contract` on a plain enum field no trait bound mandates. `excusare` — whose entire remit is weighing exemptions — returns **HOLDS**, and gives its ground explicitly: category fit is *"a check another ward can decide"* and therefore outside its remit; its own question is only whether the reason earns the `#[allow(dead_code)]`, and it independently confirmed the reason is TRUE (`acc_form` written once at `clause.rs:354`, never read as this field; fire reads `node_named_ast(node, "acc-form")` in `kernel/arm.rs`). **Both stand as returned.** `vigilia` forbids me re-classifying a child's verdict — so this is a decision for the builder. | 2P2 says mis-categorised · 2X1 says HOLDS | ✅ **RESOLVED 2026-09-08** — **`shape-contract` MINTED** (`690c35522`). All THREE `trait-contract` runes in the tree named no trait; `CONVENTIONS.md` had already diagnosed the gap in the rune's own words and said no category existed. Row added, category added to the gate's closed set, all three recategorised, **mutation-proven** (removing it reds the gate and names all three sites). | the reason's truth and the category's fit are **separable**, and the two wards each judged a different one. Closed when the builder rules whether a true reason under a wrong category is a defect |
...
| **2I1 ★★** | intueri | `vocabulary.rs:104-105`, `:1845`, `:1846`, `:1853` | ⭐⭐ **FOUR NUMBERS DESCRIBE ONE ARRAY, IN ONE FILE, AND ONLY ONE IS GATED.** `NAMING_RULE_EXCEPTIONS` is claimed as **nine** by the module header, **eleven** by the test's own doc comment, **fourteen** by that test's function NAME one line below it, and **14** by its assertion one line below that. The assertion is the only mechanically enforced one, and it is the true one. ⛔ **The doc comment disagrees with its own test's name TWO LINES AWAY.** | **L1** (ward's severity, passed through) | ✅ **RESOLVED 2026-09-08** (`c3c9caa54`) — prose only; **no new gate needed.** ⚠ **Partly refuted**: the row is right that four numbers describe one array, but the *asserted* one was **already correct (14) and already enforced**. Only the surrounding prose (9, 11) was stale. ⭐ **That is the row's own thesis proving itself** — the mechanically enforced number was the one that had not rotted. | `sed -n '104,105p'` → *"Nine rows total"*; `:1845` → *"exactly the eleven rows"*; `:1846` → `fn naming_rule_exceptions_are_exactly_the_documented_fourteen`; `:1853` → `assert_eq!(…len(), 14)`. Closed by deleting both prose numbers in favour of *"count enforced by the gate below"* |
| **2I2** | intueri | `export.rs:2274` | `import_export`'s doc says *"Its 194 lines are phase COUNT rather than depth."* The body spans `:2278-2535` — **258 lines**. The qualitative claim it supports (9 phases, brace nesting peaks at 3) is still true; only the number rotted, by 64. | **L1** (ward's severity, passed through) | ✅ **RESOLVED 2026-09-08** (`c3c9caa54`) — **CUT, not corrected.** *"Its 194 lines"* is gone (0 occurrences); the qualitative claim (9 phases, nesting peaks at 3) stands without it. Per the pinned rule: a line count moves on every edit and means nothing, so it is not worth gating — and a count merely corrected is a rot restarted. | body end located by brace-match at `:2535`; 2535−2277 = 258. Closed by dropping the number — the point stands without it |
...
| **2R1 ★★** | probare | `matcher.rs:114-131` | ⭐⭐ **"THREE INDEPENDENT SITES" — THERE ARE FOUR, AND THE FOURTH IS NAMED NOWHERE.** The doc justifies `enum_variant_ctor`'s `(enum, variant, arity)` return shape by *"the three callers"* and says so three times (`:115`, `:124`, `:127`). Actual callers: `purity.rs:976`, `expr_ir/mod.rs:1087`, `validate/mod.rs:1056`, **and `validate/typing.rs:335`** — the last resolving `Unit`/`Tagged` for a diagnostic classification, a fourth purpose the doc does not mention. ⚠ **The ward proposed a cause worth keeping**: this likely postdates `partire`'s 2026-08-30 split of `validate.rs` into `mod`/`typing`/`error` — the split created a caller in the new file that the doc in the *other* file never absorbed. | **L1** (ward's severity, passed through) | ✅ **RESOLVED 2026-09-08** (`c3c9caa54`) — **ASSERTED.** The row was right: `validate/typing.rs`'s `classify_keyword_constant` was missing from the count. New gate `enum_variant_ctor_still_has_exactly_the_four_documented_callers`, **mutation-proven**. ⛔ The return *shape* was left alone — the doc's justification was wrong; the design may still be right, and this strike fixes the claim, not the design. | `grep -rn 'enum_variant_ctor(' src/` → 4 callers of `matcher::enum_variant_ctor` (`check.rs`'s `literal_enum_variant_ctor` is a different, local fn — correctly excluded). `grep tests/lint/` → **no gate pins this count.** Closed by correcting the number and naming the fourth |
...
| id | ward | site | finding | sev | status | re-derivation |
|---|---|---|---|---|---|---|
| **3P1 ★★★** | peragrare | the grid's 5-axis grid; census at `wat-scripts/perf/grid/peragrare-census.sh` | ⭐⭐⭐ **THE CORPUS PROVES EVERY MECHANISM ALONE AND NO TWO TOGETHER.** All three founding defects that birthed this ward are now closed **as isolated axes** — `userfn-head`, `retract-multiplicity`, `accum-over-derived` each have a fixture and a mutation proof. But **no fixture combines any two of them.** 5 of 108 cells are empty *and* carry a live compound hypothesis: a user-fn head whose LHS accumulates over a type the same ruleset derives; a duplicate-retract feeding a **leading** accumulate; a leading accumulate whose `:from` is itself derived; a positive consumer downstream of a **leading** gate; a duplicate-retract of the accumulate's own source. **Each cure was proven only where the other pressure is absent.** | **L1** ×5 | ✅ **RESOLVED 2026-09-09** (`22d96739b` + `b4801eb5f`) — **all five compound cells built; all five AGREE.** Census **9→14 visited, 99→94 empty, 16→21 members**, total unchanged at 108. The corpus that proved every mechanism alone now proves five pairs together: `accum-lead-derived` (A×L), `userfn-accum-derived` (H×A), `retract-lead-accum` (R×L), `leading-neg-consumer` (N×L), `retract-accum-derived` (R×A). **Each captured from Clara 0.24.0, wat-oracle and wat-native independently — byte-for-byte identical, not just a matching summary line.** The cures compose. ⭐⭐ **AND ADDING A SECOND MEMBER TO A ONE-MEMBER POPULATION EXPOSED A LATENT BUG IN THE INSTRUMENT ITSELF**: the census's `verify_fixture` hardcoded `userfn-head`'s own literal `:then` snippet for **any** `H=userfn` row, and would have silently misfired on the second such fixture. Generalised to a per-name lookup. **That is `peragrare`'s thesis proving itself — a corpus with one member cannot discriminate, and neither can a checker written against one.** ⚠ Scoped honestly: the first cell was built alone to measure cost, and the executor's judgement that the remaining four were *cheap but not derisked* (each crossing a different axis pair) was taken rather than overridden. **Four independent experiments, four agreements.** ⛔ My own design was corrected en route: I wrote *"discovered by walk — no runner registration needed"*, true of the differential's glob and **false** of two Rust gates asserting exact set equality (`SIZED_AXES`, `CORRECTNESS_SIZES`). Registration is **five** sites; an unregistered fixture reddens the floor. | `bash peragrare-census.sh` → 108 cells, 9 visited, 99 empty, 16 members; `--pins` → all three PASS, moving pin **8 of 8** with the spell's own denominator `(2-1)+(2-1)+(3-1)+(3-1)+(3-1)`; `--verify` → *"all 16 fixtures' mechanical facts agree with the table"*. Closed cell by cell, each with a fixture |
...
| **3Q2 ★★** | ⛔ **orchestrator, found while verifying 3Q1** | same three files: `check-where-shapes.sh:152` vs `:158`; `check-query-compat.sh:135` vs `:141`; `check-spec-native.sh:87` vs `:93` | ⭐⭐ **THE OUTER NON-VACUITY IS GATED AND THE INNER ONE IS NOT — IN THREE SCRIPTS, WHILE A FOURTH IN THE SAME DIRECTORY NAMES THE PRINCIPLE.** All three guard `if [ "$PAIRS" -eq 0 ]` — *did we discover any stems?* **None guards `ROWS_TOTAL`.** So `PAIRS=5, ROWS_TOTAL=0` **passes green**, printing `"5 pair(s), **0 rows** — wat == Clara on every shape"`. Five pairs each comparing nothing, and the success claim is vacuously true with the zero printed in the message. ⛔ **`check-grid-speed.sh:85-90` guards exactly this class by name** — *"only $seen verdict(s) … **A short sweep is a gate that cannot fail, not a green one**"*, `exit 2`. The principle is written down, in this directory, by this author, and three sibling gates do not apply it to the number they themselves print as coverage. | **L1** | ⚠ **REFUTED 2026-09-08, guard added anyway** (`96d409985`). ⛔ **This row was MINE and the vacuity it describes CANNOT OCCUR.** All three scripts already carry a per-pair `wn -lt 1` check that `return 1`s on a zero-row pair — verified present at `HEAD~3:115`, long predating the strike — so a pair comparing nothing FAILS and never reaches `ROWS_TOTAL`. The `PAIRS=5, ROWS_TOTAL=0` example does not reproduce. ⭐ **The guard was added regardless, as belt-and-suspenders, WITH the refutation written into the code** so nobody re-derives a bug that isn't there: *"check_pair()'s own `wn -lt 1` check already makes this unreachable today … so a future change to check_pair() can't reopen the gap without this also going red."* **An honest guard beats a silent one.** | `grep -n 'ROWS_TOTAL\|PAIRS'` on each of the three → `PAIRS -eq 0` guard present, no `ROWS_TOTAL` guard anywhere; `sed -n '85,90p' check-grid-speed.sh` → the named counter-example. Closed by a `ROWS_TOTAL -eq 0` floor in each, mirroring the sibling |
...
| **3W1 ★★★** | circumspicere | `check-grid-speed.sh:5-12,42-46` · `GRID-native-vs-clara-2026-08-27T07-15-56Z.txt` · `ci.yml:175-178,224-227` | ⭐⭐⭐ **THE CI SPEED FLOORS WERE MEASURED ON A JDK NOBODY RECORDED, AND CI NOW PINS A JDK THAT WAS NEVER CHECKED AGAINST THEM.** The `FLOOR` array is derived, by its own comment, *"from the recorded 33-cell grid"* — a file which, like **all 29 recorded grids**, names **zero** JDK/Temurin/Clojure version (`grep -il 'jdk\|temurin\|openjdk\|java '` over all 29 → **0**). It is a manual local capture (`bbeb1997d`), taken through the undocumented `PATH → JAVA_HOME → $HOME/opt/jdk-*` discovery. CI pins `distribution: temurin`, `java-version: "21"`. **Nothing has ever re-derived the floors under the JDK CI actually runs.** ⭐⭐ **AND THE SHAPE IS SHARPER THAN THE WARD PUT IT — THIS COMMENT WAS WRITTEN BY A PRIOR `circumspicere` CAST.** `:5` opens *"`circumspicere` found the speed half runs in no CI job"*, and `:7` records that the old excuse *"EXPIRED on 2026-08-27 when the `parity` job added Temurin 21."* **The Temurin pin and the floor-defining grid date to the same window, and the cure that predecessor prompted is what opened this gap.** `[[a-cure-can-carry-the-defect-one-level-down]]`. | **L1** | ✅ **RESOLVED 2026-09-09** (`35f1f4e8b`) — **the grid now records its instrument, and the FLOOR comment states what is and is not known.** ⛔ **The row's second half was REFRAMED, not accepted.** It said CI *"pins a JDK that was never checked against them"* — **not established.** This box runs `Temurin-21.0.12+8` via `JAVA_HOME=/home/john/opt/jdk-21`, the same distribution and major version CI pins, through exactly the `$HOME/opt/jdk-*` path the row calls undocumented. **The floors were plausibly measured under the very JDK CI uses. The defect is that nobody can tell — not that the numbers are wrong.** `run-all.sh` now emits a `#grid/Capture` EDN header carrying `java -version`, the Clojure CLI version, the Clara version, host and timestamp, so **every future capture is self-describing**. ⭐ The Clara pin is **read from `run-axis.sh`'s own `CLARA_DEP` rather than a second hardcoded copy** — the executor applying this vigilia's duplication lesson unprompted. Both consumers (`check-grid-speed.sh`'s `case` filter, `compare-grids.sh`'s awk) were proven inert by reading **and** driving; an EDN-escaping bug caught in the first draft (an unescaped `"` from `java -version`'s own output truncating the string). The FLOOR comment is now three explicit parts — **KNOWN** (29/29 grids name no JDK, independently re-checked), **EVIDENCE NOT PROOF** (this box runs Temurin 21.0.12 *today*; the capture was 2026-08-27), **WHAT SUPERSEDES** (the first grid recorded under the new header). It never claims the floors were measured on Temurin 21. ⛔ **The floor numbers were NOT re-derived**, per the pin: that is a performance act needing six samples and its own scorecard, and doing it here would produce numbers with recorded provenance while the mechanism that loses provenance stayed unfixed. ⭐ **And no 30th `GRID-*.txt` was committed** — a demonstration capture was generated, judged to be `4G1`'s shape (a golden nothing regenerates), and left in the session scratchpad. Still 29. | `sed -n '5,14p' check-grid-speed.sh` → the citation and the EXPIRED note; `grep -il` over all 29 grids → **0** name a JDK; `grep -A3 setup-java ci.yml` → temurin 21, twice, **and no `cache:` line**. Closed by adding a JDK line to the provenance header every capture already carries (*"Box: … Code: …"*) and re-deriving the floors once under Temurin 21 |
...
| **4N1 ★★** | cernere | `probe_constructor_meta_surface_total_enum.wat.bad:37-38` ⚠ **(ward cited `:542-543`; the file is 40 lines — see below)** | ⭐⭐ **TWO RETIRED FORMS IN A MUST-FAIL FIXTURE THAT NO GATE PARSES.** `:37` calls `(:wat::rete::fire-rules-spec session)` and `:38` calls `(:wat::rete::query-by-type-string fired "cg::Wrap")`. **Both are retired, not merely undocumented:** `fire-rules-spec` is the explicit LHS of a rename in `wat-scripts/fixes/rete-oracle-sigil.wat:4` → `fire-rules$oracle`, whose own body calls it *"retired by the very rewrite recorded below"*; `query-by-type-string` is the target of the `type-query-to-defquery.wat` codemod. **Neither appears as a dispatch arm anywhere in `src/`** — they are dead, not aliased. ⭐ **And the corpus-wide sweep found exactly these two** — no other `.wat`/`.wat.bad` in 163 files uses either spelling. ⚠ The forms sit in `:user::run` (`:32`), which the governing test never calls, so they are **inert rather than proven** — dead code inside a fixture that no load gate reads. | L2 | **OPEN** · ✅ I VERIFIED, coordinates corrected | `grep -n 'fire-rules-spec\|query-by-type-string'` on the file → `:37`, `:38`; corpus-wide → **2 hits, both here**; `grep -rc` over `src/` → **0** dispatch arms; `sed -n '4p;45p' rete-oracle-sigil.wat` → the rename and the retirement rune |
| **4E1** | exigere | `src/rete/kernel/tests/termination_verdict.rs:5` | **A SCOPE-DEFENSE THAT NAMES NO TRACKER, IN A 221-LINE FILE THAT CONTAINS NONE.** *"surfacing it would need a new `(:wat::rete::CompileOutcome)` variant behind the outcome wall, which is **affirmatively out of scope for the strike that split this type**."* ⛔ **"The strike that split this type" names no artifact a reader can locate** — I grepped the whole file for `arc`, `DESIGN` and `rune:` and got **zero**. ⭐ The ward's own direction is the right one and worth keeping: the reasoning it gives reads as a **permanent architectural invariant**, not deferred work — so the cure is to *drop the deferral framing and state the invariant*, not to open an arc. | **L1** | ✅ **RESOLVED 2026-09-08** — deferral framing dropped, invariant stated (`a8c233eb3`). `exigere`'s own direction was the right one: the reasoning read as a permanent architectural bound, not deferred work, so no arc was opened and none was needed. | `sed -n 5p` → the phrase verbatim; `grep -c 'arc [0-9]\|DESIGN\|rune:'` over the file → **0** |
| **4E2** | exigere | `tests/rete/probe_arc278_P4c_native_retraction.rs:8-9` | **A HYPOTHETICAL CAPABILITY LABELLED "A DEFERRED SURFACE" WITH NOTHING TRACKING IT.** *"the support store only buys O(delta) retract for a PERSISTENT cross-fire streaming engine, **a deferred surface**"*. ⚠ **The ward argued its own finding down to its narrowest honest form and I keep that**: the primary claim (*"there is NO … cascade to build"*) is honest present-tense fact; **only the parenthetical is the defect**. No tracker named in the 62-line file. Cure is a reword, not an arc. | **L1** | ✅ **RESOLVED 2026-09-08** — the parenthetical cured, the sentence left standing (`a8c233eb3`). The row's own narrowing held on re-derivation: the primary claim (*"there is NO cascade to build"*) is honest present-tense fact and only *"a deferred surface"* was the defect. | `sed -n '8,9p'` → the phrase verbatim; file is 62 lines; `grep 'arc\|rune'` → only the title's own "Arc 278" |

| **4S1 ★★** | solvere | `mod.rs:141,212,417-418,603-604,618,671,673` + 16 further files | ⭐⭐ **THE FIRE-DRIVER WRAPPER IS HAND-TYPED ACROSS THE WHOLE TARGET AND NO SHARED BUILDER EXISTS.** Every cost/census helper embeds the same wat-source literal — `CompileOutcome::MayNotTerminate` + `FireOutcome::MemoryCeilingExceeded` + `FireOutcome::RoundCapExceeded` match arms with fixed `assertion-failed!` text. ⭐ **The ward proved the text never varies**: `sort -u` over the message strings across the target **collapses to one line per message.** Six copies in `mod.rs` alone. **I re-derived the arm occurrences independently: `MayNotTerminate` 80, `MemoryCeilingExceeded` 43, `RoundCapExceeded` 43 — 166 arm occurrences across 16 files.** ⛔ **And it is an ABSENT abstraction, not a bypassed one** — the ward checked for `wrap_fire`/`wrap_compile`/any fire-driver helper anywhere in `src/rete/` and found none, which is what puts it in `solvere`'s territory rather than `vocare`'s. A change to `FireOutcome`'s variants needs synchronized hand-edits with no compiler check. | L2 | **OPEN** · ✅ I VERIFIED (counts re-derived, higher than reported) | `grep -rc` per arm over both trees → 80 / 43 / 43. ⚠ The ward's unit was *wrapper instances* (75/22 files), mine is *arm occurrences* — different units, same conclusion. Closed by one `drive_fired(seed)` builder in `mod.rs` |
| **4S2 ★★★** | solvere | `mod.rs:474-478` (the comment) + `:546` (the canonical formula) vs `accum_cost.rs:698`, `fanout_cost.rs:425,558,828`, `cascade_cost.rs:393`, `strat_cost.rs` | ⭐⭐⭐ **THE CODEBASE WROTE DOWN WHY THIS ABSTRACTION MUST EXIST, AND THEN REINTRODUCED THE DUPLICATION TWENTY TIMES.** `render_phase_table`'s doc comment says it verbatim: *"Extracted 2026-08-01… Copying it would have put the instrument-subtraction arithmetic in two places, and the whole reason that arithmetic exists is that a table which misreports its own instrument is worse than no table — **two copies is how one of them silently stops subtracting**."* The canonical `net_of` lives at `:546`. **The same `raw − pairs × cal` subtraction is hand-rolled at 20 further sites** — I widened the ward's pattern and measured **20**, against its 17. ⛔ **The failure the comment names is not hypothetical; it is the present shape.** ⚠ **And two of the three files that DO call the abstraction — `accum_cost.rs` and `fanout_cost.rs` — reintroduce the duplication in their other tests in the same file.** | L2 | ✅ **RESOLVED 2026-09-08** — `net_ns()` extracted; 13 sites became 19 call sites of one `fn` (`4d96c7534`). ⛔ **The row was wrong twice**: `net_of` was a CLOSURE nothing could call (so nothing was bypassing anything), and the count was 13, not 20. Safety proven by a **noise-floor control** — before-vs-before on unmodified code diffs 23 lines; before-vs-after diffs 23, same kind. A 14th sibling in `rank_and_instrument.rs` was found and **reported, not silently converted**. | `sed -n '474,478p' mod.rs` → the comment verbatim; `sed -n` on `fanout_cost.rs:425,558,828` and `cascade_cost.rs:393` → four distinct hand-rolled subtractions read individually; widened pattern → **20** sites. Closed by one `instrument_net(raw, pairs, cal)`; the bespoke table SHAPES are legitimate composition and should stay |


| **4T1 ★★** | struere | `src/rete/kernel/tests/node_share_cost.rs:829` (call sites `:876`, `:910`) vs `where_tree_branch_differential.rs:78-86` | ⭐⭐ **THE NAMED TYPE EXISTED THE DAY BEFORE, IN THE ONE FILE THE AUTHOR SAYS THEY CHECKED — AND THE NEW FUNCTION WAS WRITTEN POSITIONAL ANYWAY.** `node_share_filter_counts(n, m) -> (u64, u64, u64)` returns the three census keys `filter:test-evals` / `filter:test-reuse` / `filter:test-pass` as an unlabeled triple of **identical type**, so any transposition compiles silently; both call sites destructure `let (evals, reuse, passes) = …`, correct today only by convention. Two files over, `struct Dispatch { evals, reuse, pass }` holds **the same three keys**, each field doc'd to the key it counts, and both are extracted by the same `get(k)`-over-census-rows closure. ⭐ **I corrected the ward's evidence UPWARD.** It said the two files were *edited* in one commit; `git show d878408f7 -- node_share_cost.rs` carries **`+fn node_share_filter_counts(…) -> (u64, u64, u64)`** — the function was **BORN** positional in that commit, whose own message reads *"The rider also checked the one other reader (**where_tree_branch_differential**)."* And `Dispatch` is not same-day: `5f0b2f1b1` created it **2026-09-04**, one day before. | **L1** | **OPEN** · ✅ I VERIFIED, evidence strengthened | `sed -n '829p'` + `grep -n node_share_filter_counts` → `:829`, `:876`, `:910`; `sed -n '74,90p' where_tree_branch_differential.rs` → the struct and its three doc'd fields; `git show d878408f7` → the `+` line; `git log --date=short` → 09-04 vs 09-05. Closed by returning `Dispatch` from both, or lifting it where both modules can `use` it |
| **4T2** | struere | `probe_arc278_enum_variant_typo.rs:19` + 8 sibling definitions; 55 call sites | **A HELPER THAT UNPACKS AN ALREADY-NAMED TYPE INTO A POSITIONAL ONE, NINE TIMES OVER.** `fn run(rel: &str) -> (bool, String, String)` takes a `std::process::Output` — which **already carries named `status` / `stdout` / `stderr`** — and actively discards those names for an unlabeled triple whose last two members share a type. Defined independently in **9 files** (`D6:51`, `D10:52`, `D11:57`, `enum_variant_typo:19`, `field_span:45`, `fixpoint_round_cap:45`, `import_accounting:46`, `match_arm_is_not_a_call:50`, `nested_wall:49`), plus four same-shape variants with extra params in the two grid files and one narrower `(bool, String)`. ⭐ **The ward re-derived the swap risk rather than asserting it, and I re-derived it again**: all **55** destructuring sites tally to `(ok, out, err)`×35, `(ok, stdout, stderr)`×7 and 13 suffixed variants — **zero swaps**. Latent, not manifest, and L2 is the correct call. ⚠ **Distinct from `4S1`**, which is the absent in-process *fire-driver* builder; this is the subprocess-spawn family and the defect is the return type, not the missing abstraction. | L2 | **OPEN** · ✅ I VERIFIED | `sed -n '19,31p' probe_arc278_enum_variant_typo.rs` → the unpack verbatim; `grep -rn 'fn run.*-> (bool, String, String)'` → 13 sites / 11 files; `grep -rhoE 'let \([a-z_]+, *[a-z_]+, *[a-z_]+\) *= *run'` → the 55-site tally above |
| **4T3 ★★★** | ⛔ **ORCHESTRATOR — found by CHECKING the ward's claim, not by the ward** | `probe_arc278_import_accounting.rs:46` + `probe_arc278_fixpoint_round_cap.rs:45` vs the other **7** copies of `fn run` | ⭐⭐⭐ **NINE HELPERS SHARE A NAME, A SIGNATURE AND A CALLING CONVENTION — AND TWO OF THEM SILENTLY DIFFER IN WORKING-DIRECTORY SEMANTICS.** `struere` reported the nine bodies as **byte-identical**. I md5'd the 15-line block at each definition and they fall into **two** groups, 7 + 2, and `diff` shows the split is not cosmetic: the **7** pass a RELATIVE path with **`.current_dir(manifest)`**; the **2** pass `manifest.join(rel)` as an ABSOLUTE path and call **`current_dir` not at all**. A caller writing `run("…")` in any of the nine cannot tell which behaviour they get, and whether it matters depends on how the spawned `wat` binary resolves anything relative to cwd. ⛔ **This is `4S2`'s own comment — *"two copies is how one of them silently stops subtracting"* — realized in a SECOND family**, except the divergence is not arithmetic but the process environment, and it is already present rather than merely possible. ⭐ **The finding exists only because the ward stated byte-identity as settled and I checked it** (`[[a-briefs-read-list-vouches-for-what-it-points-at]]`, inverted: a ward's own settled claim is a claim too). | **L1** | **OPEN** · ✅ I VERIFIED | per-definition `md5sum` of `sed -n "n,n+14p"` → **two** digests, 7×`57256302` + 2×`1269b85f`; `diff` of the two representatives → `.arg(rel)`+`.current_dir(manifest)` vs `.arg(&path)` with `path = …join(rel)` and no `current_dir` |

| **4Q1 ★★** | sequi | `tests/rete/probe_arc278_import_accounting.rs:177-215` (steps at `:191-193`, `:201`, `:204`, `:206`, `:207-212`); ambient cells at `src/alloc_counter.rs:91,168,192` | ⭐⭐ **A FIVE-STEP CHAIN THAT COORDINATES ENTIRELY THROUGH THREE THREAD-LOCALS, AND NOTHING IN THE TARGET ACKNOWLEDGES IT.** `an_origin_already_filed_is_never_re_based` reads `thread_bytes()`, writes `mark_session_origin_at(key, 0)` then `(key, late)`, reads `session_bytes(key)`, and asserts on the result — every step reached by a free function whose signature carries **none** of that state. ⛔ **`pub fn mark_session_origin_at(key, origin)` (`:247`) returns nothing**: the mutation is invisible in the type. This is the spell's own canonical *"hidden state via global counter"* example in shape. ⚠ **The author saw half of it** — `:178-180` defends against KEY collision (*"a key no `PMap` will ever mint… colliding with a live session would make the reading below measure that session instead"*) — but that is an untyped invariant argued in prose, and it says nothing about `thread_bytes()` being thread-wide state any other code on this OS thread perturbs. **Zero `rune:sequi` exist anywhere in the 306-file target.** ⭐ Evidence the ward had in reach and did not use: `alloc_counter.rs:157-159` documents the coupling itself — *"This map is NOT read from inside the allocator — only `THREAD_LIVE` is."* ⚠ **Its recommendation (rune it `ambient-context`) is NOT adopted here** — see the doctrine split in the orchestrator section: the spell forbids a rune for hidden domain state and this repo's vocabulary mints one. | **L1** | ✅ **RESOLVED 2026-09-08** — `rune:sequi(ambient-context)` added, naming the invariant the site's own comment argued informally (`a8c233eb3`). ⛔ **And the executor refuted my design**: I claimed `no_unknown_sequi_rune.rs` gates this rune's category. **It does not** — that gate walks `Path::new("src")` only (`:84`) and this site is under `tests/rete/`, so the rune is ungated. Rowed separately as `4Z1`. | `grep -n` → fn at `:177`; `sed -n '178,181p;191,193p;201p;204p;206,208p'` → each step verbatim; `grep -n` on `alloc_counter.rs` → `THREAD_LIVE:91` (read `:115`), `SESSION_ORIGINS:168`, `LAST_ORIGIN:192`; signatures at `:114`, `:247`, `:272` |
| **4Q2 ★★★** | sequi | `src/rete/kernel/tests/arm_lease.rs` — five chains at `:12`, `:99`, `:137`, `:167`, `:259`; example `:110-116`. Definition + rune at `src/rete/kernel/arm.rs:727-728`; the category's own table at `docs/CONVENTIONS.md:1054-1070` | ⭐⭐⭐ **THE REPO'S CONVENTION TABLE NAMES `ARM_BUILDS` AS ITS EXAMPLE OF A COUNTER WHOSE ARMING *CANNOT CHANGE A RESULT* — AND IN FIVE TESTS IT IS THE ONLY THING THAT DECIDES THE RESULT.** Each chain snapshots the global, calls `fire_fixpoint_delta` (whose `Result<Value, EvalBreak>` signature reveals **nothing** about whether an arm rebuilt), snapshots again, and asserts on the delta: `assert_eq!(ARM_BUILDS.load(…), builds, "fire HIT must not rebuild")`. ⛔ **If the counter stopped counting, all five assertions pass vacuously — `0 == 0`.** The `performance-counter` row of the closed set reads *"instrumentation, off by default… **nothing about the answer — arming it cannot change a result, only a measurement**"* and cites **`ARM_BUILDS`** by name. **This is `Q2`'s shape recurring one line down the same file** — a rune whose own adjacent definition contradicts its use. ⚠ **DISTINCT from `4D1` on a stated axis**: 4D1 is the fork-dependent *race*; this holds single-threaded and single-process, and is about the chain being **un-reorderable** — a maintainer inserting a step between two loads gets no compiler help and the test still compiles. ⭐ Also answers Question 1: the missing measurement citation at `:727` (its two `census.rs` siblings both cite one, *"~75-80ns measured 2026-08-01 against a no-sub-marks control"* and *"~1-2ns vs timer tax"*) is the lesser half; the category being wrong for this use is the greater. **Cure the ward proposes: return the rebuilt-or-not bool from the call, so the tests assert on a value the call hands back.** | **L1** | ✅ **RESOLVED 2026-09-08** — closed as a consequence of `4D1`, not a second edit (`4495aafcd`). Once thread-owned the honest category is `ambient-context`, which its neighbour `ARM_TABLE` already carried for the same reason. ⭐ And `CONVENTIONS.md` cited `ARM_BUILDS` as its `performance-counter` exemplar — **struck and kept as a worked counter-example** (`ff729e3a7`), because that citation is how the mislabel survived. | five chains counted directly — `ARM_BUILDS` reads in 5 of the file's 9 tests, at `:17` / `:110,113,124` / `:151,157` / `:181,184,192` / `:263,284`; `sed -n '110,117p'` → the block verbatim; `sed -n '1054,1070p' CONVENTIONS.md` → the table row naming `ARM_BUILDS` |

| **4M1 ★★** | temperare | `src/rete/kernel/tests/accum_cost.rs:320` (fn `:277`) and `fanout_cost.rs:526` (fn `:472`); tempered siblings at `strat_cost.rs:47`, `cascade_cost.rs:224` | ⭐⭐ **THE WORLD-FREEZE IS ON THE WRONG SIDE OF THE CLOSURE, AND THE SAME FILE FAMILY ALREADY CONTAINS THE TEMPERED FORM.** Both tests build a variant-dependent `world_src`, freeze a world **inside** the `shot(with_query)` closure, then call it from `for _ in 0..RUNS { shot(false); shot(true); }` — **6 freezes where 2 suffice**, on every release floor run (`const RUNS: usize = 3`; neither test is `#[ignore]`d). ⭐ **The structural contrast is exact:** `strat_cost.rs` and `cascade_cost.rs` define the *same* `let shot = \|with_query: bool\| -> Shot` closure over the *same* `RUNS` loop with the freeze **outside** it (`:47`→`:50`→`:99` and `:224`→`:227`→`:282`). ⭐⭐ **AND THE WARD RESOLVED MY LEAD RATHER THAN INHERITING IT** — I handed over 6 indent-8+ candidates as a QUESTION, explicitly saying the heuristic cannot tell a loop body from a match arm; it read all six and returned **2 real, 4 not**, each with the reason. ⭐ **It also volunteered the distinction the scope correction was built around**: `Instant::now()` sits at `:339`/`:543`, **after** the freeze — so this is hot but does **NOT** contaminate the benchmark's own number, which is a weaker and more honest claim than the one available to it. ⚠ **My own correction: the siblings are not an exact precedent** — they freeze a bare constant (`WORLD`, `ACCUM_AXIS_WORLD`), while these two build `world_src` by `format!` from `with_query`. The cure is **two** hoisted freezes, not one — which is exactly the arithmetic the ward gave. | L2 | **OPEN** · ✅ I VERIFIED (four files read; contrast tabulated) | `sed -n '314p;320p;339p;384p' accum_cost.rs` → closure, freeze, timer, loop **in that order**; `sed -n '47,51p;99p' strat_cost.rs` and `'224,228p;282p' cascade_cost.rs` → freeze **before** closure; `grep -n 'const RUNS'` → `:282`, `:477` |
| **4F1** | conformare | `tests/rete/probe_arc278_8i_accumulator_folds.rs:26,32,38,44,50,56,63,70,77,83` vs 460 sibling sites | **A 460-TO-10 CONVENTION WITH NOTHING HOLDING IT UP, AND ALL TEN EXCEPTIONS IN ONE FILE.** `call_beside_value(…) -> Result<Value, RuntimeError>` is called ~470 times across `tests/`; **460 use `.expect("<context>")` and 10 use a bare `.unwrap()`** — every one of the ten in this single file, while its siblings (e.g. `probe_arc278_5b_collect_rules.rs:16,22,28,34,36`) use `.expect("eval")` on the identical call. ⛔ **Nothing in the signature or in `RuntimeError` distinguishes the ten from the 460**: the discipline is hand-held, and a new author copying *this* file rather than any of the other 470 silently drops the context string with no objection from the type system. ⚠ **The ward graded it L1 while arguing the consequence is minor** — `#[track_caller]` still lands the reader on the exact line, so only the human-authored *"what was I testing"* phrase is lost. **Both halves are defensible and they are not the same axis; do not read this L1 as equal in weight to `4Q2` or `4P1`.** ⭐ Cure it proposes is the right shape for a test corpus with no enum to retrofit: **a lint mirroring `no_loose_string_assert.rs`, turning "uncompilable" into "un-green".** | **L1** | ✅ **RESOLVED 2026-09-08** — the ten bare `.unwrap()` now carry context strings matching their siblings (`a8c233eb3`). ⚠ **Sibling count corrected: 462, not the 460 I handed down** — immaterial to the cure, since the ten are confirmed globally unique to one file. ⛔ **`conformare`'s proposed lint was deliberately NOT minted**: a gate for a ten-site convention in one file is heavier than the defect. | `grep -rn 'call_beside_value(.*)\.expect('` → **460**; `…\.unwrap()` → **10**, all ten in the one file at the ten lines cited |
| **4F2 ★★** | conformare | `tests/lint/no_loose_string_assert.rs:11-15` (the licence) and `:82-87` (the stale count) — ⚠ **scope argued below** | ⭐⭐ **A GATE WHOSE HEADER TELLS EVERY FUTURE READER TO EXPECT A RED, IN A REPO WHOSE DOCTRINE SAYS A RED IS A RED.** Verbatim at `:14-15`: *"Until then this is an **expected-red test**; nextest isolates it, so a **SECOND red is a real regression**."* `wat-rs/CLAUDE.md` strikes exactly this shape by name — *"⛔ THERE IS NO SUCH THING AS A KNOWN FLAKE. A RED IS A RED"* — annihilating four pre-blessed tests and the two-strikes framing on 2026-08-05 at the builder's direction. **The gate carries no `#[ignore]`** (its one `#[test]` is at `:72`), so the licence is not even structurally inert. ⭐⭐ **THE WARD SETTLED WHICH OF MY TWO HYPOTHESES HOLDS, BY READING, WITHOUT RUNNING THE FLOOR** — it re-implemented the gate's own statement-scoped state machine in a read-only port, measured **0 violations over 1028 files**, cross-checked against a raw grep of 17 in-target candidates read individually, and anchored on the 52-rune count matching exactly. **Verdict (a): the campaign reached zero, the gate is green, and the licence is STALE PROSE — inert today and hazardous tomorrow**, because a single future violation meets a header inviting the dismissal the doctrine forbids. ⚠ **It stated plainly that only `scripts/floor.sh` makes this certain and that it did not run one.** ⭐ **Folded in, same header, same disease:** `:82-87` claims the walk's count *"comes from a gate, never from prose — so it catches a walk gone blind **without rotting as the tree grows**"* — and writes **998** beside it. The guard is `files.len() > 500`, a threshold, so the mechanism genuinely does not rot; **the illustration did — 998 → 1028 in six days.** ⚠ **SCOPE:** `tests/lint/` is not in target 4's paths. Rowed on `4P1`'s precedent (an instrument that gates the target's corpus) **plus** the stronger ground that this is a collision with the repo's own governing doctrine, not a code defect. **If the builder rules `tests/lint/` out of scope, this row moves; it does not dissolve.** | **L1** | **OPEN** · ✅ I VERIFIED | `sed -n '11,15p'` → the licence verbatim; `grep -n '#\[ignore\]'` → **nothing**; `sed -n '80,92p'` → `998` in prose beside `files.len() > 500`; `find src tests crates -name '*.rs'` → **1028**; 17 raw candidates re-checked by hand |

| **4L4 ★★★** | ⛔ **ORCHESTRATOR — a ward's STRUCTURAL PROOF, inverted** (complectens' prefix; found while measuring `circumspicere`'s brief) | `Cargo.toml:198-200`; `tests/rete/mod.rs` (3 lines); `build.rs:86-89` | ⭐⭐⭐ **THE COMPILATION MODEL OF THIS TARGET WAS MIS-STATED FOR THE WHOLE CAST, AND A "STRUCTURAL PROOF" RESTS ON THE EXACT INVERSION OF THE TRUTH.** I handed `complectens` the 257-file `probe_arc278_` prefix family as a QUESTION. It returned a **structural proof of the negative**: *"each file is a separate cargo integration-test binary, so cross-file code dependency is **impossible** under Rust's compilation model."* ⛔ **The disk says otherwise.** `Cargo.toml:198-200` declares **ONE** target — `[[test]] name = "rete", path = "tests/rete/mod.rs"`; that `mod.rs` is **three lines**, calling itself *"a thin include! stub"*; and `build.rs:86-89` emits `#[allow(non_snake_case)] #[path = "<abs>"] mod <stem>;` for **every** sibling `.rs`. **The 100 `.rs` files are MODULES IN ONE BINARY. Cross-file dependency is a `super::<stem>::` away.** ⭐ **The CONCLUSION survives; the PROOF does not** — I grepped `use super::`/`use crate::` across `tests/rete/` and got **0**, so the family really is independent **today**. But it is independent **by convention, contingently** — not **by impossibility, structurally.** That distinction is the entire finding: one is a closed question, the other is an open one that nothing guards. ⛔⛔ **AND IT CONTAMINATED A LATER CAST**: I handed `solvere` *"that surface is shut"* on this premise, narrowing its duplication search over the largest file family in the target. **`solvere` should be re-asked whether 257 files sharing one binary share anything.** ⚠ Note what made this invisible to sixteen wards: **every inward brief confined the ward to the 306 target files, and the answer is in `Cargo.toml` and `build.rs`.** A scope that protects a cast from noise also blinds it to its own foundations. | **L1** | **OPEN** · ✅ I VERIFIED (four files read this session) | `sed -n '198,200p' Cargo.toml` → one `[[test]]`; `cat tests/rete/mod.rs` → 3 lines, one `include!`; `sed -n '86,89p' build.rs` → the emitted `mod <stem>;`; `grep -rn 'use super::\|use crate::' tests/rete --include='*.rs'` → **0** |

| **4W1 ★★★** | circumspicere | `.config/nextest.toml:283-330` (the override) and `:208-209` (its own stated reason) vs **nine** sibling `#[test]`s in `src/rete/kernel/tests/` — `accum_cost.rs:33,:606,:1659`; `accum_alpha_cost.rs:75,:383`; `rank_and_instrument.rs:1149,:1233,:1319`; `gather_probe_cost.rs:31` | ⭐⭐⭐ **ONE TEST OF A MEASURED COST CLASS WAS GIVEN A BESPOKE TIMEOUT BY NAME. NINE SIBLINGS RUNNING THE SAME PRIMITIVE GET THE DEFAULT 15s/30s.** `accum_fire_phase_census` was measured at **8.13s alone / 35.39s under floor — 4.35x contention** and granted `slow-timeout = 90s/180s, priority = 98`. ⛔ **The config records WHY that clause is needed**: the test lives in `binary_id(wat)`, not `binary_id(wat::rete)`, so *"a `binary_id(wat::rete)` filter alone **silently misses it**."* Someone found that gap, understood it, and closed it **for the one test in front of them.** ⭐⭐ **I derived the population from the PRIMITIVE rather than from the ward's list** — `grep` for `accum_phase_census(200,200)` / `accum_count_census(200,200)` / `(G, W)` over the tree, then walked back to each enclosing `fn` — and found **nine** uncovered siblings, one more than the ward (`accum_matcher_op_census`, `accum_cost.rs:33`). **`accum_fire_phase_census` appears 7 times in the config; all nine siblings appear 0 times.** All nine are plain `#[test]`; `#[ignore]` across the four files: **0**. ⛔ **Four of them run THREE (200,200) calls in a bare `RUNS=3` loop with no cheaper ladder points — a heavier shape than the test that was measured.** At the cohort's own recorded 3.5x–4.4x band (which the file explicitly says applies to *"a future test added here"*) they project to **30–53s against a 30s kill.** ⛔⛔ **And the file says what that failure would look like**: its struck `retries = 1` note reads *"the second run passing DESTROYS the only evidence the first run produced, and the report says 'flaky' where the truth is 'failed, once, with an arm nobody kept'"* — so a sibling timing out presents as **a flaky rete test, in a repo whose doctrine says there is no such thing.** ⚠ The ward's NAMES were right and its ADDRESSES ~4 lines high at 7 of 8 sites (it cited the `RUNS`/`const` lines); corrected above. **Closure: one line — extend the existing filter's name alternation.** | **L1** | ✅ **RESOLVED 2026-09-08 — MEASURED, NO ACTION.** The first vigilia row driven to resolution. All nine measured six-sample alone / one shared floor loaded; **worst loaded is `cell_rank_after_fanout` at 7.262s against a 30s kill.** Measured contention **1.40x–1.80x**, far under the 3.5x–4.4x band the projection used. **No name added to the filter** — adding one would have padded it with headroom nothing needs, which is the defect this row named, committed as its own cure. `.config/nextest.toml` verifiably unchanged. | `grep -rn 'accum_phase_census(200, *200)\|accum_count_census(200, *200)\|…(G, *W)'` → 10 call sites; `awk` walk-back → the 10 enclosing `fn`s tabulated in the report; `grep -c <name> .config/nextest.toml` → **7** for `accum_fire_phase_census`, **0** for each of the nine; `sed -n '$((n-1))p'` at all nine → `#[test]`; `grep -rn '#\[ignore\]'` over the four files → **0** |

| **4W2 ★★★** | ⛔ **ORCHESTRATOR + executor — found by MEASURING `4W1`, not by a ward** | `.config/nextest.toml`, the rete-cohort block's measured table (derived 2026-08-17) vs `.floor/2026-09-09T00-50-29Z/clean.log` | ⭐⭐⭐ **THE CONTENTION BAND EVERY COHORT BUDGET IN THIS FILE IS DERIVED FROM NO LONGER HOLDS.** The file records `accum_fire_phase_census` at **8.13s alone / 35.39s under floor / 4.35x**, and instructs that *"a future test added here should be budgeted at alone × 4.4, not × 3.5."* ⛔ **Measured on today's floor, the same test loaded at 10.580s — 1.30x, not 4.35x.** Its loaded time fell **3.3x**. Every one of the nine siblings measured 1.40x–1.80x. **Anyone following the file's own instruction today would over-budget by roughly 3x**, in a file whose entire discipline is *"THE NUMBER, DERIVED NOT PICKED."* ⚠ **Not dangerous — over-provisioned budgets are safe — but the derivation is stale, and a stale derivation in this file is exactly the class it exists to prevent.** ⭐ The likely cause is checkable and nobody has: the band was derived when the suite held **4697** tests and floored at ~220s; it now holds **5484** and floors at ~452s, and `priority = 98` places the cohort in an early wave whose contention profile the growth may have changed. **The floor got slower overall while its heaviest tests got faster** — that is the shape to explain. | L2 | **OPEN** · ✅ I VERIFIED (durations re-read from the floor log myself) | `grep -oE 'PASS \[ *[0-9.]+s\].*<name>'` over `.floor/2026-09-09T00-50-29Z/clean.log` → the ten durations tabulated in the SCORE; `grep Summary` → `452.835s, 5484 passed, 19 skipped` |

| **4Z1 ★★** | ⛔ **executor — refuting my own DESIGN while curing `4Q1`** | `tests/lint/no_unknown_sequi_rune.rs:84` (`collect_rs(Path::new("src"), …)`) vs the `rune:sequi` population under `tests/` | ⭐⭐ **THE `rune:sequi` CLOSED-SET GATE WALKS `src/` ONLY, SO EVERY SUCH RUNE UNDER `tests/` IS UNGATED.** I asserted in the `4Q1` design that this gate would enforce the new rune's category. It cannot see it. **The category vocabulary is machine-checked in `src/` and hand-held everywhere else** — and this is not hypothetical: a pre-existing instance already sits at `tests/types/probe_stone_binder_is_universal.rs:58`, plus the one just added at `probe_arc278_import_accounting.rs`. ⚠ Note the sibling gate does NOT share this hole: `no_unknown_ward_rune.rs` walks six roots including `tests/` — *"a root that holds none today still belongs here."* **Two gates over the same kind of marker, one of which already wrote down the argument the other needed.** | L2 | **OPEN** · ✅ I VERIFIED | `sed -n '84p'` → the `src`-only walk; `grep -rn 'rune:sequi' tests/` → the ungated population; `grep SCAN_ROOTS no_unknown_ward_rune.rs` → the six-root sibling |
## Verified by the orchestrator — target 4

⭐⭐⭐ **`intueri` RETURNED CLEAN, AND IT IS THE STRONGEST NEGATIVE OF THE VIGILIA BECAUSE IT ANSWERED A CLASS QUESTION WITH METHOD.** `complectens` found `4L1` — a test whose name promises two things and whose body is three statements — and I asked `intueri` whether that was **isolated or a class**, across all **613** test names. It built a mechanical pass over a stated vocabulary (`_and_`, `every`, `only`, `exactly`, `never`, `always`, `proves`, `guarantees`, `_per_`, `names_the` …), got **99 candidates, read ALL 99 bodies in full**, plus **40 drawn at random** — **128 of 613 (20.9%), at essentially 100% coverage of the risky shape.** ⭐ **Zero new instances.** `4L1` is an isolated lapse, not a class. It also confirmed no `utils.rs`/`helpers.rs`/`common.rs` anywhere (I checked: **0**), and judged three 1–2 letter names acceptable with reasoning rather than silently passing them. **A negative earned this way is a fact about the corpus nobody had.**

⭐⭐ **A CORPUS FACT PROMOTED — THE SAME CONVENTION HAS NOW BROKEN TWO INDEPENDENT SCANNERS, BOTH IN THE SAME FILE.** `purgare`'s orphan scan produced **six false positives** in `probe_arc278_export.rs` because a `// rune:vocare(…)` comment sits **between `#[test]` and the `fn` signature**. `intueri` hit the identical bug in its own name-extractor, in the same file — and, separately, a same-line `#[test] fn x() { … }` form in `probe_arc278_seq1b_list_hofs.rs`. **Both wards caught and fixed their own instrument mid-cast and re-verified the total held at 613.** ⛔ **Any future scanner that pairs an attribute with its item in this corpus must expect both shapes.** Two independent hits make it a property of the corpus, not a coincidence of one script.

⛔ **AN EIGHTEENTH FIGURE OF MINE CORRECTED — AND THIS ONE WAS INVERTED, NOT MERELY WRONG.** I handed `solvere` *"5 of the 8 cost files call `render_phase_table`; three do not."* `grep -rl` does return 5 — **but two of them are not cost-family members**: `mod.rs`, which matches its own definition site, and `rank_and_instrument.rs`, which is outside the 8-file family entirely. **Within the 8, only 3 call it, so 5 do NOT — the exact inverse of what I said.** I verified by classifying each of the 5 by filename. ⭐ **The ward named the trap by number**: a name-grep over a whole directory answering a **membership** question it cannot see — trap #1, for the fifth time this vigilia, and the first time it inverted a conclusion rather than shifting a count.

⚠ **AND ITS OWN COORDINATES ARE MOSTLY BUT NOT WHOLLY EXACT.** Of the five hand-rolled subtraction sites I spot-checked, **four read verbatim as claimed** (`fanout_cost.rs:425,558,828`; `cascade_cost.rs:393`) and one — `strat_cost.rs:412` — lands on `};`, off by a line or two. **The substance survives independently**: widening the pattern myself gives **20** sites, more than its 17. Recorded because trap #7 landed one cast ago and every citation now gets both halves checked.

⛔⛔⛔ **`cernere`'s FORMS ARE RIGHT AND ITS COORDINATES ARE INVENTED — AND THAT IS THE INVERSE OF EVERY ERROR THIS VIGILIA HAS RECORDED SO FAR.** It cited the finding at `:542-543`, its reachability analysis at `:527-535`, and `:user::run` at `:537-545`. **The file is 40 lines.** The forms are at `:37-38` and `:user::run` is at `:32`. ⭐ **Every substantive claim nonetheless verifies exactly**: both spellings are retired per the codemod's own rename table, neither has a dispatch arm anywhere in `src/`, and a corpus-wide sweep of all 163 `.wat`/`.wat.bad` finds precisely these two occurrences. **So the ward read the right forms and fabricated the addresses.** `[[never-cite-a-symbol-you-have-not-grepped]]` says a `file:line` is TWO claims; **this vigilia has spent seventeen corrections on the first half, and this is the first failure of the second.** The rule *"a finding whose line number does not hold is discarded"* exists to stop me crediting unverified claims — I verified this one myself and corrected the address rather than discarding a real finding, and the ward's report carries a note saying so.

⭐⭐ **AND ITS BEST WORK IS A POSITIVE NOBODY COULD HAVE OBTAINED OTHERWISE: `datamancer.src.wat` IS CLEAN.** That file is driven by no probe and parsed by no gate (`4G1`), so **every form in it was unvalidated by any automated means**. The ward read all 141 lines and traced ~30 distinct forms each to a definition site — `defrule`/`where`/`not`/`exists` to `wat/rete/syntax.wat` and `src/rete/clause.rs:376-382`, the outcome enums to `wat/rete/oracle/`, `insert`'s variadic dispatch to `src/runtime.rs:5381`. **Zero phantoms.** That reading is the only validation the file has ever had.

⭐ **IT ALSO DISCONFIRMED ITS OWN SHARPEST SUSPICION.** Bare `:wat::rete::fire-once` (7 uses) looked retired per the codemod's header. It checked `src/runtime.rs:5664` — `":wat::rete::fire-once$native" | ":wat::rete::fire-once" =>` — and found the codemod **reassigned the bare name's meaning rather than retiring the spelling.** I re-ran that line myself. **Not a phantom, and it said so.**

⭐⭐ **`exigere` PRODUCED THE DISMISSAL LIST I ASKED FOR, AND ITS `banked` VERDICT IS A POSITIVE WORTH KEEPING.** It examined **24 of 26** hits and disposed of each with a reason. On the slice I flagged as most interesting: **every `banked` in this target carries the self-clearing machinery** — all six hits resolve to two passages, both describing banking episodes that have *already closed*. One is exemplary: a `.wat` banked with `rune:lint(red-by-design)` and its own disposal clause (*"if this file ever loads, D5 is cured and the rune must go with it"*) — **the rune is now gone, the file loads, and a standing test drives the pair.** That is the honest deferral form, mechanised and then discharged.

⚠ **AND THE `#[test]` COUNT SPLIT TWO-TO-THREE, WHICH SETTLES IT.** `exigere` reported **615**, the second ward to do so after `vocare`. But `grep -rno '#\[test\]'` gives me **618** raw and **613** at start-of-line — so **neither 615 is reproducible, and the two wards that reported it could not reproduce each other's derivation.** Against that, `complectens`, `secare` and `probare` each independently derived **613**, and the latter two each named the identical pair of prose lines that account for the difference. **613 is confirmed four ways** (three wards plus my own start-of-line derivation) and stands.
...
⛔⛔ **A NINETEENTH FIGURE OF MINE WAS WRONG, AND IT IS THE FIRST THAT NO TOOL PRODUCED.** I handed `sequi` a lead — *"`SEQUI_CATEGORIES` holds FIVE entries while its own message says four"* — **explicitly as a question**. It read `tests/lint/no_unknown_sequi_rune.rs:31-36` and **refuted it**: the list holds **four**, the message is correct, there is no defect. I re-derived it myself: `sed -n '31,36p' | grep -c '"'` → **4**. ⭐ **The other eighteen were grep artifacts — a pattern matching prose, or data, or a name where a walk was the mechanism. This one was a plain miscount of a four-item list I had printed in front of me**, reading the *line range* `31-36` as the entry count. **The cure runs the other way from all the previous ones: derive the count with a command even when the list is short enough to eyeball — ESPECIALLY then, because a short list is exactly what disarms the instinct to check.** ⭐⭐ And it is the fourth confirmed instance of `[[hand-a-lead-down-as-a-question]]`: stated as a finding it would have come back agreed, and a defect that does not exist would now be rowed.

⛔ **AND THE SAME WARD MISCOUNTED `#[test]` AS 618 — TRAP #2, COMMITTED IN THE ACT OF CORRECTING SOMEBODY ELSE'S NUMBER.** Its 618 is `493 + 125`, and the **493 is `grep -rn '#[test]' tests/rete` with NO `--include='*.rs'`** — sweeping in three `.wat` COMMENT lines that use the token in prose (`probe_arc278_P2_native_fire_once.wat:17`, `probe_arc278_session_ceiling_second_session.wat:55`, `probe_arc278_P4a_native_fire_rules.wat:60`). 490 real + 3 prose = 493. The anchored `.rs`-only count is **613**; unanchored-but-`.rs`-only is 615. ⭐ **613 now stands for the SIXTH independent derivation.** ⛔ **Trap #2 was in that ward's brief, by number, and it fired anyway** — which is the argument for keeping the traps as *worked instances* rather than a list of names.

⭐⭐ **ITS `AtomicU64` CORRECTION IS RIGHT AND MINE WAS THE TRAP — SAME TRAP, OPPOSITE DIRECTION, ONE EXCHANGE.** I handed down *"`AtomicU64` — 4 hits / 2 files."* The ward reported **3 code hits, all function-local in `harvest_cost.rs`**, plus prose. Re-derived: `grep -rn` returns **6** lines — `harvest_cost.rs:365` (a `use`), `:399` and `:431` (`AtomicU64::new(1)`, both **local**, neither global) — and **three prose lines** describing a *production* global (`probe_arc278_concurrent_retes.wat:13,23` and `.rs:4`). My `--include='*.rs'` excluded the two `.wat` prose lines and kept the one `.rs` prose line: **arithmetically defensible, semantically misleading**, implying two files carry an `AtomicU64` when in code one does and none is global. **Both of us over-counted by admitting prose, in the same exchange, in opposite directions. The trap does not care which side of the brief you are standing on.**

⚠ **A DOCTRINE SPLIT, RECORDED AND DELIBERATELY NOT ROUTED.** The `sequi` spell says hidden domain state *"has no rune category **by design**… sequi refuses that suppression for the case the spell exists to catch."* This repo's `docs/CONVENTIONS.md:1054-1070` mints exactly such a category — **`ambient-context`, defined as *"real DOMAIN state, reached globally or per-thread instead of threaded"*** — citing `ARM_TABLE` and `EXEC_ARENA`. **The local vocabulary licenses precisely what the spell refuses.** That is why the ward's `4Q1` recommendation is correct under this repo and forbidden under the spell, and why `Q2` (target 1) remains a builder decision rather than a settled row. ⛔ **Not routed as a finding**: `CONVENTIONS.md` is a `docs/*.md` file and the builder's standing ruling for this cast is that those are stale by default and out of scope — *"they will be addressed in time, not now."* It is recorded because it changes how **every** `rune:sequi` in this repo must be read.

⛔⛔⛔ **A TWENTIETH ERROR OF MINE — AND IT IS THE THIRD INDEPENDENT SCANNER THIS CORPUS HAS BROKEN THE SAME WAY.** I tried to falsify `conformare`'s claim that all 17 loose-assert candidates carry a rune. My one-line-lookback check returned **6 apparently BARE** sites — which would have refuted the ward. **I read all six before reporting them, and every one carries a MULTI-LINE rune comment**, the `rune:lint(loose-assert)` sitting 2–4 lines above the `assert!` with continuation comment lines between (e.g. `probe_arc278_then_user_forms.rs:117-120`). ⭐⭐ **This is the SAME property that broke `purgare`'s orphan scan (six false positives) and `intueri`'s name-extractor** — both a `// rune:vocare(…)` sitting between `#[test]` and `fn`, both in `probe_arc278_export.rs`. Mine is the same shape one construct over. ⛔ **Three independent scanners, three casts, one corpus property, and it is now a RULE rather than a coincidence: any scanner pairing a marker with its item in this corpus must tolerate an arbitrary run of comment lines between them.** ⭐ The real gate does exactly that — its state machine skips comment lines — which is why the gate is green and my throwaway was not. **The instrument that disagrees with a careful ward is the thing to check first.**

⭐⭐ **AND THE PRACTICE THAT KEEPS PAYING: BOTH OF THIS PAIR'S LEADS WENT DOWN AS QUESTIONS, AND BOTH CAME BACK SETTLED RATHER THAN AGREED.** `temperare` was handed **6 world-freeze candidates with the heuristic's blindness disclosed** (*"a crude indent heuristic cannot tell a loop body from a match arm or a closure — read them and settle it"*) and returned **2 real, 4 not**, each with its reason. `conformare` was handed a **doctrine collision with two named hypotheses and neither endorsed**, plus explicit permission to answer *"unreachable without running it"* — and it built a read-only re-implementation of the gate's own algorithm rather than guessing, measured 0 violations over 1028 files, and named the one thing that would make it certain. ⛔ **Neither ward was told which answer I expected, because I did not have one.** That is now five for five in this vigilia: `[[hand-a-lead-down-as-a-question]]`.
