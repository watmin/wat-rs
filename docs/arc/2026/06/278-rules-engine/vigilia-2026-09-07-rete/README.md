...
| 4 | `tests/rete/` + `src/rete/kernel/tests/` (**306 files, 38,058 lines** — measured 2026-09-08) | **17 / 17** | **28** | ✅ **CLOSED** |
...
only.** ⛔ **A SIXTH derivation came back 618 and was wrong in a way worth naming:** `sequi` ran
`grep -rn '#[test]' tests/rete` **without `--include='*.rs'`**, sweeping in three `.wat` COMMENT lines
that use the token in prose (`probe_arc278_P2_native_fire_once.wat:17`,
`probe_arc278_session_ceiling_second_session.wat:55`, `probe_arc278_P4a_native_fire_rules.wat:60`).
**The `--include` is not optional here — this corpus is 47% `.wat` and its comments discuss Rust
attributes.**
...
(1), `cernere` (1), `exigere` (2 L1), `solvere` (2), `intueri` **CLEAN**, `struere` (2 + 1 of mine, two L1), `sequi` (2, both L1), `temperare` (1), `conformare` (2, two L1), `circumspicere` (1 L1) — plus **2 of mine** (`4T3`, `4L4`).

**✅ TARGET 4 — CLOSED at 17/17, 28 rows. ⭐⭐⭐ THE VIGILIA IS COMPLETE: FOUR TARGETS, 61 WARD
CASTS, 136 ROWS, 30 L1.**

⭐⭐⭐ **`circumspicere` CAST LAST RETURNED THE SHARPEST FINDING OF THE TARGET FOR THE FOURTH TIME
RUNNING — 4 for 4.** `4W1`: one test of a measured cost class holds a bespoke 90s/180s timeout by
name; **nine siblings running the same primitive get the default 15s/30s**, and four of them run a
*heavier* shape than the test that was measured. The config records why that one clause exists —
*"a `binary_id(wat::rete)` filter alone silently misses it"* — so the gap was found, understood,
and closed for the one test in front of someone. **Cast it last, every time. That is not a
preference; it is four measurements.**

⛔ **AND THE CAST'S OWN FOUNDATION WAS WRONG UNTIL THE LAST HOUR.** `4L4`: `complectens`'s
*"structural proof"* that the 257 `probe_arc278_` files are separate cargo binaries — which I
accepted and used to narrow `solvere`'s search — is the **exact inversion** of the truth
(`Cargo.toml:198-200` = ONE target; `build.rs:86-89` emits `mod <stem>;` for each). Found while
measuring `circumspicere`'s brief, because that ward is the only one allowed to read `Cargo.toml`.
**Every inward brief confined its ward to the 306 target files, and the answer was in the build
system. A scope that protects a cast from noise blinds it to its own foundations.**
...
**NOTHING IS DRIVEN TO RESOLUTION.** **136 rows open, 30 of them L1** (one, `3P1`, is itself L1×5). ⛔ **Do not copy those two numbers forward — re-derive them, because both were wrong here and a recolligere caught them 2026-09-08:** `grep -c '^| \*\*' FINDINGS.md` → 136, and `grep -c '^| \*\*.*\*\*L1\*\*' FINDINGS.md` → 30. The prose said **83 and 8** while the table three lines above it said 38+36+10; a total stated beside the table it could be derived from is the same defect this cast rows against the substrate. **THREE** decisions the builder still owes:
