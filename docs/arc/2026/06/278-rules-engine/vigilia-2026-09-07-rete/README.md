...
| 3 | `wat-scripts/perf/grid/` (148 files, 16,616 lines) | **12 cast** | **29** | ⏳ MID-FLIGHT |
...
`probare` (3). ⭐ **`purgare` LANDED before the wall** (1 row, `3G1`) — it is in `reports-target3/purgare.md`.

**Target 3 — still to cast:** `cernere` · `perspicere`,
...
**NOTHING IS DRIVEN TO RESOLUTION.** **103 rows open, 16 of them L1** (one, `3P1`, is itself L1×5). ⛔ **Do not copy those two numbers forward — re-derive them, because both were wrong here and a recolligere caught them 2026-09-08:** `grep -c '^| \*\*' FINDINGS.md` → 103, and `grep -c '^| \*\*.*\*\*L1\*\*' FINDINGS.md` → 16. The prose said **83 and 8** while the table three lines above it said 38+36+10; a total stated beside the table it could be derived from is the same defect this cast rows against the substrate. **THREE** decisions the builder still owes:
