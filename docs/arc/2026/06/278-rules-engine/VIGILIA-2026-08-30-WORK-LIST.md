...
| **C19** | `src/check.rs:485` (`InferCtx::fresh`) + every diagnostic that renders a `TypeExpr::Var` | ⛔ **A USER-FACING DIAGNOSTIC PRINTS A VALUE THAT CHANGES EVERY RUN.** Same binary, same file, five runs of `tests/function/probe_arc247_hof_coll_first.wat.bad` → **five different outputs**. The sole difference is the type-variable id: `expects (:wat::stream::Stream :- [:?125])` vs `[:?3399]`. **Bounded, measured:** error kinds, their ORDER, spans and message text are all **stable** — normalising `:?[0-9]+` makes three runs byte-identical. `InferCtx::fresh` is a monotonic `u64` from 0, so the printed id encodes *how many type variables were allocated before this one*, and that count varies per process — i.e. **some traversal upstream is `HashMap`-ordered** under Rust's per-process random hasher. **0 goldens pin a literal id today**, so nothing is flaky yet — but the first golden written over such a message flakes, in a repo that bans known flakes absolutely. Second edge, independent of testing: the id is an internal counter that means nothing to a reader and differs every time they run. **Two render sites, both leaking the raw id: `check.rs:16342` `format!(":?{}", id)` and `:16372` `format!("?{}", id)`.** ⛔ **A cascade hypothesis was TESTED AND DISPROVED** — correcting the fixture's argument order leaves the same `=` error with the variable **resolved** to `:wat::core::i64`, so inference is sound and the var appears only where an earlier error left the type genuinely undetermined. **The defect is that "undetermined" renders as an allocator counter.** Its sibling message in the same output renders a declared parameter as `T`, so one output shows one unknown as `T` and another as `:?801`. Likely cure: render `_`/`?`, or renumber per-diagnostic from 1, rather than chasing the traversal. ⚠ Found because a D11 rider reported `.wat.bad` output as nondeterministic and the builder refused to let that pass as a footnote. |
