...
| **C16** | `fire/delta.rs:118-170` (`record_seed_leaf_vs_alpha`) | ⛔ **THE OCCUPANCY DIFFERENTIAL IS STRUCTURALLY BLIND TO THE DEFECT IT IS NAMED FOR.** It builds `predicted` by `continue`-ing on any fact whose `i64_by_fact[i]` is `None` — **the same predicate that decides batch membership** — so `predicted` re-derives writer 2's output and is compared against writer 2's output. Measured while D7 was actively dropping a fact: `predicted=2 actual=2 extra=[] missing=[]`. It agrees by construction with the corruption. ⚠ The orchestrator's D7 brief told the rider to trust it as the detector; a rider obeying only that files a confident false negative on a live bug. Its reference must come from outside the batch predicate — the ORACLE is what found D7. |
| **C17** | `src/rete/kernel/insert.rs:221` | `insert_facts_on_session` per-element-checks its argument **only when it is a `PersistentVector`** — `if let Value::wat__core__PersistentVector(pv) = &new_facts_vec`. Any other shape reaching `vector_concat_inner` skips `require_record_fact`. Found while driving D7: it is the second, unconstructed route to the same alpha collision, since a non-Aggregate reaching `input_facts` desyncs `wm.i64_by_fact` from the fact index and misroutes the NEXT packable fact. **Not driven** — the row is the `if let`, not the collision. |
