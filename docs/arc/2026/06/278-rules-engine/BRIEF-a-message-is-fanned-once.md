...
remaining pairs before replying, and counts it. Read `DESIGN-a-message-is-fanned-once.md` first —
...
`SCORE-the-server-manages-its-own-capacity.md` — the stone that created this, same file.

Write `SCORE-a-message-is-fanned-once.md`, then `pulsare_yield kind=scored`.
