...
`DESIGN-partial-only-when-it-must.md` first — it carries five dead models so you do not re-run any
of them.
...
`SCORE-the-server-manages-its-own-capacity.md` — the stone this narrows.

Write `SCORE-partial-only-when-it-must.md`, then `pulsare_yield kind=scored`.
