...
1. **`DESIGN-vis-is-swept-not-chosen.md`** — the failure it unblocks and why the band is not picked
   by reasoning.
2. **`SCORE-a-reconnect-is-not-an-abandonment.md`** — the evidence: one stuck batch per queue, every
