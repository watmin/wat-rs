...
1. **The `log` MACRO** (builder-directed NEXT) — capture `(:wat::kernel::call-site)` AT THE USER'S log-call boundary (capture-at-expansion, like `assertion-failed!` fires at the author's line). caller.2 fills *constructions*, which capture the ENCLOSING-fn's caller (offset 0) — so a Log built in a Rust-invoked fn gets the Rust site (absolute path); the macro is how `emitted-from` becomes the user's real log line. Pairs with the **runtime-remainder exact gate** (`budget − serialized-bytes(the filled required params)`; `edn::write` + real byte sizing exist at runtime — the exact per-caller enforcement; capacity stone 1's floor is the advisory hint).
