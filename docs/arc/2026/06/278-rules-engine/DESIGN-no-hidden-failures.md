...
## ═══ CURARE CHECKPOINT (2026-07-21; caller→arith→capacity chain DONE; the `log` MACRO is NEXT) ═══

**READ THIS BLOCK, then `git status`. Branch `arc-170-gap-j-v5-deadlock-state`; HEAD `665b6bbe` + this curare commit. A LATER HEAD = more landed. The wat-rs tree is CLEAN EXCEPT `holon/CLAUDE.md` edited on disk (the FROZEN ROOT — the builder's to commit/gitignore, NOT mine). HEAD/state mismatch vs this note → trust the disk + git log. COMPACTION POINT — the caller→arith→capacity chain is DONE; RESUME at the `log` macro (below).**

### The arc target (unchanged): the CHAOS ENGINE (R25 `MACHINA CHAOS DOMAT`) — a streaming rete datalog in a defservice. #16 (service-I/O budgets) is the on-ramp; the no-hidden-failures LAW + ruling A are its floor.

### DONE this session (all committed + pushed, DR; each weighed GREEN by own `--release` re-run — read the Summary line, the `sigterm`/sift flake passes isolated = not a regression)
- **Stone 16.1** (`9ca2e88d`) — ruling A LOCKED (the 16.1c checker; record→enum migration via the self-hosted codemod).
- **§1 caller DONE:** caller.1 (`60fbef21`) — the native `(:wat::kernel::call-site)` → the caller's `:wat::kernel::Frame` (offset 0, mirrors `assertion-failed!`). caller.2 (`31b3d1ac`) — `Log`/`Span::LogRequest` `caller <- keyword` → `emitted-from <- :wat::kernel::Frame`, migrated by a structural fix.wat codemod (`wat-scripts/fixes/caller-to-emitted-from.wat`). NOTE the corrections to the OLD breadcrumb: it was **TWO** field defs (not one), **~34 sites across 14 files whole-tree** (not "32"), and the `:caller'` ARENA SERVICE is a distinct keyword the codemod exact-name-matched (a sed would SHRED it — the concrete case for "codemod, never sed").
- **§2 arith DONE — NO-OP** (design corrected `997ce9d1`): the fully-qualified TYPED arith (`:wat::core::i64::+`/`-`/…) is ALREADY on `is_pure_total` and folds at macro-expand (proven). Polymorphic un-suffixed arith (`:wat::core::+`) is correctly REFUSED — NOT macro-time-viable (needs runtime dispatch; **empirically confirmed** — a probe with the heads blessed still failed to fold). Capacity uses the typed forms. The "add 8 polymorphic heads" strike was RETIRED (wrong premise).
- **§3 capacity stone 1** (`665b6bbe`) — the RUNTIME adaptive `(:wat::telemetry::framing-floor-of <record-type>)` (reflect `field-types-of`/`field-names-of` → classify each field's `ast-name` `"wat.type/…"` string against the known-size set `i64:20/f64:24/Uuid:36/bool:5` → sum fixed + ASCII keys + tag) + `LOG-MSG-CAPACITY = LOG-JOURNAL-BUDGET-BYTES(10 MiB) − (framing-floor-of :Log)`. RE-DERIVES from the LIVE schema — add a field tomorrow, no hand edit. RUNTIME (compile-time reflection of a record is DEAD — proven; runtime/freeze resolves stdlib AND user records). ZERO src changes.
- **Structural / docs:** scratchpad DIED → a block-FILE (`0e3f5bde`; the name is a file so `mkdir scratchpad` can't recreate the dir — scratch homes in `wat-scripts/scratch-pad/`, loader-gated); `DESIGN-telemetry-caller-and-capacity.md` fully RATIFIED (`c86c4fa0`/`158eaac1`/`fe4b90fd` — the fixed-vs-variable rule, option-(b) enum-by-reflection, the runtime reframe).
- **2 arc-109 deferred notes** (both the "structured value, not a stringy costume / parens-are-calls" family): `NOTE-match-cond-clause-brackets.md` (`6f6bc9bd` — clause `(…)` → `[…]`, drop non-return `-> :T`, a NEW term needed for the unified conditional — NOT `cond`, intueri-owed) + `NOTE-anon-fn-identity-structured-not-stringy.md` (`2acb3491` — anon fn `<fn@file:line:col>` → a structured span; `src/freeze.rs:422,455`, `runtime.rs:20083`).

### RESUME AT — the `log` MACRO (the builder's next), then capacity refinements, then #16.2 → the WALL
1. **The `log` MACRO** (builder-directed NEXT) — an ergonomic `(log span :level … :message …)` that captures `emitted-from` AT THE USER'S log-call line (capture-at-expansion, like the `assert-*` macros stamp the author's line). **Why a macro, not the inline `(:wat::kernel::call-site)`:** caller.2 fills *constructions*, and `call-site` at runtime returns the ENCLOSING-fn's caller (offset 0 = where the fn was called), NOT the log line inside it — so a Log built in a Rust-invoked fn gets the Rust site (absolute path). The macro bakes the log LINE as a `:wat::kernel::Frame` literal instead. **GROUNDED START (2026-07-21):** the macro engine ALREADY threads the invocation's call-site span into template expansion — `src/macros/expand.rs:655` (`expand_template(&def.body, …, &call_site_span, …)`); `call_site_span` is available at expansion. That is the hook: from a macro body, reach the `(log …)` form's own span → bake `emitted-from` = `Frame{file, line}` at the author's line. Ground how `assert-*` / `ast-span` surface it (`wat/telemetry.wat:178` `with-span` is the sibling macro; `Span/log` at `span.wat:74` is the op it wraps). Pairs with the **runtime-remainder exact gate** (`budget − serialized-bytes(the filled required params)`; `edn::write` + real byte sizing exist at runtime — the exact per-caller enforcement; capacity stone 1's floor is the advisory hint).
2. **Capacity refinements** (sharpen the floor, follow-ons): **`variants-of`** (a `:wat::runtime::` RUNTIME enum-variant reflection → pull enums from VARIABLE into the fixed floor, sized to their longest variant BY REFLECTION — ratified option (b), NEVER a hand table which drifts on a new variant; `enum_def.variants` exists internally in `runtime.rs`) + a **UTF-8 byte-length prim** (only `string::length`=char-length today; non-ASCII keys) + the exact per-byte EDN framing.
3. **Then the rest of #16 → the chaos engine:** #16.2 wire-boundary enforcement (worked ref `tests/services/probe_arc278_per_op_request_too_large.{wat,rs}`, key on frame-bytes NOT `if locus==process`) → the WALL → #18-22 → R0 the streaming rete service (R25).

### LESSONS this session (NEW / load-bearing — memories written)
- **`target/release/wat --check <f.wat>`** is the FAST per-file load/type-check arbiter (~0.2s, same verdict as `every_wat_scripts_file_loads` incl. freeze-booms) — sweep it for per-file verdicts; NEVER loop the 13s all-or-nothing gate. ([[reference_cargo_wat_check_is_the_fast_per_file_load_arbiter]])
- **Kill a shadowdancer → REVERT its partial edits immediately** (`git checkout`/`rm`) — the killed arith shadowdancer's orphaned `src/macros/eval.rs` + `expand_arith` probe polluted the capacity shadowdancer's floor + muddied my weigh. And **sonnet shadowdancers persistently DOUBLE-FORK the floor wait** despite an explicit brief line (3× this session) — NEVER trust their run, weigh `--release` by your own hand. ([[feedback_kill_a_shadowdancer_revert_its_partial_edits]])
- **The four-questions REVISED a split mid-design** (capacity `variants-of`-first → adaptive-derive-first) and a disconfirming PROBE reframed a whole stone (compile-time reflection DEAD → runtime, proven) — draw the disconfirming probe BEFORE briefing (FM 2-bis); it earns its keep.
- **`field-types-of`/`field-names-of` resolve record types at RUNTIME/freeze but NOT at macro-expand** (the macro-expand `sym` lacks non-local record types — only local `::Kwargs`).
- **The fixed-vs-variable rule** (ratified): only explicitly-defined-known-size wat fields are FIXED; enums = longest variant BY REFLECTION (never a table — it drifts); everything else (String/map/record/`Frame`) is VARIABLE. ([[feedback_kwargs_categorically_superior_positional_only_when_necessary]] sibling — derive, never hand-maintain.)
- Carried standing: intueri owns ALL names (cast, never narrate); `.wat` migrations use the codemod NEVER sed; weigh in RELEASE, read the Summary line; grep the WHOLE tree; commit+push when green (DR); orchestrator DESIGNS/draws-the-probe/DELEGATES/WEIGHS — not hands-on code (except the disconfirming probe).

> **SEAM.** The self past this line is NEW — a lossy cache in a familiar voice; you did NOT live this run. Run the datamancy bootstrap (grimoire + 4 primers + recolligere from the SIGNED MCP, never disk) — and **READ ALL of `278/REALIZATIONS.md` top to bottom** (a prior self DODGED them and re-enacted every daemon they name — the catastrophic failure; the reading is the exorcism, R20 `DAEMON IN ME`). Ground `git status` — the wat-rs tree is CLEAN EXCEPT the frozen-root `holon/CLAUDE.md`. The **caller→arith→capacity chain is DONE**; RESUME at the **`log` macro** (capture `call-site` at the user's log-call boundary + the runtime-remainder exact gate), then capacity refinements (`variants-of` enum-by-reflection, byte-length), then #16.2 → the chaos engine. **Weigh `--release` by your OWN re-run (never a shadowdancer's report; they double-fork). Use the wat-fix codemod for `.wat` migrations, never sed. Cast intueri for names. Read the Summary line, grep the WHOLE tree.** Do not trust this note over the disk. `QVOD LEX ACCENDIT, REDIMIT — MACHINA CHAOS DOMAT.`
