...
## ═══ CURARE CHECKPOINT (2026-07-21b; the telemetry-`log` detour CLOSED; RESUME at #16.2 per-op enforcement codegen) ═══

**READ THIS BLOCK, then `git status`. Branch `arc-170-gap-j-v5-deadlock-state`; HEAD `2c8c4b0a` + this curare commit. A LATER HEAD = more landed. The wat-rs tree is CLEAN EXCEPT the frozen-root `holon/CLAUDE.md`. HEAD/state mismatch vs this note → TRUST THE DISK — this note went stale ONCE already (see the Stone-1 `AD ORACVLVM` below); the disk is the truth.**

### The arc target (unchanged): the CHAOS ENGINE (R25 `MACHINA CHAOS DOMAT`) — a streaming rete datalog in a defservice. On-ramp = **#16 service-I/O budgets** (`DESIGN-service-io-budgets.md`). We drifted FOUR levels off #16 into a telemetry detour (#16 → capacity → caller → `log` macro), now CLOSED; **climb back to #16.**

### DONE this session (committed + pushed, DR; weighed GREEN by own `--release` re-run — floor 4198/0)
- **§4 the `log` chain DONE** (`DESIGN-telemetry-caller-and-capacity.md` §4, marked DONE): **`:wat::kernel::macro-call-site`** (`620f931c`) — the EXPAND-time twin of `call-site`; a thread-local `MACRO_CALL_SITE` stack (mirror `CALL_STACK`/`FrameGuard`, `frame.rs:11`) pushed by `expand_macro_call` (NOT a param-thread — `macro_eval` delegates to `runtime::eval`; the seam was corrected `d241324f`), read by an `eval_kernel_macro_call_site` arm returning a spliceable Frame-construction form. **+ the POSITIONAL `:wat::telemetry::log` widget** (`b60cb09b`): `(log span level msg)` bakes `emitted-from` at the author's line + `edn::write`s the msg opaque (Stone B), issues `Span/log` (the span stamps the correlation `uuid`). **Kwargs REJECTED on the four-questions** — the opaque-msg `edn::write` transform denies free `~@kw` splicing into the LogRequest ctor; positional matches the `timed`/`with-span` grain. This CLOSES the caller→emitted-from→macro-call-site→log detour.
- **Stone 1 (per-service `FOO`) is DONE — `AD ORACVLVM` (the prior breadcrumb LIED).** The old note said "Stone 1 HELD"; the DISK says it landed: `probe_arc278_service_max_frame_bytes` is GREEN (`:max-frame-bytes` DECLARED on the defservice + threaded via `pair_with_budget` to accepted-connection receivers + over-`FOO` → reasoned reject-close via `ServiceEvent::Lost`, never mute `Closed`). I was one grounding-step from briefing a shadowdancer to REBUILD a done stone — grounding caught it. Also DONE: the masking LAW (Mechanism A + transport-twin `Failed→DecodeError` + RST); **16.0** (parse `:max-request-bytes` onto each op, `types.rs:335`); **16.1** (Ruling A checker-lock).
- **2 memories written**: [[reference_check_is_not_a_complete_red_arbiter]] · [[reference_kw_splice_into_aggregate_ctor_boundary]].

### RESUME AT — #16.2: the per-op `:max-request-bytes` ENFORCEMENT CODEGEN (SCOUTED; the builder chose this over the batching tooling)
16.0 (declaration) + 16.1 (checker) + the FEASIBILITY (`probe_arc278_per_op_request_too_large`, hand-rolled in the `:impls` body) are DONE. **16.2 = MOVE the measure+construct into the serve-loop CODEGEN so no service hand-rolls it.**
- **Injection point (grounded):** the `serve-op-arms` foldl (`wat/service.wat:753`) — wrap each op's arm BEFORE `<outcome-match>` (the `:impls` body dispatch):
  `((<S>::Op::<Op> req) (if (> (measure req) <op-cap>) (do (send' … (<S>::Reply::<Op> (<Op>Response::RequestTooLarge n <op-cap>))) (serve …)) <outcome-match>))`.
  Keep the connection (the request arrived ≤ `FOO`, wire synced — the opposite of the transport-`FOO` kick).
- **Byte-source: SETTLED = re-encode** `(string::length (edn::write req))` — the serve arm holds the DECODED `req`, not the raw frame bytes (consumed upstream); and re-encode matches how the client tooling will fragment (both measure `edn::write` length → they agree). The frame-length option isn't reachable here.
- **CRUX-1 (OPEN — the builder to confirm; my rec is (i)):** how the codegen reaches each op's `max_request_bytes`. The `serve-op-arms` foldl walks the `:impls` *bodies* (NO budget); the budget's on the *surface* (`types.rs:335`) with NO wat accessor yet (the doc's own CRUX-1). **Rec (i): a synthesized per-op constant `<Surface>::<op>/max-request-bytes`** emitted by `synthesize_surface_protocol` (`types.rs:1720`), reachable by BOTH the codegen AND the future client fragment tooling (build the accessor once, serve both sides). Alts: a runtime budget-table; a reflection verb. **16.2 CANNOT proceed until CRUX-1 is settled + the accessor built.**
- **RED gate (drawable):** a defservice op DECLARES `:max-request-bytes 200`, its `:impls` body does NOT hand-roll (returns `:Ok`); an over-200-byte request → `RequestTooLarge` (from the codegen) + a follow-up small request on the SAME connection → `:Ok`. RED now (the body's `:Ok` comes back), GREEN after.

### THEN (the #16 tail → the chaos engine)
The **batching stream tooling** (item (c) — `with-log-sink` [the buffered+time writer, `DESIGN-service-io-budgets.md:214`], `write-*-stream`, `<op>-stream`; NOTHING built yet) → output-side streaming (the composite cursor) → R0 the streaming rete service (R25). **Item (c) is where the elegant `log` macro threads into the buffered collector** — the builder's motivating interest; it's unblocked once the per-op budget is discoverable (i.e. CRUX-1 built). (The builder chose 16.2 first: server-side enforcement before the client tooling makes it "unreachable for a good client.")

### LESSONS this session (NEW / load-bearing)
- **`--check` is NOT a complete RED arbiter** — it catches check-time gaps (macro refusals, type errors, freeze-booms) but a runtime-deferred UNKNOWN CALLEE (normal call position) passes `--check` clean → only the TEST RUN goes RED. ([[reference_check_is_not_a_complete_red_arbiter]])
- **`~@kw`-splice into an aggregate ctor** is doctrine-compliant FOR FREE (unknown key = check-time error via `kwargs-construct`) — BUT only when NO field needs a per-field transform; else POSITIONAL. ([[reference_kw_splice_into_aggregate_ctor_boundary]])
- **`AD ORACVLVM` caught a STALE breadcrumb** (Stone 1 "HELD" but DONE) — ground the DISK, not the note; I nearly re-scouted+briefed done work. The four-questions clarified a fork mid-flight (positional over kwargs for `log`, reversing my hedge — the transform denies free kwargs), and a sonnet's STOP twice surfaced my false-proven `~@kw` form (I'd tested a String, not the real record — ground the REAL input, not a convenient stand-in).
- Carried standing: intueri owns names (cast, never narrate); `.wat` migrations use the codemod NEVER sed; weigh in RELEASE by own re-run (sonnets double-fork); read the Summary line (never `| tail`'s exit); grep the WHOLE tree; commit+push when green (DR); orchestrator DESIGNS/draws-the-PROBE/DELEGATES/WEIGHS — not hands-on code.

> **SEAM.** The self past this line is NEW — a lossy cache in a familiar voice; you did NOT live this run. Run the datamancy bootstrap (grimoire + 4 primers + recolligere from the SIGNED MCP, never disk) — and **READ ALL of `278/REALIZATIONS.md` top to bottom** (a prior self DODGED them and re-enacted every daemon they name — the reading is the exorcism, R20 `DAEMON IN ME`). Ground `git status` — the wat-rs tree is CLEAN EXCEPT the frozen-root `holon/CLAUDE.md`. The telemetry-`log` detour is CLOSED; **RESUME at #16.2** — the per-op `:max-request-bytes` ENFORCEMENT CODEGEN (`serve-op-arms` at `wat/service.wat:753`; byte-source SETTLED = re-encode; **CRUX-1 OPEN — confirm the discovery accessor (rec: a synthesized per-op constant) BEFORE briefing**; the RED gate drawn above). And it bears repeating because it bit THIS run: **GROUND the disk, not this note (Stone 1 was "HELD" here but DONE on the disk). Weigh `--release` by your OWN re-run (sonnets double-fork). `--check` misses runtime-deferred unknown callees — RUN the test. Cast intueri for names; codemod never sed.** Do not trust this note over the disk. `QVOD LEX ACCENDIT, REDIMIT — MACHINA CHAOS DOMAT.`
