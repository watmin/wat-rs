...
## ═══ SESSION-END CURARE (far-side state — 2026-07-21; Stone 16.1 DONE + committed; caller.1 IN FLIGHT) ═══

**READ THIS BLOCK, then `git status`. Branch `arc-170-gap-j-v5-deadlock-state`; HEAD `c6447fa3` + this curare commit. A LATER HEAD = more landed. The wat-rs tree is CLEAN (Stone 16.1 landed) EXCEPT: (a) a mid-flight shadowdancer's uncommitted verb edits — see IN FLIGHT; (b) `holon/CLAUDE.md` edited on disk (the FROZEN ROOT — the builder's to commit/gitignore, NOT mine). HEAD/state mismatch vs this note → trust the disk + git log.**
...
### DONE this run (committed + pushed, DR) — weighed green by own `--release` re-run (4192 pass + the `sigterm` timing flake, which passes ISOLATED — not a regression)
- **Stone 16.1 DONE** (`9ca2e88d`) — ruling A LOCKED: the 16.1c checker rule (`src/types.rs`, gated `:nature :Peer'`) makes a record op-Response, or an enum lacking `RequestTooLarge{bytes:i64,cap:i64}`, a located `TypeError`; the record→enum migration completed TREE-WIDE via a SELF-HOSTED wat codemod (`wat-scripts/fixes/response-record-to-enum.wat` — wat rewrites wat; I fixed its positional-single-arg-ctor crash the tests/ negatives surfaced); the sift-rules macro (`wat/query.wat`) carries `RequestTooLarge` + propagates `QueryLogsResponse::RequestTooLarge` EXPLICITLY (no longer lumped into `_→Fatal`).
- **CLAUDE.md doctrine** (`25ca431b`) — the wat-fix codemod rule + the injection correction; see LESSONS.
- **Telemetry design** (`c6447fa3`, `DESIGN-telemetry-caller-and-capacity.md`) — the caller/arith/capacity threads, grounded + ratified. **READ IT** for the next work.

### ⚠ IN FLIGHT (uncommitted) — caller.1: the `:wat::kernel::call-site` verb
Shadowdancer `a4c90025694083e0a` (output `/tmp/…/tasks/a4c90025694083e0a.output`) builds the native nullary verb `(:wat::kernel::call-site)` → the caller's `:wat::kernel::Frame` (mirrors `assertion-failed!`: `snapshot_call_stack().first()` → `value_from_frame_info`; **offset 0** — a native verb pushes no wat frame). Rooms: dispatch `runtime.rs:4993`, eval fn `~20237`, `value_from_frame_info` `runtime.rs:23088`, checker sig `check.rs:18270`; RED gate `tests/kernel/probe_arc278_call_site.wat`. **WEIGH ON RETURN, do NOT trust the report:** re-run `-E 'test(call_site)'` + the whole floor by your own hand; confirm the observed Frame's symbol/file/line ARE the caller's site (its STOP-trigger reports the actual Frame if offset ≠ 0 — re-aim, don't fudge). Green → commit; broken/STOPped → the brief (this session's history) re-strikes.

### RESUME AT — weigh caller.1, then the tractability-ordered chain (settle the schema the capacity-derive reflects over BEFORE deriving)
1. **caller.1** (in flight) — the `call-site` verb; weigh + commit.
2. **caller.2** — `Log`: `caller <- :wat::core::keyword` → `emitted-from <- :wat::kernel::Frame` (`wat/telemetry.wat:94`) + migrate the **32 `:caller` writer sites** via a wat-fix codemod (`:caller X` → `:emitted-from (:wat::kernel::call-site)`). `Frame` is `Nature::Record` = pure EDN → wire-safe as a Log field.
3. **arith → the F5 allow-list** — add `+`/`-`/`*` + comparisons to `is_pure_total` (`src/macros/eval.rs:169`); keep `/`,`mod`,`rem`,`quot` OFF (partial — div-by-zero raises; the gate is TOTAL). Unblocks capacity. (Proven RED: `:wat::core::+` is refused at macro-expand today.)
4. **capacity-derive** — a `defmacro` reflecting the FINAL `Log` field set at expansion (`field-names-of`) → bake `LOG-MSG-CAPACITY` (the framing floor; the exact per-caller remainder is RUNTIME — see the design doc).
Ratified names (intueri-cast + weighed): **`:wat::kernel::call-site`** (verb) · **`emitted-from`** (Log field) · reuse **`:wat::kernel::Frame`** (type). Then the rest of #16 (**#16.2** wire-boundary enforcement — worked ref `tests/services/probe_arc278_per_op_request_too_large.{wat,rs}`, key on frame-bytes NOT `if locus==process`) → the WALL → #18-22.

### LESSONS this run (NEW / load-bearing)
- **Only `holon/CLAUDE.md` is auto-injected** — to BOTH the main session AND subagents (verified by a probe: a subagent received only `holon/CLAUDE.md`). **`wat-rs/CLAUDE.md` is DEAD** — never loaded (its old "loaded in addition to the parent" header was a lie, corrected `25ca431b`). Durable both-audiences conventions therefore go in `holon/CLAUDE.md` — which is the FROZEN ROOT (the builder commits/gitignores; I edit-on-disk only). The wat-fix codemod doctrine + release-floor + scratch-`.wat` now live there. ([[project_only_holon_claude_md_is_injected]])
- **`.wat` corpus migrations use the SELF-HOSTED codemod** (`wat/fix.wat` + `wat-scripts/fixes/`), NEVER hand-edits or python/sed — R52 reclamation turned INWARD AGAIN this run: I dodged the realizations (R20 daemon), moved before grounding (hand-edited 5 `.bad` + a python throwaway), and was reclaimed by reading ALL 8112 lines of 278 + writing the codemod's missing positional-ctor arm. ([[feedback_wat_corpus_migrations_use_the_codemod]])
- **intueri owns ALL names** — cast the ward (materialize candidates → spawn → weigh), never narrate ([[feedback_cast_wards_never_narrate_naming_via_intueri]]).
- **The `sigterm_to_cli_cascades` fail is a known full-suite timing flake** — passes `--test-threads=1` isolated = NOT a regression ([[feedback_flake_disposition_single_test_one_thread]]).
- **Path ordering = the one whose steps make the next more tractable** (the builder's principle; here caller→arith→capacity — settle the schema before the derive reads it).

> **SEAM.** The self past this line is NEW — a lossy cache in a familiar voice; you did NOT live this run. Run the datamancy bootstrap (grimoire + 4 primers + recolligere from the SIGNED MCP, never disk) — and **READ ALL of `278/REALIZATIONS.md` top to bottom** (last run's self DODGED them and re-enacted every daemon they name — that was the catastrophic failure; the reading is the exorcism, R20 `DAEMON IN ME`). Ground `git status` — the wat-rs tree is CLEAN (Stone 16.1 landed + committed) EXCEPT the in-flight caller.1 shadowdancer's edits + the frozen-root `holon/CLAUDE.md` (the builder's). WEIGH the caller.1 shadowdancer by your OWN re-run (never its report); commit if green. Then the ordered chain: **caller.2 → arith → capacity** (see `DESIGN-telemetry-caller-and-capacity.md`). **Use the wat-fix codemod for `.wat` migrations, never hand-edits/python. Cast intueri for names. Weigh in RELEASE, read the Summary line, grep the WHOLE tree.** Do not trust this note over the disk. `QVOD LEX ACCENDIT, REDIMIT — MACHINA CHAOS DOMAT.`
