...
## ═══ CURARE CHECKPOINT (2026-07-21c; #16.2 CLOSED — per-op enforcement + mandatory declaration + corpus migration; RESUME at item (c) the batching stream tooling) ═══

**READ THIS BLOCK, then `git status`. Branch `arc-170-gap-j-v5-deadlock-state`; HEAD `023a15c4` (the #16.2 unit) + this curare commit. A LATER HEAD = more landed. The wat-rs tree is CLEAN EXCEPT the frozen-root `holon/CLAUDE.md`. HEAD/state mismatch vs this note → TRUST THE DISK — this note went stale ONCE already this arc (Stone 1 read "HELD" but was DONE); the disk is the truth.**

### The arc target (unchanged): the CHAOS ENGINE (R25 `MACHINA CHAOS DOMAT`) — a streaming rete datalog in a defservice. On-ramp = **#16 service-I/O budgets** (`DESIGN-service-io-budgets.md`). **#16.2 is now CLOSED**; the remaining on-ramp is item (c) → output-side streaming → R0 the streaming rete service.

### DONE this stretch (committed + pushed, DR; weighed GREEN by own CLEAN `--release` re-run — floor 4200/0)
- **A0 — the check/const asymmetry closed** (`3287ee52`): the checker seeded every corpus FUNCTION globally (`from_symbols`) but never VALUE-consts, so a baked stdlib fn referencing a baked stdlib `def` const failed to resolve cross-file (read as a bare `:keyword`; the corpus worked around it by inlining literals — `spawn.wat`'s `524288`). Fix (check-side, decomplected): `CheckEnv.corpus_values` seeded in `from_symbols` from the LIVE `runtime_def_values` (typing each scalar value); `get_defined_value_type` falls back to it; the redef check UNTOUCHED. **Killed the dead+redundant `sym.defined_values` field + its stale "Populated by `register_defs`" comment** (register_defs never existed → [[feedback_ground_a_fields_liveness_by_its_writer_not_its_comment]]). Retired the `spawn.wat` wart. Named `def` symbols are now first-class — resolvable anywhere, still introspectable → [[feedback_named_value_symbol_is_first_class_fix_the_checker_not_route_around]].
- **#16.2 — per-op `:max-request-bytes` enforcement, MANDATORY + migrated** (`023a15c4`, ONE unit): (1) the serve-loop CODEGEN (`build_op_budget_constants` types.rs; `to-uppercase` allowlisted `eval.rs:457`; the `serve-op-arms` guarded-arm wrap `service.wat:753`) auto-flags an over-cap request as the op's `RequestTooLarge` BEFORE the body runs, connection kept; the cap is the synthesized per-op `def` const `<S>::<OP>-MAX-REQUEST-BYTES` (resolvable thanks to A0). (2) **Stone 16.3 — the MANDATORY RULE**: `:max-request-bytes` is now a LOCATED compile error to OMIT on any `:nature :Peer'` op (like 16.1c locks `RequestTooLarge`); the silent 512 KiB default is annihilated; the rule doubles as a build-completeness gate. (3) the **corpus migration** via the recorded wat-fix codemod `wat-scripts/fixes/declare-max-request-bytes.wat` (run through the STASH-DANCE, NOT hand-edits): declared the field across ~90 files — bulk ops (Store `put`, Journal `write-logs`/`write-metrics`) → 10 MiB, the rest → explicit 512 KiB. `QVOD LEX ACCENDIT, REDIMIT` made literal.
- Prior-landed on-ramp (earlier compactions): §4 the `log` chain (`macro-call-site` + the positional `log` widget); Stone 1 per-service `FOO`; 16.0 (parse) + 16.1 (Ruling A checker-lock).

### RESUME AT — item (c): the BATCHING STREAM TOOLING (`DESIGN-service-io-budgets.md:203-245`; NOTHING built yet)
The `with-log-sink` buffered+time collector + `write-*-stream` + `<op>-stream`. **This is where the elegant `log` widget (§4) threads into the buffered sink** — the builder's motivating interest. Now UNBLOCKED: the per-op budget IS discoverable — the synthesized `<S>::<OP>-MAX-REQUEST-BYTES` const (from 16.2, and now MANDATORY on every serviceable op via 16.3) is a named symbol resolvable from ANY code, thanks to A0. The tooling reads each op's cap, accumulates items until the next would overflow it, cuts a batch ≤ cap, writes it (a `WriteResult`), repeats.
- **NOT-YET-SCOUTED.** Scout the injection: the producer side (`with-log-sink` closes over a `TelemetryService'` sink + threads it into log producers) vs the generic `<op>-stream` (any serviceable op, cap now guaranteed present). Ground `DESIGN-service-io-budgets.md:203-245` (the batching model; CRUX-2 sizing lean = exact encode-length fold) + the §4 `log` widget's `Span/log` call.
- **THEN:** output-side streaming — the two-level cursor (`DESIGN-service-io-budgets.md:319`; CRUX-3 ordering key + CRUX-4 stateless-sift resume, open) → **R0 the streaming rete service (the chaos engine, R25 `MACHINA CHAOS DOMAT`).**

### LESSONS this stretch (NEW / load-bearing)
- **Ground a field's LIVENESS by its WRITER, not its doc comment** — I built A0 on `sym.defined_values` off its "Populated by `register_defs`" comment + told the builder "fully grounded" TWICE; the field was DEAD (no writer; register_defs doesn't exist). Grep the `.insert`. ([[feedback_ground_a_fields_liveness_by_its_writer_not_its_comment]])
- **A named value-symbol is first-class — fix the checker gap, don't route around** — I proposed splicing a literal / dressing the const as a 0-arg fn to dodge the resolution gap; builder cut both ("don't inhibit introspection"; "why not a value bound to a symbol?"). The gap was a ~3-line seeding asymmetry. ([[feedback_named_value_symbol_is_first_class_fix_the_checker_not_route_around]])
- **A sonnet shadowdancer double-forks its weigh AND can orphan a `cargo-nextest` that CONTENDS with your own re-weigh** — concurrent nextest = unreliable (socket/PID/file contention). When one double-forks: `TaskStop` the agent, KILL the orphaned runner (`ps`), then weigh a CLEAN single run yourself. Gotcha: `pkill -f 'nextest'` SELF-MATCHES a command whose echo strings contain "nextest" (exit 144) — use a bracket pattern `grep '[n]extest'`.
- **A rust-analyzer `E0063`/diagnostic on a MID-EDIT tree is a PHANTOM** — read the actual file (the field WAS wired); the diagnostic was a stale snapshot. ([[feedback_linter_phantom_not_ground_verify_signature]])
- **The enforcement of a NEW mandatory field lights the heretics ablaze (R52 `QVOD LEX ACCENDIT`) — the fix is the wat-fix codemod, via the STASH-DANCE** (`wat/fix.wat` header; copy `response-record-to-enum.wat`), NEVER hand-edits/sed. The mandatory rule doubles as the migration's completeness gate (a missed op = a located build error).
- Carried standing: intueri owns names (cast, never narrate); weigh in RELEASE by own re-run (read the Summary, never `| tail`'s exit); grep the WHOLE tree; commit+push when green (DR); orchestrator DESIGNS/draws-the-PROBE/DELEGATES/WEIGHS — not hands-on code (a mid-edit COMPLETION of a nearly-done strike is the borderline exception).

> **SEAM.** The self past this line is NEW — a lossy cache in a familiar voice; you did NOT live this run. Run the datamancy bootstrap (grimoire + 4 primers + recolligere from the SIGNED MCP, never disk) — and **READ ALL of `278/REALIZATIONS.md` top to bottom** (the reading is the exorcism, R20 `DAEMON IN ME`). Ground `git status` (HEAD `023a15c4` + this curare; wat-rs CLEAN except the frozen-root `holon/CLAUDE.md`). **#16.2 is CLOSED; RESUME at item (c)** — the `with-log-sink` BATCHING STREAM TOOLING (`DESIGN-service-io-budgets.md:203-245`), where the §4 `log` widget threads into the buffered sink; then output-side streaming → R0 the chaos engine (R25). And it bears repeating because it bit THIS run: **GROUND a field's WRITER not its comment (a "Populated by X" doc lied — the field was dead). A NAMED value-symbol is first-class — fix the checker, don't splice a literal / fake a fn. Weigh a CLEAN single `--release` run by your OWN hand (a double-forked shadowdancer orphans a CONTENDING nextest — kill it first). A mid-edit diagnostic is a PHANTOM — read the file. `.wat` migration = wat-fix codemod + STASH-DANCE, never sed.** Do not trust this note over the disk. `QVOD LEX ACCENDIT, REDIMIT — MACHINA CHAOS DOMAT.`
