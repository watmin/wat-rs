...
giving up the retry**. Read `DESIGN-find-the-934-milliseconds.md` first — it carries the A/B
...
`SCORE-what-does-publish-actually-cost.md` — the arc's prior measurement stone, same shape.

Write `SCORE-find-the-934-milliseconds.md`, then `pulsare_yield kind=scored`.
