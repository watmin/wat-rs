...
## STATUS (2026-07-21c) — #16.2 DONE (`023a15c4`): the enforcement CODEGEN + Stone 16.3 (`:max-request-bytes` MANDATORY on `:nature :Peer'` ops) + the corpus migration (`wat-scripts/fixes/declare-max-request-bytes.wat`, ~90 files); floor 4200/0. RESUME at item (c) — the `with-log-sink` batching stream tooling (§ below). [Historical scout of 16.2 preserved below.]
