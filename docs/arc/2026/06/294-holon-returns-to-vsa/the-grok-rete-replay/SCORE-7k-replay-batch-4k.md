...
| E19 | **PASS — see the disclosures under E4/E5/E6/E8/E10 above and #342/#344/#349's own commit bodies, plus (d) and (e) below — both folded in after this SCORE was first written.** | Every rename, bank-rejection, quarantine-expansion and count deviation appears in the SCORE row it affects, not buried only in `REPLAY-LOG.md`. (d) Per `BRIEF-7k-ADDENDUM-352-the-var-render-pin.md`: the orchestrator's post-batch floor at `a9d09e504` was RED 1-of-5792 — `tests/comms/probe_arc214_stone46b_select_prime.rs`'s `probe_2_select_wrong_return_annotation_rejected`, a MAIN-ONLY matcher (last touched at `4b49f3c5c`, never touched by grok's #352) pinned to a `:?NNNN` prefix for a reason (non-determinism) that #352's own render fix retired. Folded into #352 itself (finding 38's fourth instance) via detach/amend/`rebase --onto`. Cure: exact equality on the whole `got` string (`"(:wat::spawn::ServiceEvent :- [:wat::core::i64 :wat::core::i64 _])"`), measured — not assumed — via 5 fresh-process `--check` runs, byte-identical every time, and confirmed against the test's own pre-fix failure message on this tree. (e) That same exact-equality literal is a COMPLETE, reader-parseable form (the old prefix's brackets never closed, so it never parsed) — the orchestrator's NEXT floor was RED 1-of-5792 again, `wat::lint::no_inlined_wat_in_tests::tests_carry_no_inlined_wat`, finding 33's class. Ruled 4-YES: kept exact equality, added `// rune:lint(no-inlined-wat)` directly above the matcher (model: `tests/services/probe_arc170_c2_d_bodiless_edge.rs:43`, `tests/function/stone18a_errors.rs:22`), reason stated honestly for this file. Folded into #352 a second time, same commit; #353→#360 and this SCORE rebuilt on top BOTH times (final #352 `aa09e0aaf`, final tip before this docs commit `67a86b2dc`). `lint-subset`/`kind(lib)`/`doctest` re-measured (not assumed) at #352 and at #360 after both amends — every number identical to the pre-existing record lines. Full account in `REPLAY-LOG.md`'s "#352 VAR-RENDER FOLD" and "PART 2" entries and `BRIEF-7k-ADDENDUM-352`'s Section 2. |
...
| E22 | **PASS — five real disagreements with the brief, reported as results, not forced to match (the fourth and fifth landed after this SCORE's own first pass).** | (1) D10 (#342/#344) is NOT live on this tree — main's own arc-277 work (`6df9ba1ab`, an ancestor of the replay base) already closed the exact gap grok found, before grok found it. (2) D11 (#349) is likewise mostly redundant with main's own #262 convergence, with one genuine residual (computed-nested-operand) — the SAME class of disagreement, one step later. (3) `QUARANTINE_LEN` (#352) did NOT hold at 3 as the brief's own pre-flight implied it might — it moved to 7, a real, measured, disclosed deviation (E8). (4) The orchestrator's post-batch verification floor found a fifth: a main-only prefix-pin matcher (`tests/comms/probe_arc214_stone46b_select_prime.rs`) went stale under #352's own render fix, RED 1-of-5792, per `BRIEF-7k-ADDENDUM-352-the-var-render-pin.md` — folded into #352, see E19(d) and item 6 below. (5) That fold's own exact-equality cure then made the pinned literal reader-parseable, tripping `tests_carry_no_inlined_wat` — RED 1-of-5792 a second time, ruled 4-YES to keep exact equality and add the house rune rather than re-pin a shorter prefix — see E19(e) and item 7 below. None of these was bent to match a forecast; all five are measured and reported plainly in the commits that found them. |
...
6. **The #352 Var-render fold, landed after this SCORE's own first pass.** The orchestrator's
   verification floor at `a9d09e504` came back RED 1-of-5792 on a main-only matcher pinned to a
   `:?NNNN` prefix that #352's own render fix (`_` in place of `:?{id}`) retired. Folded into #352
   (never a repair commit) via detach/amend/`rebase --onto`, per
   `BRIEF-7k-ADDENDUM-352-the-var-render-pin.md`; the exact `got` string was independently
   re-measured (5 fresh-process `--check` runs, byte-identical) rather than trusted on the
   addendum's word. Full account in E19(d) above and `REPLAY-LOG.md`'s "#352 VAR-RENDER FOLD"
   entry. New #352 `8f87cd9d8`, new tip `f87ed74e7` (old tip `a9d09e504`).
7. **A second red in the same step, caused by item 6's own cure.** The exact-equality literal item
   6 landed is a COMPLETE, wat-reader-parseable form (the old prefix's brackets never closed, so it
   never parsed) — the orchestrator's next floor was RED 1-of-5792 again,
   `wat::lint::no_inlined_wat_in_tests::tests_carry_no_inlined_wat`, finding 33's class. Ruled
   4-YES: exact equality stands (correct per item 6); added `// rune:lint(no-inlined-wat)` directly
   above the matcher, modeled on `tests/services/probe_arc170_c2_d_bodiless_edge.rs:43` and
   `tests/function/stone18a_errors.rs:22`, reason stated honestly for this file (parseable only
   since #352/C19's render fix). Did not split or reshape the literal. Folded into #352 a second
   time via a further detach/amend/`rebase --onto` (this time replaying #353→#360, this SCORE
   commit, AND item 6's own docs commit together, so its content could be extended rather than
   duplicated). `lint-subset`/`kind(lib)`/`doctest` re-measured (not assumed) at the twice-amended
   #352 (267/1496/8, all unchanged) and at the rebuilt #360 (293/1496/8, all unchanged). Full
   account in E19(e) above and `REPLAY-LOG.md`'s "#352 VAR-RENDER FOLD, PART 2" entry and
   `BRIEF-7k-ADDENDUM-352`'s Section 2. Final #352 `aa09e0aaf` (old `8f87cd9d8`), new SCORE-commit
   tip `67a86b2dc` (old `f87ed74e7`); this docs commit itself is further amended on top to record
   both the item-6 and item-7 accounts together.
