...
- **#328's green** is bought by: (a) a real correctness fix beyond the cherry-pick itself — grok's
  ...
  `format!`; **(d) — folded in after this SCORE was first written, per
  `BRIEF-7j-ADDENDUM-328-two-stale-goldens.md`** — two MAIN-ONLY goldens the D6 doc-comment rewrite
  left stale (`tests/cli/pprintln_doc_row__step_payload.edn`, regenerated by capturing
  `target/release/wat`'s stdout directly, no bless path; and
  `tests/reflection/probe_stone_metadata_of_whole_row__step_payload_row.edn`, regenerated with
  `UPDATE_EDN=1`), both re-measured "which keys moved" with no filter and confirmed prose-only
  (`:doc` only — 30 lines in Golden A, 1 line in Golden B; every other key byte-identical), folded
  into #328 itself via detach/amend/`rebase --onto` with #329→#340 rebuilt unchanged on top (new tip
  `e7bcb65fc`; inertness and published-history proofs in REPLAY-LOG.md's "#328 GOLDEN FOLD" entry).
  Absent this fold, the orchestrator's own post-batch floor was RED 2-of-5735.
...
6. **The #327 verdict-line wrap, found late and repaired via rebuild-descendants** — the most
   ...
   into a "PASS" row.
7. **The #328 golden fold, landed after this SCORE's own first pass** — the orchestrator's
   verification floor at `24ca5ef5c` came back RED 2-of-5735 on two main-only goldens D6's doc
   rewrite left stale. Folded into #328 (never a repair commit) via detach/amend/`rebase --onto`,
   per `BRIEF-7j-ADDENDUM-328-two-stale-goldens.md`; both regenerations independently re-measured
   prose-only (no filter) rather than trusted on the addendum's word. Full account in E19 above and
   in REPLAY-LOG.md's "#328 GOLDEN FOLD" entry.
