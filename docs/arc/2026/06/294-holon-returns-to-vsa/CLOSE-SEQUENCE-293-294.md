...
11. ◐ **294.f — reflection-IR → WatAST** *(294, step 8)* — **COMMON CASE LANDED (2026-07-18), pulled ahead of the PHASE-1 gate by builder decree** (*"i do not wish to bear this cost any longer — the crusades revealed the pressure"*). Reflection now emits canonical `wat.type/` **plain EDN** (spec-perfect: `(:anonymous (n wat.type/i64) (s wat.type/String) -> wat.type/String)`, no `#wat-edn.holon`, no `::`, no `<>`): `type_expr_to_ast`→`type_expr_to_clojure_form`; 14 producers flipped `holon__HolonAST`→`wat__WatAST`; verbs `extract-arg-names`/`extract-arg-types`/`rename-callable` + `check.rs` return-types retyped to `WatAST`; `holon_type_ast_to_wat_type_form` **DELETED**. Committed with the edn-crusade (green floor 4195/0). **DEFERRED — the proper clojure flip** (`grep -rn 'rune:clojure-flip'` finds every bridge): the 8 edge cases (multi-slash keywords `__internal/…`/`Vector/length` + `<T,Acc>` multi-param generics need the symmetric faithful codec `keyword_from_wat_path`↔`ns_to_wat_path`, drop-`<>`-in-names) + the `:-` typed-clojure sigils (`(n :- wat.type/i64)`). **294.d + 294.e (below) STILL GATED behind PHASE 1** — the wire-kill (`HolonRepresentable` + `#wat-edn.holon` tags) and the `HolonAST→Hologram` rename are UNTOUCHED; only reflection's producer/verb/checker surface flipped. The `probe_diagnostic_typed_entities` VSA fixtures + `Bundle/*`/`hologram.rs` were correctly left alone (genuine holographic).
