...
;;
;; RED at HEAD: today `stop` is `<svc>/stop [c <- client-peer-ty]` (service.wat:587) — a CLIENT method
;; sending Op::Stop over the dial peer. `(<svc>/stop h)` where `h` is a `<svc>::Handle` is a TYPE ERROR
;; (Handle ≠ client-peer-ty). That type error IS the gap strike-3 fills: stop is client-gated today;
;; the facet split makes it Handle-gated (owner-only) by construction. GREEN once stop takes the Handle,
;; dials the private admin surface the client cannot reach, and returns the final state.
...
(:wat::service::defservice :wat-tests::admin-counter
  ...
  [(:Increment [s <- :State n <- :wat::core::i64]
               -> [value <- :wat::core::i64]
     (:wat::core::let [s' (:wat::core::i64::+ (:wat-tests::admin-counter::State/count s) n)]
       (:wat::service::Outcome::Reply (:wat-tests::admin-counter::State s')
         (:wat-tests::admin-counter::IncrementResponse s'))))]
  :init (:wat::core::fn [seed <- :wat::core::i64] -> :wat-tests::admin-counter::State
          (:wat-tests::admin-counter::State seed)))
...
(:wat::test::deftest' :wat-tests::service::admin-stop-on-thread
  ...
  (:wat::test::assert-eq
    (:wat::core::let
      [h (:wat-tests::admin-counter/start (:wat::spawn::thread) 0)
      ...
      (:wat-tests::admin-counter::State/count final))
    7))
...
(:wat::test::deftest' :wat-tests::service::admin-stop-on-process
  ...
  (:wat::test::assert-eq
    (:wat::core::let
      [h (:wat-tests::admin-counter/start (:wat::spawn::process) 0)
       c (:wat::kernel::connect' (:wat-tests::admin-counter::Handle/addr h))
       _ (:wat-tests::admin-counter/increment c (:wat-tests::admin-counter/increment-request 7))
       final (:wat-tests::admin-counter/stop h)]
      (:wat-tests::admin-counter::State/count final))
