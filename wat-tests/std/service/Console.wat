...
;; discipline) — the driver thread would panic writing to it. So
;; these tests use run-sandboxed-HERMETIC: fresh subprocess, real
;; stdio, thread-safe writes. Same tradeoff as the Rust-era wat_cli.rs
;; Console tests which shell out to the built binary.
...
(:wat::test::deftest :wat-tests::std::program::Console::test-hello-world 1024 :error
  (:wat::core::let*
...
;; message. The writes race; the test sorts the output before
;; comparing to dodge the scheduler-order nondeterminism.

(:wat::test::deftest :wat-tests::std::program::Console::test-multi-writer 1024 :error
  (:wat::core::let*
    (((r :wat::kernel::RunResult)
      (:wat::kernel::run-sandboxed-hermetic
        "(:wat::config::set-dims! 1024)
        ...
        :None))
     ((stdout :Vec<String>) (:wat::kernel::RunResult/stdout r))
     ;; Assert the set of lines, not the order — three workers race
     ;; across threads and the scheduler picks the write order.
     ((seen-alpha :bool)
